    ...
    , m_wactSearchEdit(nullptr)
{
    ...
    phsModel* pModel = UiHelper::getGraphsBySubg(m_subgIdx);
    ZASSERT_EXIT(pModel);    m_cateActions = getCategoryActions(pModel, m_subgIdx, m_scenePos);
...
QList<QAction*> ZenoNewnodeMenu::getCategoryActions(IGraphsModel* pModel, QModelIndex subgIdx, QPointF scenePos)
{
    Q_ASSERT(pModel);
    if (!pModel)
