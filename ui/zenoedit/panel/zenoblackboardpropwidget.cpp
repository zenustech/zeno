    ...
    m_subgIdx(subIndex) 
{
    ...
    insertRow("background", PARAM_CONTROL::CONTROL_COLOR, info.background, 0, pGroupLayout);
...
void ZenoBlackboardPropWidget::insertRow(const QString &desc, const PARAM_CONTROL &ctrl, const QVariant &value, int row,QGridLayout *pGroupLayout) {    
    ...
    cbSet.cbEditFinished = [=](QVariant newValue) {
        ...
        if (!pModel)
            return;
        pModel->updateBlackboardByParam(m_idx.data(ROLE_OBJID).toString(), desc, newValue, m_subgIdx, true);
    };
