...
void ZenoSceneTreeModify::generateModificationNode(QString outNodeId, QString outSock, QString inNodeType, QString inSock, QString inModifyInfoSock, Json& msg)
{
    ...
    if (outNodeId.contains("SetSceneXform"))//输出来自SetSceneXform节点
    {
        ...
        QModelIndex nodeIdx = pModel->indexFromName(outNodeId);
        ZASSERT_EXIT(nodeIdx.isValid());

        QModelIndex existModifyNode;
        ParamsModel* paramsM = pModel->params(nodeIdx);
        QModelIndex outSockIdx = paramsM->paramIdx(outSock, false);
        const auto& links = outSockIdx.data(QtRole::ROLE_LINKS).value<PARAM_LINKS>();

        for (auto link : links)
        {
            zeno::EdgeInfo info = link.data(QtRole::ROLE_LINK_INFO).value<zeno::EdgeInfo>();
            QModelIndex inIdx = pModel->indexFromName(QString::fromStdString(info.inNode));
            if (inIdx.data(QtRole::ROLE_CLASS_NAME).toString() == inNodeType) {
                existModifyNode = inIdx;
                break;
            }
        }
        if (existModifyNode.isValid()) {
            addMultiLineStrToXform(existModifyNode.data(QtRole::ROLE_NODE_NAME).toString(), msg, inModifyInfoSock);
        }
        else {
            QPointF pos = nodeIdx.data(QtRole::ROLE_OBJPOS).toPointF();
            zeno::NodeData newNodeDat = pModel->createNode(inNodeType, "", pos);

            QString newNodeident = QString::fromStdString(newNodeDat.name);
            //pModel->updateNodeStatus(newNodeident, info, subgIdx, false);

            QModelIndex newNodeIdx = pModel->indexFromName(newNodeident);
            auto inNodeParams = newNodeIdx.data(QtRole::ROLE_PARAMS).value<ParamsModel*>();
            QModelIndex inSockIdx = inNodeParams->paramIdx(inSock, true);
            auto outNodeParams = nodeIdx.data(QtRole::ROLE_PARAMS).value<ParamsModel*>();
            QModelIndex outSockIdx = outNodeParams->paramIdx(outSock, false);

            pModel->addLink(outNodeId, outSock, newNodeident, inSock);

            pModel->setView(nodeIdx, false);    //关掉旧节点的view
            pModel->setView(newNodeIdx, true);  //新节点开view
...
void ZenoSceneTreeModify::addMultiLineStrToXform(QString outNodeId, Json& msg, QString inModifyInfoSock)
{
    ...
    if (multistring_uuid.size()) {//还在修改状态没有运行，
        QModelIndex multilinestrIdx = pModel->indexFromName(QString::fromStdString(multistring_uuid));
        if (multilinestrIdx.isValid()) {//multilinestr的idx有效
            bool linkedToSetSceneXform = false;

            ParamsModel* paramsM = pModel->params(multilinestrIdx);
            QModelIndex outSockIdx = paramsM->paramIdx("value", false);
            const auto& links = outSockIdx.data(QtRole::ROLE_LINKS).value<PARAM_LINKS>();
            for (auto link : links)
            {
                zeno::EdgeInfo info = link.data(QtRole::ROLE_LINK_INFO).value<zeno::EdgeInfo>();

                QString inNode = QString::fromStdString(info.inNode);
                std::string insock = info.inParam;
                int slashpos = insock.find_first_of('/');
                QModelIndex inIdx = pModel->indexFromName(inNode);
                if (inIdx.isValid() && inNode == outNodeId && slashpos != std::string::npos && insock.substr(0, slashpos) == "xformsList") {
