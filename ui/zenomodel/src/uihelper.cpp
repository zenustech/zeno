...
bool UiHelper::validateVariant(const QVariant& var, const QString& type)
{
    ...
    case CONTROL_CURVE:
    {
        return (varType == QMetaType::VoidStar || varType == QMetaType::QVariantMap);
    }
...
QVariant UiHelper::parseVarByType(const QString& descType, const QVariant& var, QObject* parentRef)
{
    ...
    {
        if (varType == QMetaType::VoidStar || varType == QMetaType::QVariantMap)
        {
            return var;
        }
...
QVariant UiHelper::parseJsonByType(const QString& descType, const rapidjson::Value& val, QObject* parentRef)
{
    ...
    else if (descType.startsWith("vec"))
    {
        ...
        if (!val.HasMember("x") && !val.HasMember("y") && !val.HasMember("z")) { //compatible old version zsg file
            ...
            curves.insert("x", QVariantPtr<CurveModel>::asVariant(xModel));
        } else {
            for (auto i = val.MemberBegin(); i != val.MemberEnd(); i++) {
                if (i->value.IsObject()) {
                    ...
                    curves.insert(i->name.GetString(), QVariantPtr<CurveModel>::asVariant(pModel));
                }
            }
        }
