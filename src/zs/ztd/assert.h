...
#define __ZS_ZTD_ASSERT_COM ,
#define __ZS_ZTD_ASSERT_TST(y, ...) y
#define __ZS_ZTD_ASSERT_EXT(xs, ...) __VA_OPT__(": {}"), xs __VA_OPT__(__ZS_ZTD_ASSERT_COM) __VA_ARGS__
#define ZS_ZTD_ASSERT(x, ...) (([&] (auto &&__assert_val) -> decltype(auto) { \
    [[unlikely]] if (!((__assert_val) __ZS_ZTD_ASSERT_TST(__VA_ARGS__))) \
        throw zs::ztd::format_error("AssertionError: {}" __ZS_ZTD_ASSERT_EXT(#x __VA_OPT__(__ZS_ZTD_ASSERT_COM) __VA_ARGS__)); \
