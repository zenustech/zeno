...
struct levelGen{
		...
		int ni, int nj, int nk)
	{
		...
		{
			LosTopos::Vec3i ijk = Dof_ijk_fine[idx_f];
			int coarse_i = ijk[0] / 2, coarse_j = ijk[1] / 2, coarse_k = ijk[2] / 2;
			unsigned int idx = coarse_i + nni * (coarse_j +  coarse_k*nnj);
			if (index_mapping.find(idx) == index_mapping.end())
			{
				...
				Dof_ijk_coarse.push_back(LosTopos::Vec3i(coarse_i, coarse_j, coarse_k));
			}
			else
			{
		...
		std::vector<LosTopos::Vec3i> & Bulk_ijk)
	{
			...
			[&](size_t index)
		{
			for (int kk = 0;kk < base;kk++)for (int jj = 0;jj < base;jj++)for (int ii = 0;ii < base;ii++)
			{
				...
				Dof_ijk_coarse[coarse_idx] = base*Bulk_ijk[index] + LosTopos::Vec3i(ii, jj, kk);
				for (int c = 0;c < 8;c++)
				{
	...
	(std::vector<FixedSparseMatrix<T> *> &A_L,
			...
			[&](size_t index)
		{
			for (int i = 0;i < 512;i++)
			{
				...
				Dof_ijk_fine[g_idx] = 8*Bulk_ijk[index] + LosTopos::Vec3i(ii, jj, kk);
			}
		});
		A_L.resize(0);
		S_L.resize(0);
		...
		A_L.push_back(&A);
		b_L.push_back(new std::vector<bool>);
		S_L.push_back(LosTopos::Vec3i(0, 0, 0));
		unsigned int unknowns = A.n;
		//parallel build
		int base = 8;
		// TODO: temp
		std::vector<LosTopos::Vec3i> temp;
...
//			Dof_ijk_fine.swap(std::vector<LosTopos::Vec3i>());
		...
		{
			nni = std::max(Bulk_ijk[i][0] + 1, nni);
			nnj = std::max(Bulk_ijk[i][1] + 1, nnj);
			nnk = std::max(Bulk_ijk[i][2] + 1, nnk);
		}
		S_L[S_L.size() - 1] = LosTopos::Vec3i(nni, nnj, nnk);
		//sequential build
		while (unknowns > 4096)
		{
			A_L.push_back(new FixedSparseMatrix<T>);
