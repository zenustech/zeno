...
struct chunck3D {
	...
	T&
		operator()(const LosTopos::Vec3i& ijk)
	{
		auto i = ijk[0];
		auto j = ijk[1];
		auto k = ijk[2];
...
struct fluid_Tile
{
	...
	chunck3D<unsigned char, N> eight_solid;
	chunck3D<unsigned char, N> any_solid;
	...
	fluid_Tile(const LosTopos::Vec3i & corner, uint index)
	{
		tile_corner = corner;
		bulk_index = index;
		is_boundary = false;
