...
public:
	...
	struct Laplacian_with_level {
		...
		//return a shared pointer of the result of L*in_rhs where L is the laplacian matrix
		openvdb::FloatGrid::Ptr apply(openvdb::FloatGrid::Ptr in_rhs);

		void residual_apply_assume_topo(openvdb::FloatGrid::Ptr in_out_residual, openvdb::FloatGrid::Ptr in_lhs, openvdb::FloatGrid::Ptr in_rhs);
		void Laplacian_apply_assume_topo(openvdb::FloatGrid::Ptr in_out_result, openvdb::FloatGrid::Ptr in_lhs);
		void weighted_jacobi_apply_assume_topo(openvdb::FloatGrid::Ptr in_out_updated_lhs, openvdb::FloatGrid::Ptr in_lhs, openvdb::FloatGrid::Ptr in_rhs);
		void SPAI0_apply_assume_topo(openvdb::FloatGrid::Ptr in_out_updated_lhs, openvdb::FloatGrid::Ptr in_lhs, openvdb::FloatGrid::Ptr in_rhs);
		template<bool red_first>
		void RBGS_apply_assume_topo_inplace(openvdb::FloatGrid::Ptr scratch_pad, openvdb::FloatGrid::Ptr in_out_lhs, openvdb::FloatGrid::Ptr in_rhs);
		void inplace_add_assume_topo(openvdb::FloatGrid::Ptr in_out_result, openvdb::FloatGrid::Ptr in_rhs);

		//return a shared pointer of a float grid that matches the mask of diagonal
		openvdb::FloatGrid::Ptr get_zero_vec_grid();

		void set_grid_constant_assume_topo(openvdb::FloatGrid::Ptr in_out_grid, float constant);

		void set_grid_to_result_after_first_jacobi_assume_topo(openvdb::FloatGrid::Ptr in_out_grid, openvdb::FloatGrid::Ptr in_rhs);
		void set_grid_to_result_after_first_SPAI_assume_topo(openvdb::FloatGrid::Ptr in_out_grid, openvdb::FloatGrid::Ptr in_rhs);
		template<bool red_first=true>
		void set_grid_to_result_after_first_RBGS_assume_topo(openvdb::FloatGrid::Ptr in_out_grid, openvdb::FloatGrid::Ptr in_rhs);
		//turn the input grid to std vector of length ndof
		void grid2vector(std::vector<float>& out_vector, openvdb::FloatGrid::Ptr in_grid);

		void vector2grid(openvdb::FloatGrid::Ptr out_grid, const std::vector<float>& in_vector);

		//assume the out_grid already has the same topology as the degree of freedom index tree
		void vector2grid_assume_topo(openvdb::FloatGrid::Ptr out_grid, const std::vector<float>& in_vector);

		void restriction(openvdb::FloatGrid::Ptr out_coarse_grid, openvdb::FloatGrid::Ptr in_fine_grid, const Laplacian_with_level& child);
		template <bool inplace_add>
		void prolongation(openvdb::FloatGrid::Ptr out_fine_grid, openvdb::FloatGrid::Ptr in_coarse_grid, const Laplacian_with_level& parent);

		void trim_default_nodes();
		static void trim_default_nodes(openvdb::FloatGrid::Ptr in_out_grid, float default_val, float epsilon);
		...
		std::unique_ptr<openvdb::tree::LeafManager<openvdb::Int32Tree>> m_dof_leafmanager;
	...
	void Vcycle(const openvdb::FloatGrid::Ptr in_out_lhs, const openvdb::FloatGrid::Ptr in_rhs, int n1=1, int n2=2, int ncoarse=40);
	template<int mu_time, bool skip_first_iter>
	void mucycle_RBGS(const openvdb::FloatGrid::Ptr in_out_lhs, const openvdb::FloatGrid::Ptr in_rhs, const int level, int n1 = 2, int n2 = 2);

	//scheduled relaxed jacobi see Efficient relaxed-Jacobi smoothers for multigrid on parallel computers
	template<int mu_time, bool skip_first_iter>
	void mucycle_SRJ(const openvdb::FloatGrid::Ptr in_out_lhs, const openvdb::FloatGrid::Ptr in_rhs, const int level=0, int n=3);

	//SPAI0 smoother mu cycle
	template<int mu_time, bool skip_first_iter>
	void mucycle_SPAI0(const openvdb::FloatGrid::Ptr in_out_lhs, const openvdb::FloatGrid::Ptr in_rhs, const int level = 0, int n = 3);


	template<bool skip_first_iter>
	void Kcycle_SRJ(const openvdb::FloatGrid::Ptr in_out_lhs, const openvdb::FloatGrid::Ptr in_rhs, const int level = 0, int n = 3);
	...
	std::shared_ptr<Eigen::SimplicialLDLT<Eigen::SparseMatrix<float>>> m_coarsest_solver;
	void construct_coarsest_exact_solver();
	void write_coarsest_eigen_rhs(Eigen::VectorXf& out_eigen_rhs, openvdb::FloatGrid::Ptr in_rhs);
	void write_coarsest_grid_solution(openvdb::FloatGrid::Ptr in_out_result, const Eigen::VectorXf& in_eigen_solution);
...
private:
	...
	float lv_dot(openvdb::FloatGrid::Ptr a, openvdb::FloatGrid::Ptr b, int level = 0);
	//y = a*x+y;
	void lv_axpy(const float alpha, openvdb::FloatGrid::Ptr in_x, openvdb::FloatGrid::Ptr in_out_y, int level = 0);
	//y = x + a*y;
	void lv_xpay(const float alpha, openvdb::FloatGrid::Ptr in_x, openvdb::FloatGrid::Ptr in_out_y, int level = 0);
	//out = a*x + b*y;
	void lv_out_axby(openvdb::FloatGrid::Ptr out_result, const float alpha, const float beta, openvdb::FloatGrid::Ptr in_x, openvdb::FloatGrid::Ptr in_y, int level = 0);
	void lv_copyval(openvdb::FloatGrid::Ptr a, openvdb::FloatGrid::Ptr b, int level = 0);
	int m_iteration;
	int m_max_iter;
