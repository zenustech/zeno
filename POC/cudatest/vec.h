...
struct vec {
    ...
    explicit constexpr vec(T const &x) {
    ...
    template <class S = T>
    constexpr vec(std::initializer_list<S> const &x) {
        T val;
        auto it = x.begin();
        for (size_t i = 0; i < N; i++) {
            if (it != x.end())
    ...
    template <class ...Ts>
    constexpr vec(T const &t, Ts const &...ts) : vec({t, T(ts)...}) {}

    constexpr operator std::conditional_t<N == 1, T, std::tuple<char, char, std::true_type> const &>() const {
        if constexpr (N == 1) {
            return (*this)[0];
        } else {
            return {42, -42, {}};
    ...
    template <class S>
    explicit constexpr vec(vec<N, S> const &x) {
        for (size_t i = 0; i < N; i++) {
            (*this)[i] = T(x[i]);
        }
    }
    ...
    template <class S>
    constexpr operator vec<N, S>() const {
        vec<N, S> res;
        for (size_t i = 0; i < N; i++) {
            res[i] = (*this)[i];
        }
...
template <class OtherT, class T, std::enable_if_t<!is_vec_v<T>, int> = 0>
auto constexpr vec_to_other_ce(T const &a) {
    return (OtherT)a;
}

template <class OtherT, class T>
auto constexpr vec_to_other_ce(vec<1, T> const &a) {
    return OtherT(a[0]);
}

template <class OtherT, class T>
auto constexpr vec_to_other_ce(vec<2, T> const &a) {
    return OtherT(a[0], a[1]);
}

template <class OtherT, class T>
auto constexpr vec_to_other_ce(vec<3, T> const &a) {
    return OtherT(a[0], a[1], a[2]);
}

template <class OtherT, class T>
auto constexpr vec_to_other_ce(vec<4, T> const &a) {
    return OtherT(a[0], a[1], a[2], a[3]);
}

template <class OtherT, size_t N, class T>
auto constexpr vec_to_other_ce(vec<N, T> const &a) {
    OtherT res;
    for (size_t i = 0; i < N; i++) {
        res[i] = a[i];
    }
...
template <size_t N, class OtherT>
auto constexpr other_to_vec_ce(OtherT const &x) {
    vec<N, std::decay_t<decltype(x[0])>> res;
    for (size_t i = 0; i < N; i++) {
        res[i] = x[i];
    }
...
inline constexpr auto vapply(F const &f, vec<N, T> const &a) {
...
inline constexpr auto vapply(F const &f, T const &a) {
    return f(a);
}

template <size_t N, class T, class S, class F>
inline constexpr auto vapply(F const &f, vec<N, T> const &a, vec<N, S> const &b) {
...
inline constexpr auto vapply(F const &f, T const &a, vec<N, S> const &b) {
...
inline constexpr auto vapply(F const &f, vec<N, T> const &a, S const &b) {
...
inline constexpr auto vapply(F const &f, T const &a, S const &b) {
    return f(a, b);
}
...
inline constexpr auto operator op(T const &a, S const &b) -> decltype(auto) { \
...
inline constexpr vec<N, T> &operator op##=(vec<N, T> &a, S const &b) { \
    a = a op b; \
    return a; \
}
_PER_IOP2(+)
...
inline constexpr auto operator op(T const &a) { \
...
inline constexpr auto func(T const &a, S const &b) -> decltype(auto) { \
...
template <class T> \
inline constexpr auto func(T const &a) { \
...
inline constexpr auto vcast(T const &a) {
    return vapply([](auto const &x) { return (To)x; }, a);
}

template <class T>
inline constexpr auto fract(T const &a) {
    return a - floor(a);
}

template <class T>
inline constexpr auto ifloor(T const &a) {
...
template <size_t N, class T>
inline constexpr bool vany(vec<N, T> const &a) {
    bool ret = false;
    for (size_t i = 0; i < N; i++) {
        ret = ret || (bool)a[i];
    }
...
template <class T>
inline constexpr bool vany(T const &a) {
    return (bool)a;
}

template <size_t N, class T>
inline constexpr bool vall(vec<N, T> const &a) {
    bool ret = true;
    for (size_t i = 0; i < N; i++) {
        ret = ret && (bool)a[i];
    }
...
template <class T>
inline constexpr bool vall(T const &a) {
    return (bool)a;
}

inline constexpr auto dot(float a, float b) {
    return a * b;
}

template <size_t N, class T, class S>
inline constexpr auto dot(vec<N, T> const &a, vec<N, S> const &b) {
...
template <size_t N, class T>
inline constexpr auto lengthSquared(vec<N, T> const &a) {
...
template <size_t N, class T>
inline constexpr auto length(vec<N, T> const &a) {
    std::decay_t<decltype(a[0])> res(0);
    for (size_t i = 0; i < N; i++) {
        res += a[i] * a[i];
    }
...
template <size_t N, class T>
inline constexpr auto distance(vec<N, T> const &a, vec<N, T> const &b) {
    return length(b - a);
}

template <size_t N, class T>
inline constexpr auto normalize(vec<N, T> const &a) {
    return a * (1 / length(a));
}

template <class T, class S>
inline constexpr auto cross(vec<2, T> const &a, vec<2, S> const &b) {
    return a[0] * b[1] - b[0] * a[1];
}

template <class T, class S>
inline constexpr auto cross(vec<3, T> const &a, vec<3, S> const &b) {
...
inline constexpr auto mix(T const &a, S const &b, F const &f) {
    return a * (1 - f) + b * f;
}

template <class T, class S, class F>
inline constexpr auto clamp(T const &x, S const &a, F const &b) {
    return min(max(x, a), b);
}

template <size_t N, class T, std::enable_if_t<!is_vec_v<T>, int> = 0>
inline constexpr auto tovec(T const &x) {
    return vec<N, T>(x);
}

template <size_t N, class T>
inline constexpr auto tovec(vec<N, T> const &x) { return x; }
