...
TEST_CASE("numeric operators", "[numeric]") {
    ...
    zeno::getSession().loadSceneFromList(json);
    auto input = std::make_shared<zeno::NumericObject>(40);
    zeno::getSession().getGraph().setGraphInput("input", input);
    zeno::getSession().getGraph().applyGraph();
    auto output_ptr = zeno::getSession().getGraph().getGraphOutput("output");
