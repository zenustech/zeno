...
namespace hg::OpenGL {
  ...
  struct Buffer {
    ...
    Buffer(GLuint target = GL_ARRAY_BUFFER) : target(target) { CHECK_GL(glGenBuffers(1, &buf)); }

    ~Buffer() { CHECK_GL(glDeleteBuffers(1, &buf)); }
    ...
    void bind_data(const void *data, size_t size, GLuint usage = GL_STATIC_DRAW) const {
    ...
    void attribute(GLuint index, size_t offset, size_t stride, GLuint type, GLuint count) const {
    ...
    void unbind() const { CHECK_GL(glBindBuffer(target, 0)); }
    ...
    void unmap() const { CHECK_GL(glUnmapBuffer(target)); }
  };
