...
struct AssetsMgr : std::enable_shared_from_this<AssetsMgr> {
    ...
    ZENO_API std::shared_ptr<INodeAny> newInstance(std::shared_ptr<Graph> pGraph, const std::string& assetsName, const std::string& nodeName, bool createInAsset);
    ZENO_API void updateAssetInstance(const std::string& assetsName, std::shared_ptr<INodeAny>& spNode);
...
private:
    void initAssetsInfo();
    std::shared_ptr<Graph> forkAssetGraph(std::shared_ptr<Graph> assetGraph, SubnetNode* subNode);
