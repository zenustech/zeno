...
namespace zs {
  ...
  namespace math {
    ...
    template <typename T> __forceinline__ __device__ void svd(
...
#  endif
    ...
    /**
       ...
       \param[out] Sigma Vector of singular values sorted with decreasing magnitude. The second one
       can be negative. \param[out] V Robustly a rotation matrix in Givens form
    */
    template <typename T> __forceinline__ __host__ __device__ void singularValueDecomposition(
        const T AA[4], GivensRotation<double>& U, T Sigma[2], GivensRotation<double>& V) {
    ...
    __forceinline__ __device__ void svd(const T F[Dim * Dim], T U[Dim * Dim], T S[Dim],
                                        T V[Dim * Dim]) {
    ...
    /**
       ...
       \param[out] Sigma Vector of singular values sorted with decreasing magnitude. The second one
       can be negative. \param[out] V Robustly a rotation matrix.
    */
    template <> __forceinline__ __device__ void svd<float, 2>(const float A[4], float U[4],
                                                              float Sigma[2], float V[4]) {
    ...
    template <> __forceinline__ __device__ void svd<double, 2>(const double A[4], double U[4],
                                                               double Sigma[2], double V[4]) {
      GivensRotation<double> gv(0, 1);
      GivensRotation<double> gu(0, 1);
      singularValueDecomposition(A, gu, Sigma, gv);
    ...
    template <> __forceinline__ __device__ void svd<float, 3>(const float F[9], float U[9],
                                                              float S[3], float V[9]) {
      svd(F[0], F[3], F[6], F[1], F[4], F[7], F[2], F[5], F[8], U[0], U[3], U[6], U[1], U[4], U[7],
          U[2], U[5], U[8], S[0], S[1], S[2], V[0], V[3], V[6], V[1], V[4], V[7], V[2], V[5], V[8]);
    }

    template <> __forceinline__ __device__ void svd<double, 3>(const double F[9], double U[9],
