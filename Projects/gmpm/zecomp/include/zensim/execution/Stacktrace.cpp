...
namespace Kokkos {
  namespace Impl {
    ...
    template <class Callback> void for_each_token(const std::string &s, Callback c) {
    ...
    void demangle_and_print_traceback_entry(std::ostream &out, const std::string &traceback_entry,
    ...
    void demangle_and_print_traceback(std::ostream &out,
      ...
      for (auto &&entry : traceback) {
        demangle_and_print_traceback_entry(out, entry, result.found_main, result.main_col,
                                           result.main_col_lens);
        out << std::endl;
      }
    }
