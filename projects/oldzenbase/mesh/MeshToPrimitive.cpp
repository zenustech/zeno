...
struct ConvertTo_MeshObject_PrimitiveObject : MeshToPrimitive {
    virtual void apply() override {
        ...
        get_input<PrimitiveObject>("prim")->movefrom(std::move(smart_any_cast<std::shared_ptr<IObject>>(outputs.at("prim"))).get());
    }
};
...
struct ConvertTo_PrimitiveObject_MeshObject : PrimitiveToMesh {
    virtual void apply() override {
        ...
        get_input<MeshObject>("mesh")->movefrom(std::move(smart_any_cast<std::shared_ptr<IObject>>(outputs.at("mesh"))).get());
    }
};
