	...
	int flags, UrdfVisualShapeCache* cachedLinkGraphicsShapesIn, UrdfVisualShapeCache* cachedLinkGraphicsShapesOut, bool recursive)
{
    ...
    btCompoundShape* tmpShape = u2b.convertLinkCollisionShapes(urdfLinkIndex, localInertialFrame);
	...
	int graphicsIndex;
	{
		...
		{
			graphicsIndex = u2b.convertLinkVisualShapes(urdfLinkIndex, localInertialFrame); // it makes the object into GL in OpenGLWindow/GlInstancingRenderer.cpp
	...
	if (compoundShape)
	{
		...
		if (createMultiBody)
		{
			//if (compoundShape->getNumChildShapes()>0)
			{
                ...
                {
                    B3_PROFILE("createCollisionObjectGraphicsInstance2"); // TODO: add color and other stuff
                    //u2b.createCollisionObjectGraphicsInstance(urdfLinkIndex, col, color2);
		...
		{
			int mbLinkIndex = cache.getMbIndexFromUrdfIndex(urdfLinkIndex);
			//u2b.convertLinkVisualShapes2(mbLinkIndex, urdfLinkIndex, localInertialFrame, col, u2b.getBodyUniqueId());
			//u2b.convertLinkVisualShapes2(-1, urdfLinkIndex, localInertialFrame, linkRigidBody, u2b.getBodyUniqueId());
		}
	}
	...
	if (recursive)
	{
		for (int i = 0; i < numChildren; i++)
		{
			...
			ConvertURDF2BulletInternal(u2b, creation, cache, urdfChildLinkIndex, linkTransformInWorldSpace, world1, createMultiBody, flags, cachedLinkGraphicsShapesIn, cachedLinkGraphicsShapesOut, recursive);
		}
	}
...
btMultiBody* ConvertURDF2Bullet(
	...
	btMultiBodyDynamicsWorld* world1,
#endif
	bool createMultiBody, int flags, UrdfVisualShapeCache* cachedLinkGraphicsShapes)
{
    // use MyMultiBodyCreator to create a multibody
    MyMultiBodyCreator* creation = new MyMultiBodyCreator();
	...
	if (recursive)
	{
		ConvertURDF2BulletInternal(u2b, *creation, cache, urdfLinkIndex, rootTransformInWorldSpace, world1, createMultiBody,  flags, cachedLinkGraphicsShapes, &cachedLinkGraphicsShapesOut, recursive);
	}
	else
	{
		...
		for (int i = 0; i < allIndices.size(); i++)
		{
			...
			btTransform tr = ConvertURDF2BulletInternal(u2b, *creation, cache, urdfLinkIndex, parentTr , world1, createMultiBody, flags, cachedLinkGraphicsShapes, &cachedLinkGraphicsShapesOut, recursive);
