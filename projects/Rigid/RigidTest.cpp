...
struct BulletMakeTransform : zeno::INode {
    virtual void apply() override {
        ...
        if (has_input("translate")) {
            auto origin =
                get_input<zeno::NumericObject>("translate")->get<zeno::vec3f>();
        ...
        if (has_input("rotation")) {
            if (get_input<zeno::NumericObject>("rotation")->is<zeno::vec3f>()) {
                auto rotation = get_input<zeno::NumericObject>("rotation")
                                    ->get<zeno::vec3f>();
                trans->trans.setRotation(
                    zeno::vec_to_other<btQuaternion>(rotation)); // ypr
            } else {
                auto rotation = get_input<zeno::NumericObject>("rotation")
                                    ->get<zeno::vec4f>();
                trans->trans.setRotation(
                    zeno::vec_to_other<btQuaternion>(rotation));
            }
        }
...
struct BulletTransformSetBasisEuler : zeno::INode {
    virtual void apply() override {
        ...
        auto euler =
            get_input<zeno::NumericObject>("eulerZYX")->get<zeno::vec3f>();
        trans.getBasis().setEulerZYX(euler[0], euler[1], euler[2]);
    }
};

ZENDEFNODE(BulletTransformSetBasisEuler,
           {{"trans", "eulerZYX"}, {}, {}, {"Bullet"}});

struct BulletMakeFrameFromPivotAxis : zeno::INode {
    virtual void apply() override {
        auto pivot = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("pivot")->get<zeno::vec3f>());
        auto axis = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("axis")->get<zeno::vec3f>());

        auto trans = std::make_shared<BulletTransform>();

        trans->trans.setOrigin(pivot);
        trans->trans.getBasis().setValue(axis.getX(), axis.getX(), axis.getX(),
                                         axis.getY(), axis.getY(), axis.getY(),
                                         axis.getZ(), axis.getZ(), axis.getZ());
...
ZENDEFNODE(BulletMakeFrameFromPivotAxis,
           {{"pivot", "axis"}, {"frame"}, {}, {"Bullet"}});

struct BulletQuatRotate : zeno::INode {
    virtual void apply() override {
        auto quat = zeno::vec_to_other<btQuaternion>(
            get_input<zeno::NumericObject>("quat")->get<zeno::vec4f>());
        auto v = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("vec3")->get<zeno::vec3f>());
...
ZENDEFNODE(BulletQuatRotate, {{"quat", "vec3"}, {"vec3"}, {}, {"Bullet"}});
...
struct PrimitiveToBulletMesh : zeno::INode {
    virtual void apply() override {
        ...
        for (int i = 0; i < prim->tris.size(); i++) {
            auto f = prim->tris[i];
            mesh->mesh.addTriangle(zeno::vec_to_other<btVector3>(pos[f[0]]),
                                   zeno::vec_to_other<btVector3>(pos[f[1]]),
                                   zeno::vec_to_other<btVector3>(pos[f[2]]),
                                   true);
        }
    ...
    */
    virtual void apply() override {
        ...
        params.m_paramsVHACD.m_resolution =
            100000; // Maximum number of voxels generated during the voxelization stage (default=100,000, range=10,000-16,000,000)
        params.m_paramsVHACD.m_depth =
            20; // Maximum number of clipping stages. During each split stage, parts with a concavity higher than the user defined threshold are clipped according the "best" clipping plane (default=20, range=1-32)
        params.m_paramsVHACD.m_concavity =
            0.001; // Maximum allowed concavity (default=0.0025, range=0.0-1.0)
        params.m_paramsVHACD.m_planeDownsampling =
            4; // Controls the granularity of the search for the "best" clipping plane (default=4, range=1-16)
        params.m_paramsVHACD.m_convexhullDownsampling =
            4; // Controls the precision of the convex-hull generation process during the clipping plane selection stage (default=4, range=1-16)
        params.m_paramsVHACD.m_alpha =
            0.05; // Controls the bias toward clipping along symmetry planes (default=0.05, range=0.0-1.0)
        params.m_paramsVHACD.m_beta =
            0.05; // Controls the bias toward clipping along revolution axes (default=0.05, range=0.0-1.0)
        params.m_paramsVHACD.m_gamma =
            0.0005; // Controls the maximum allowed concavity during the merge stage (default=0.00125, range=0.0-1.0)
        params.m_paramsVHACD.m_pca =
            0; // Enable/disable normalizing the mesh before applying the convex decomposition (default=0, range={0,1})
        params.m_paramsVHACD.m_mode =
            0; // 0: voxel-based approximate convex decomposition, 1: tetrahedron-based approximate convex decomposition (default=0, range={0,1})
        params.m_paramsVHACD.m_maxNumVerticesPerCH =
            64; // Controls the maximum number of triangles per convex-hull (default=64, range=4-1024)
        params.m_paramsVHACD.m_minVolumePerCH =
            0.0001; // Controls the adaptive sampling of the generated convex-hulls (default=0.0001, range=0.0-0.01)
        params.m_paramsVHACD.m_convexhullApproximation =
            true; // Enable/disable approximation when computing convex-hulls (default=1, range={0,1})
        params.m_paramsVHACD.m_oclAcceleration =
            true; // Enable/disable OpenCL acceleration (default=0, range={0,1})
        ...
        for (size_t c = 0; c < nConvexHulls; c++) {
            ...
            if (nPoints > 0) {
                for (size_t i = 0; i < nPoints; i++) {
                    size_t ind = i * 3;
                    outpos[i] =
                        zeno::vec3f(ch.m_points[ind], ch.m_points[ind + 1],
                                    ch.m_points[ind + 2]);
                }
            } else {
                good_ch_flag = false;
            ...
            if (nTriangles > 0) {
                for (size_t i = 0; i < nTriangles; i++) {
                    size_t ind = i * 3;
                    outprim->tris[i] = zeno::vec3i(ch.m_triangles[ind],
                                                   ch.m_triangles[ind + 1],
                                                   ch.m_triangles[ind + 2]);
                }
            } else {
                good_ch_flag = false;
            }
...
struct BulletMakeBoxShape : zeno::INode {
    virtual void apply() override {
        auto size =
            get_input<zeno::NumericObject>("semiSize")->get<zeno::vec3f>();
...
struct BulletMakeStaticPlaneShape : zeno::INode {
    virtual void apply() override {
        auto planeNormal = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("planeNormal")->get<zeno::vec3f>());
        auto planeConstant = btScalar(get_input2<float>("planeConstant"));

        auto shape = std::make_shared<BulletCollisionShape>(
            std::make_unique<btStaticPlaneShape>(planeNormal, planeConstant));
        set_output("shape", std::move(shape));
    }
};
...
struct BulletMakeCapsuleShape : zeno::INode {
    virtual void apply() override {
        ...
        auto shape = std::make_shared<BulletCollisionShape>(
            std::make_unique<btCapsuleShape>(btScalar(radius),
                                             btScalar(height)));
        set_output("shape", std::move(shape));
    }
};
...
struct BulletMakeCylinderShape : zeno::INode {
    virtual void apply() override {
        auto halfExtents = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("halfExtents")->get<zeno::vec3f>());

        auto shape = std::make_shared<BulletCollisionShape>(
            std::make_unique<btCylinderShape>(halfExtents));
        set_output("shape", std::move(shape));
    }
};
...
struct BulletColShapeCalcLocalInertia : zeno::INode {
    virtual void apply() override {
        auto isCompound =
            (std::get<std::string>(get_param("isCompound")) == "true");
        ...
        localInertia->set<zeno::vec3f>(
            zeno::vec3f(lInertia[0], lInertia[1], lInertia[2]));
...
struct BulletMakeConvexHullShape : zeno::INode {
    virtual void apply() override {
        ...
        auto margin = get_input2<float>("margin");
        convexHC->compute(vertices.data(), sizeof(float) * 3,
                          vertices.size() / 3, 0.0f, 0.0f);
        auto convex = std::make_unique<btConvexHullShape>(
...
struct BulletMakeObject : zeno::INode {
    virtual void apply() override {
        ...
        auto object = std::make_shared<BulletObject>(mass, trans->trans, shape);
...
struct BulletObjectSetFriction : zeno::INode {
    virtual void apply() override {
        ...
        log_debug("set object {} with friction={}", (void *)object.get(),
                  friction);
...
struct BulletObjectSetRestitution : zeno::INode {
    virtual void apply() override {
        ...
        log_debug("set object {} with restituion={}", (void *)object.get(),
                  restitution);
...
struct BulletObjectGetTransform : zeno::INode {
    virtual void apply() override {
        ...
        if (body && body->getMotionState()) {
            ...
            trans->trans =
                static_cast<btCollisionObject *>(body)->getWorldTransform();
        }
        set_output("trans", std::move(trans));
    }
};
...
ZENDEFNODE(BulletInverseTransform, {{"trans"}, {"trans_inv"}, {}, {"Bullet"}});
...
ZENDEFNODE(BulletMakeConstraint,
           {
               {"obj1", "obj2"},
               {"constraint"},
               {{"enum ConeTwist Fixed Gear Generic6Dof Generic6DofSpring "
...
struct BulletConstraintSetFrames : zeno::INode {
    virtual void apply() override {
        ...
        if (constraintType == "ConeTwist") {
            dynamic_cast<btConeTwistConstraint *>(cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Fixed") {
            dynamic_cast<btFixedConstraint *>(cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Generic6Dof") {
            dynamic_cast<btGeneric6DofConstraint *>(cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Generic6DofSpring") {
            dynamic_cast<btGeneric6DofSpringConstraint *>(
                cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Generic6DofSpring2") {
            dynamic_cast<btGeneric6DofSpring2Constraint *>(
                cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Hinge") {
            dynamic_cast<btHingeConstraint *>(cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Hinge2") {
            dynamic_cast<btHinge2Constraint *>(cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Slider") {
            dynamic_cast<btSliderConstraint *>(cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        } else if (constraintType == "Universal") {
            dynamic_cast<btUniversalConstraint *>(cons->constraint.get())
                ->setFrames(frame1->trans, frame2->trans);
        }
        set_output("constraint", std::move(cons));
    }
...
ZENDEFNODE(BulletConstraintSetFrames,
           {
               ...
               {"constraint"},
               {{"enum ConeTwist Fixed Generic6Dof Generic6DofSpring "
                 "Generic6DofSpring2 Hinge Hinge2 Slider Universal",
...
struct BulletConstraintGetFrames : zeno::INode {
    virtual void apply() override {
        ...
        if (constraintType == "ConeTwist") {
            ...
            frame1->trans = dynamic_cast<btGeneric6DofSpringConstraint *>(
                                cons->constraint.get())
                                ->getFrameOffsetA();
            frame2->trans = dynamic_cast<btGeneric6DofSpringConstraint *>(
                                cons->constraint.get())
                                ->getFrameOffsetB();
        } else if (constraintType == "Generic6DofSpring2") {
            frame1->trans = dynamic_cast<btGeneric6DofSpring2Constraint *>(
                                cons->constraint.get())
                                ->getFrameOffsetA();
            frame2->trans = dynamic_cast<btGeneric6DofSpring2Constraint *>(
                                cons->constraint.get())
                                ->getFrameOffsetB();
        } else if (constraintType == "Hinge") {
            ...
            frame2->trans =
                dynamic_cast<btHingeConstraint *>(cons->constraint.get())
                    ->getFrameOffsetB();
            std::cout << "frame1:" << frame1->trans.getOrigin()[0] << " "
                      << frame1->trans.getOrigin()[1] << " "
                      << frame1->trans.getOrigin()[2] << std::endl;
            std::cout << "frame2:" << frame2->trans.getOrigin()[0] << " "
                      << frame2->trans.getOrigin()[1] << " "
                      << frame2->trans.getOrigin()[2] << std::endl;
        } else if (constraintType == "Hinge2") {
            frame1->trans =
                dynamic_cast<btHinge2Constraint *>(cons->constraint.get())
                    ->getFrameOffsetA();
...
struct BulletConstraintSetLimitByAxis : zeno::INode {
    virtual void apply() override {
        ...
        if (axisId == "linearX") {
            axis = 0;
        } else if (axisId == "linearY") {
            axis = 1;
        } else if (axisId == "linearZ") {
            axis = 2;
        } else if (axisId == "angularX") {
            axis = 3;
        } else if (axisId == "angularY") {
            axis = 4;
        } else {
            axis = 5;
        } // "angularZ"
        ...
        if (has_input("axisId")) {
            if (constraintType == "ConeTwist") {
                if ((high > low) && (high - low) < 1e-5) {
                    dynamic_cast<btConeTwistConstraint *>(
                        cons->constraint.get())
                        ->setLimit(axis, low); // axis >= 3
                }
            } else if (constraintType == "Fixed") {
                dynamic_cast<btFixedConstraint *>(cons->constraint.get())
                ...
                if (axis < 3) {
                    dynamic_cast<btSliderConstraint *>(cons->constraint.get())
                        ->setLowerLinLimit(low);
                    dynamic_cast<btSliderConstraint *>(cons->constraint.get())
                        ->setUpperLinLimit(high);
                } else {
                    dynamic_cast<btSliderConstraint *>(cons->constraint.get())
                        ->setLowerAngLimit(low);
                    dynamic_cast<btSliderConstraint *>(cons->constraint.get())
                        ->setUpperAngLimit(high);
                }
...
struct BulletConstraintSetRefFrameA : zeno::INode {
    virtual void apply() override {
        ...
        auto constraintType =
            std::get<std::string>(get_param("constraintType"));
        auto useLinearReferenceFrameA =
            std::get<std::string>(get_param("useReferenceFrameA"));
        ...
        if (constraintType == "Generic6Dof") {
            dynamic_cast<btGeneric6DofConstraint *>(cons->constraint.get())
                ->setUseLinearReferenceFrameA(flag);
        } else if (constraintType == "Generic6DofSpring") {
            dynamic_cast<btGeneric6DofSpringConstraint *>(
                cons->constraint.get())
                ->setUseLinearReferenceFrameA(flag);
            ...
            dynamic_cast<btHingeConstraint *>(cons->constraint.get())
                ->setUseReferenceFrameA(flag); // NAME DIFFERENCE
        } else if (constraintType == "Universal") {
            dynamic_cast<btUniversalConstraint *>(cons->constraint.get())
                ->setUseLinearReferenceFrameA(flag);
        }
...
ZENDEFNODE(BulletConstraintSetRefFrameA,
           {
               ...
               {{"enum Generic6Dof Generic6DofSpring Hinge Universal",
                 "constraintType", "Universal"},
                {"enum true false", "useReferenceFrameA", "true"}},
               {"Bullet"},
           });
...
struct BulletConstraintSetAxis : zeno::INode {
    virtual void apply() override {
        auto cons = get_input<BulletConstraint>("constraint");
        auto axis1 = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("axis1")->get<zeno::vec3f>());
        auto axis2 = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("axis2")->get<zeno::vec3f>());
        auto constraintType =
            std::get<std::string>(get_param("constraintType"));
        ...
        if (constraintType == "Fixed") {
            ...
            dynamic_cast<btGearConstraint *>(cons->constraint.get())
                ->setAxisA(axis1);
            dynamic_cast<btGearConstraint *>(cons->constraint.get())
                ->setAxisB(axis2);
        } else if (constraintType == "Generic6Dof") {
            ...
            dynamic_cast<btPoint2PointConstraint *>(cons->constraint.get())
                ->setPivotA(axis1);
            dynamic_cast<btPoint2PointConstraint *>(cons->constraint.get())
                ->setPivotB(axis2);
        } else if (constraintType == "Universal") {
...
ZENDEFNODE(BulletConstraintSetAxis,
           {
               ...
               {"constraint"},
               {{"enum Fixed Gear Generic6Dof Generic6DofSpring "
                 "Generic6DofSpring2 Hinge Hinge2 Point2Point Universal",
                 "constraintType", "Universal"}},
               {"Bullet"},
           });
...
struct BulletConstraintSetSpring : zeno::INode {
    virtual void apply() override {
        ...
        if (constraintType == "Fixed") {
            dynamic_cast<btFixedConstraint *>(cons->constraint.get())
                ->enableSpring(axis, enabled);
            dynamic_cast<btFixedConstraint *>(cons->constraint.get())
                ->setStiffness(axis, stiffness);
            dynamic_cast<btFixedConstraint *>(cons->constraint.get())
                ->setDamping(axis, damping);
            dynamic_cast<btFixedConstraint *>(cons->constraint.get())
                ->setEquilibriumPoint(axis, epVal);
        } else if (constraintType == "Generic6DofSpring") {
            dynamic_cast<btGeneric6DofSpringConstraint *>(
                cons->constraint.get())
                ->enableSpring(axis, enabled);
...
struct BulletConstraintSetMotor : zeno::INode {
    virtual void apply() override {
        ...
        auto maxMotorImpulseNormalized =
            btScalar(get_input2<float>("maxMotorImpulseNormalized"));
        auto motorTarget = zeno::vec_to_other<btQuaternion>(
            get_input<zeno::NumericObject>("motorTarget")->get<zeno::vec4f>());
        auto motorTargetConstraint = zeno::vec_to_other<btQuaternion>(
            get_input<zeno::NumericObject>("motorTargetConstraint")
                ->get<zeno::vec4f>());
        ...
        if (constraintType == "ConeTwist") {
            ...
            dynamic_cast<btConeTwistConstraint *>(cons->constraint.get())
                ->setMaxMotorImpulseNormalized(maxMotorImpulseNormalized);
            dynamic_cast<btConeTwistConstraint *>(cons->constraint.get())
                ->setMotorTarget(motorTarget);
            dynamic_cast<btConeTwistConstraint *>(cons->constraint.get())
                ->setMotorTargetInConstraintSpace(motorTargetConstraint);
            dynamic_cast<btConeTwistConstraint *>(cons->constraint.get())
                ->setAngularOnly(angularOnly);

            dynamic_cast<btConeTwistConstraint *>(cons->constraint.get())
                ->setFixThresh(fixThresh);
        } else if (constraintType == "Generic6Dof") {
            dynamic_cast<btGeneric6DofConstraint *>(cons->constraint.get())
                ->getTranslationalLimitMotor()
                ->m_enableMotor[axis] = enableMotor;
            dynamic_cast<btGeneric6DofConstraint *>(cons->constraint.get())
                ->getTranslationalLimitMotor()
                ->m_targetVelocity[axis] = targetVelocity;
            dynamic_cast<btGeneric6DofConstraint *>(cons->constraint.get())
                ->getTranslationalLimitMotor()
                ->m_maxMotorForce[axis] = maxMotorForce;
        } else if (constraintType == "Generic6DofSpring2") {
            dynamic_cast<btGeneric6DofSpring2Constraint *>(
                cons->constraint.get())
            ...
            dynamic_cast<btGeneric6DofSpring2Constraint *>(
                cons->constraint.get())
                ->enableMotor(axis, enableMotor);
            dynamic_cast<btGeneric6DofSpring2Constraint *>(
                cons->constraint.get())
                ->setServo(axis, enableServo);
            dynamic_cast<btGeneric6DofSpring2Constraint *>(
                cons->constraint.get())
                ->setMaxMotorForce(axis, maxMotorForce);
            dynamic_cast<btGeneric6DofSpring2Constraint *>(
                cons->constraint.get())
                ->setServoTarget(axis, servoTarget);
            dynamic_cast<btGeneric6DofSpring2Constraint *>(
                cons->constraint.get())
                ->setTargetVelocity(axis, targetVelocity);
        } else if (constraintType == "Hinge") {
            dynamic_cast<btHingeConstraint *>(cons->constraint.get())
                ->setAngularOnly(angularOnly);
            dynamic_cast<btHingeConstraint *>(cons->constraint.get())
                ->setMaxMotorImpulse(maxMotorImpulse);
            dynamic_cast<btHingeConstraint *>(cons->constraint.get())
                ->setMotorTarget(motorTarget, dt);
            dynamic_cast<btHingeConstraint *>(cons->constraint.get())
                ->setMotorTargetVelocity(targetVelocity);
        } else if (constraintType == "Hinge2") {
            dynamic_cast<btHinge2Constraint *>(cons->constraint.get())
                ->setBounce(axis, bounce);
            dynamic_cast<btHinge2Constraint *>(cons->constraint.get())
...
ZENDEFNODE(BulletConstraintSetMotor,
           {
               {"constraint",
                ...
                {"float", "maxMotorImpulse", "0"},
                {"float", "maxMotorImpulseNormalized", "0"},
                {"vec4f", "motorTarget", "0,0,0,1"},
                {"vec4f", "motorTargetConstraint", "0,0,0,1"},
                {"float", "angularOnly", "1"},
                {"float", "fixThresh", "0"},
                {"float", "dt", "0"}},
               {"constraint"},
...
ZENDEFNODE(BulletConstraintSetRotOrder,
           {
               {"constraint"},
               {"constraint"},
               {{"enum Generic6DofSpring2 Hinge2", "constraintType", "Hinge2"},
                {"enum XYZ XZY YXZ YZX ZXY ZYX", "rotateOrder", "XYZ"}},
               {"Bullet"},
           });
...
struct BulletSliderConstraintSetSpring : zeno::INode {
    virtual void apply() override {
        ...
        auto restitutionDirAng =
            btScalar(get_input2<float>("restitutionDirAng"));
        auto restitutionDirLin =
            btScalar(get_input2<float>("restitutionDirLin"));
        auto restitutionLimAng =
            btScalar(get_input2<float>("restitutionLimAng"));
        auto restitutionLimLin =
            btScalar(get_input2<float>("restitutionLimLin"));
        auto restitutionOrthoAng =
            btScalar(get_input2<float>("restitutionOrthoAng"));
        auto restitutionOrthoLin =
            btScalar(get_input2<float>("restitutionOrthoLin"));
        ...
        auto targetAngMotorVelocity =
            btScalar(get_input2<float>("targetAngMotorVelocity"));
        auto targetLinMotorVelocity =
            btScalar(get_input2<float>("targetLinMotorVelocity"));
...
struct BulletWorldSetGravity : zeno::INode {
    virtual void apply() override {
        ...
        world->dynamicsWorld->setGravity(
            zeno::vec_to_other<btVector3>(gravity));
        set_output("world", std::move(world));
    }
};
...
struct BulletWorldSetObjList : zeno::INode {
    virtual void apply() override {
        auto world = get_input<BulletWorld>("world");
        auto objList = get_input<ListObject>("objList")
                           ->get<std::shared_ptr<BulletObject>>();
...
struct BulletObjectApplyForce : zeno::INode {
    virtual void apply() override {
        auto object = get_input<BulletObject>("object");
        auto forceImpulse =
            get_input<zeno::NumericObject>("ForceImpulse")->get<zeno::vec3f>();
        auto torqueImpulse =
            get_input<zeno::NumericObject>("TorqueImpulse")->get<zeno::vec3f>();
        object->body->applyCentralImpulse(
            zeno::vec_to_other<btVector3>(forceImpulse));
        object->body->applyTorqueImpulse(
            zeno::vec_to_other<btVector3>(torqueImpulse));
    }
};

ZENDEFNODE(BulletObjectApplyForce, {
                                       {"object",
                                        {"vec3f", "ForceImpulse", "0,0,0"},
                                        {"vec3f", "TorqueImpulse", "0,0,0"}},
                                       {},
                                       {},
                                       {"Bullet"},
                                   });
...
struct BulletMultiBodyObjectMakeStart : zeno::INode {
    virtual void apply() override {
        ...
        if (has_input("inertia"))
            inertia = zeno::vec_to_other<btVector3>(
                get_input<zeno::NumericObject>("inertia")->get<zeno::vec3f>());
        auto fixedBase =
            (std::get<std::string>(get_param("fixedBase")) == "true");
        auto canSleep =
            (std::get<std::string>(get_param("canSleep")) == "true");
        auto object = std::make_shared<BulletMultiBodyObject>(
            n_links, mass, inertia, fixedBase, canSleep);
...
struct BulletMultiBodySetCollider : zeno::INode {
    virtual void apply() override {
        ...
        if (link_id < 0) {
            ...
            object->multibody->getLink(link_id).m_collider =
                collider->linkCollider.get();
        }
        set_output("object", std::move(object));
    }
};
...
struct BulletMultiBodySetupJoint : zeno::INode {
    virtual void apply() override {
        ...
        auto mass = get_input2<float>("mass");
        auto inertia = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("inertia")->get<zeno::vec3f>());
        auto jointAxis = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("jointAxis")->get<zeno::vec3f>());
        auto rotParentToThis = zeno::vec_to_other<btQuaternion>(
            get_input<zeno::NumericObject>("rotParentToThis")
                ->get<zeno::vec4f>());
        auto parentComToThisPivotOffset = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("parentComToThisPivotOffset")
                ->get<zeno::vec3f>());
        auto thisPivotToThisComOffset = zeno::vec_to_other<btVector3>(
            get_input<zeno::NumericObject>("thisPivotToThisComOffset")
                ->get<zeno::vec3f>());
        auto disableParentCollision =
            (std::get<std::string>(get_param("disableParentCollision")) ==
             "true");

        if (jointType == "Fixed") {
            object->multibody->setupFixed(
                i, mass, inertia, parent, rotParentToThis,
                parentComToThisPivotOffset, thisPivotToThisComOffset,
                disableParentCollision);
        } else if (jointType == "Prismatic") {
            object->multibody->setupPrismatic(
                i, mass, inertia, parent, rotParentToThis, jointAxis,
                parentComToThisPivotOffset, thisPivotToThisComOffset,
                disableParentCollision);
        } else if (jointType == "Revolute") {
            object->multibody->setupRevolute(
                i, mass, inertia, parent, rotParentToThis, jointAxis,
                parentComToThisPivotOffset, thisPivotToThisComOffset,
                disableParentCollision);
        } else if (jointType == "Spherical") {
            object->multibody->setupSpherical(
                i, mass, inertia, parent, rotParentToThis,
                parentComToThisPivotOffset, thisPivotToThisComOffset,
                disableParentCollision);
        } else { // planar
            object->multibody->setupPlanar(
                i, mass, inertia, parent, rotParentToThis, jointAxis,
                parentComToThisPivotOffset, disableParentCollision);
        }
...
ZENDEFNODE(BulletMultiBodySetupJoint,
           {{"object", "linkIndex", "parentIndex", "mass", "inertia",
             "jointAxis", "rotParentToThis", "parentComToThisPivotOffset",
             "thisPivotToThisComOffset"},
            {"object"},
            {{"enum Fixed Prismatic Revolute Spherical Planar", "jointType",
              "Revolute"},
             {"enum true false", "disableParentCollision", "false"}},
            {"Bullet"}});
...
struct BulletMultiBodySetJointProperty : zeno::INode {
    virtual void apply() override {
        ...
        if (has_input("maxVelocity")) {
            ...
            object->multibody->getLink(link_id).m_jointMaxVelocity =
                maxVelocity;
        }
        set_output("object", std::move(object));
    }
...
ZENDEFNODE(BulletMultiBodySetJointProperty, {{"object",
                                              {"int", "linkIndex", "0"},
                                              "damping",
                                              "friction",
                                              "lowerLimit",
...
ZENDEFNODE(BulletMultiBodySetBaseTransform,
           {{"object", "baseTrans"}, {"object"}, {}, {"Bullet"}});
...
struct BulletMultiBodyLinkGetTransform : zeno::INode {
    virtual void apply() override {
        ...
        auto trans = std::make_shared<BulletTransform>();
        trans->trans =
            object->multibody->getLink(link_id).m_collider->getWorldTransform();
        set_output("trans", std::move(trans));
    }
};

ZENDEFNODE(BulletMultiBodyLinkGetTransform,
           {{"object", "linkIndex"}, {"trans"}, {}, {"Bullet"}});
...
struct BulletMultiBodyGetTransform : zeno::INode {
    virtual void apply() override {
        ...
        for (size_t i = 0; i < object->multibody->getNumLinks(); i++) {
            auto trans = std::make_shared<BulletTransform>();
            trans->trans =
                object->multibody->getLink(i).m_collider->getWorldTransform();
            std::cout << "\nlink #" << i << ": " << trans->trans.getOrigin()[0]
                      << "," << trans->trans.getOrigin()[1] << ","
                      << trans->trans.getOrigin()[2];
...
struct BulletMultiBodyWorldGetTransform : zeno::INode {
    ...
    virtual void apply() override {
        ...
        int numCollisionObjects =
            world->dynamicsWorld->getNumCollisionObjects();
        for (size_t i = 0; i < numCollisionObjects; i++) {
            auto linkTrans = std::make_shared<BulletTransform>();
            btCollisionObject *colObj =
                world->dynamicsWorld->getCollisionObjectArray()[i];
            btCollisionShape *collisionShape = colObj->getCollisionShape();

            linkTrans->trans = colObj->getWorldTransform();
            int graphicsIndex = colObj->getUserIndex();
            std::cout << "graphicsId #" << graphicsIndex << ":"
                      << linkTrans->trans.getOrigin()[0] << ","
                      << linkTrans->trans.getOrigin()[1] << ","
                      << linkTrans->trans.getOrigin()[2] << "\n";
            if (graphicsIndex >= 0) {
...
struct BulletMultiBodySetProperty : zeno::INode {
    virtual void apply() override {
        ...
        auto canSleep =
            (std::get<std::string>(get_param("canSleep")) == "true");
        auto selfCollide =
            (std::get<std::string>(get_param("selfCollide")) == "true");
...
struct BulletMultiBodyUpdateColObjectTransform : zeno::INode {
    virtual void apply() override {
        ...
        object->multibody->updateCollisionObjectWorldTransforms(world_to_local,
                                                                local_origin);
        set_output("object", std::move(object));
    }
};
...
struct BulletMultiBodySetJointPosMultiDof : zeno::INode {
    virtual void apply() override {
        ...
        auto isSpherical =
            (std::get<std::string>(get_param("isSpherical")) == "true");
        if (isSpherical) {
            auto pos = zeno::vec_to_other<btQuaternion>(
                get_input<zeno::NumericObject>("pos")->get<zeno::vec4f>());
            pos.normalize();
...
struct BulletMultiBodyPDControl : zeno::INode {
    virtual void apply() override {
        ...
        if (has_input("qDesiredList")) {
            {
                auto numericObjs = get_input<zeno::ListObject>("qDesiredList")
        ...
        if (has_input("dqDesiredList")) {
            {
                auto numericObjs = get_input<zeno::ListObject>("dqDesiredList")
                                       ->get<std::shared_ptr<NumericObject>>();
                for (auto &&no : numericObjs)
        ...
        for (int joint = 0; joint < object->multibody->getNumLinks(); joint++) {
            int dof1 = 0;
            btScalar qActual =
                object->multibody->getJointPosMultiDof(joint)[dof1];
            btScalar qdActual =
                object->multibody->getJointVelMultiDof(joint)[dof1];
            ...
            std::cout << "current force for link #" << joint << " is " << force
                      << std::endl;
...
struct BulletStepMultiBodyWorld : zeno::INode {
    virtual void apply() override {
        ...
        auto fixedTimeStep =
            get_input<zeno::NumericObject>("fixedTimeStep")->get<float>();
...
struct BulletMultiBodyWorldAddCollisionObject : zeno::INode {
    virtual void apply() override {
        auto world = get_input<BulletMultiBodyWorld>("world");
        auto isDynamic =
            (std::get<std::string>(get_param("isDynamic")) == "true");
        auto col = get_input<BulletMultiBodyLinkCollider>("collider");
        ...
        world->dynamicsWorld->addCollisionObject(
            col->linkCollider.get(), collisionFilterGroup, collisionFilterMask);
...
struct BulletMultiBodyMakeConstraint : zeno::INode {
    virtual void apply() override {
        ...
        if (has_input("bodyB")) {
            ...
            auto cons = std::make_shared<BulletMultiBodyConstraint>(
                bodyA->multibody.get(), bodyB->multibody.get(), linkA, linkB,
                constraintType);
            ...
            if (has_input("jointMaxForce")) {
                auto jointMaxForce =
                    btScalar(get_input2<float>("jointMaxForce"));
                config["jointMaxForce"] = jointMaxForce;
            }
            if (has_input("desiredVelocity")) {
                auto desiredVelocity =
                    btScalar(get_input2<float>("desiredVelocity"));
                config["desiredVelocity"] = desiredVelocity;
            }
            auto cons = std::make_shared<BulletMultiBodyConstraint>(
                bodyA->multibody.get(), linkA, constraintType, config);
            set_output("constraint", cons);
        }
    }
};
...
ZENDEFNODE(BulletMultiBodyMakeConstraint,
           {
               {"bodyA", "linkA", "bodyB", "linkB", "lowerLimit", "upperLimit",
                "twistLimit", "jointMaxForce", "desiredVelocity"},
               {"constraint"},
               {{"enum Default DefaultMotor Spherical SphericalMotor Fixed "
                 "Gear Point2Point Slider",
...
struct BulletMultiBodyWorldAddConstraint : zeno::INode {
    virtual void apply() override {
        ...
        world->dynamicsWorld->addMultiBodyConstraint(
...
struct BulletMultiBodyWorldRemoveConstraint : zeno::INode {
    virtual void apply() override {
        auto world = get_input<BulletMultiBodyWorld>("world");
        auto constraint = get_input<BulletMultiBodyConstraint>("constraint");
        world->dynamicsWorld->removeMultiBodyConstraint(
            constraint->constraint.get());
        set_output("world", get_input("world"));
    }
};
...
struct BulletMultiBodyWorldAddConstraintEnd : zeno::INode {
    virtual void apply() override {
        auto world = get_input<BulletMultiBodyWorld>("world");
        for (int i = 0; i < world->dynamicsWorld->getNumMultiBodyConstraints();
             i++) {
...
struct BulletCalcInverseKinematics : zeno::INode {
    virtual void apply() override {
        ...
        {
            auto numericObjs =
                get_input<zeno::ListObject>("endEffectorLinkIndices")
                    ->get<std::shared_ptr<NumericObject>>();
            for (auto &&no : numericObjs)
        ...
        {
            auto numericObjs = get_input<zeno::ListObject>("targetPositions")
                                   ->get<std::shared_ptr<NumericObject>>();
            for (auto &&no : numericObjs)
        ...
        {
            auto numericObjs = get_input<zeno::ListObject>("targetOrientations")
                                   ->get<std::shared_ptr<NumericObject>>();
            for (auto &&no : numericObjs)
        ...
        {
            for (int i = 0; i < object->multibody->getNumLinks(); ++i) {
                if (object->multibody->getLink(i).m_jointType >= 0 &&
                    object->multibody->getLink(i).m_jointType <= 2) {
        ...
        endEffectorTargetWorldOrientations.reserve(numEndEffectorLinkIndices *
                                                   4);
        ...
        for (int ne = 0; ne < numEndEffectorLinkIndices; ne++) {
            ...
            validEndEffectorLinkIndices =
                validEndEffectorLinkIndices &&
                (endEffectorLinkIndex < object->multibody->getNumLinks());
            ...
            btQuaternion targetOrnWorld(
                targetOrientations[ne][0], targetOrientations[ne][1],
                targetOrientations[ne][2], targetOrientations[ne][3]);
            ...
            endEffectorTargetWorldPositions.push_back(
                targetPosBaseCoord.m_floats[0]);
            endEffectorTargetWorldPositions.push_back(
                targetPosBaseCoord.m_floats[1]);
            endEffectorTargetWorldPositions.push_back(
                targetPosBaseCoord.m_floats[2]);
        ...
        for (int i = 0; i < numIterations && currentDiff > residualThreshold;
            ...
            if (ikHelperPtr && validEndEffectorLinkIndices) {
                ...
                jacobian_angular.resize(numEndEffectorLinkIndices * 3 *
                                        numDofs);
                int jacSize = 0;
                ...
                if (tree && ((numDofs + baseDofs) == tree->numDoFs())) {
...

                    {
                        if (-1 != tree->setGravityInWorldFrame(id_grav) &&
                            -1 != tree->calculateInverseDynamics(
                                      q, qdot, nu, &joint_force)) {
                            tree->calculateJacobians(q);
                            btInverseDynamics::mat3x jac_t(3,
                                                           numDofs + baseDofs);
                            btInverseDynamics::mat3x jac_r(3,
                                                           numDofs + baseDofs);
                            currentDiff = 0;

                            endEffectorCurrentWorldPositions.resize(0);
                            endEffectorCurrentWorldPositions.reserve(
                                numEndEffectorLinkIndices * 3);

                            for (int ne = 0; ne < numEndEffectorLinkIndices;
                                 ne++) {
                                int endEffectorLinkIndex2 =
                                    endEffectorLinkIndices[ne];

                                // Note that inverse dynamics uses zero-based indexing of bodies, not starting from -1 for the base link.
                                tree->getBodyJacobianTrans(
                                    endEffectorLinkIndex2 + 1, &jac_t);
                                tree->getBodyJacobianRot(
                                    endEffectorLinkIndex2 + 1, &jac_r);

                                //calculatePositionKinematics is already done inside calculateInverseDynamics

                                tree->getBodyOrigin(endEffectorLinkIndex2 + 1,
                                                    &world_origin);
                                tree->getBodyTransform(
                                    endEffectorLinkIndex2 + 1, &world_rot);

                                for (int i = 0; i < 3; ++i) {
                                    for (int j = 0; j < numDofs; ++j) {
                                        jacobian_linear[(ne * 3 + i) * numDofs +
                                                        j] =
                                            jac_t(i, (baseDofs + j));
                                        jacobian_angular[(ne * 3 + i) *
                                                             numDofs +
                                                         j] =
                                            jac_r(i, (baseDofs + j));
                                    }
                                ...
                                endEffectorCurrentWorldPositions.push_back(
                                    world_origin[0]);
                                endEffectorCurrentWorldPositions.push_back(
                                    world_origin[1]);
                                endEffectorCurrentWorldPositions.push_back(
                                ...
                                btInverseDynamics::vec3 targetPos(btVector3(
                                    endEffectorTargetWorldPositions[ne * 3 + 0],
                                    endEffectorTargetWorldPositions[ne * 3 + 1],
                                    endEffectorTargetWorldPositions[ne * 3 +
                                                                    2]));
                                //diff
                                currentDiff =
                                    btMax(currentDiff,
                                          (world_origin - targetPos).length());
                            }
                        }
                    }
                    ...
                    if (ikMethod == "VEL_DLS_ORI_NULL" ||
                        ...
                        for (int i = 0; i < numDofs;
                             ++i) // TODO: use default data from multibody!
                        {
                            lower_limit[i] =
                                object->multibody->getLink(i).m_jointLowerLimit;
                            upper_limit[i] =
                                object->multibody->getLink(i).m_jointUpperLimit;
                            joint_range[i] = upper_limit[i] - lower_limit[i];
                            rest_pose[i] = startingPositions[i];
                        }
                        {
                            ikHelperPtr->computeNullspaceVel(
                                numDofs, &q_current[0], &lower_limit[0],
                                &upper_limit[0], &joint_range[0],
                                &rest_pose[0]);
                        }
                    }
                    ...
                    for (int i = 0; i < numDofs; ++i) {
                        joint_damping[i] =
                            object->multibody->getLink(i).m_jointDamping;
                    }
                    ...
                    if (numEndEffectorLinkIndices == 1) {
                        ikHelperPtr->computeIK(
                            ...
                            endEffectorWorldOrientation.m_floats, &q_current[0],
                            numDofs, endEffectorLinkIndices[0], &q_new[0],
                            IKMethod, &jacobian_linear[0], &jacobian_angular[0],
...
ZENDEFNODE(BulletCalcInverseKinematics,
           {
               {"object",
                ...
                {"float", "residualThreshold", "0.0001"}},
               {"poses"},
               {{"enum VEL_DLS_ORI_NULL VEL_SDLS_ORI VEL_DLS_ORI VEL_DLS_NULL "
                 "VEL_SDLS VEL_DLS JACOB_TRANS",
                 "IKMethod", "VEL_DLS_ORI_NULL"}},
               {"Bullet"},
           });
...
struct BulletMultiBodyMakeJointMotor : zeno::INode {
    virtual void apply() override {
        ...
        auto jointMotor = std::make_shared<BulletMultiBodyJointMotor>(
            object->multibody.get(), linkIndex, linkDof, desiredVelocity,
            maxMotorImpulse);
        world->dynamicsWorld->addMultiBodyConstraint(
            jointMotor->jointMotor.get());
...
ZENDEFNODE(BulletMultiBodyGetJointTorque,
           {{"object", "jointIndex"}, {"torque"}, {}, {"Bullet"}});
...
struct BulletMultiBodyGetLinkForce : zeno::INode {
    virtual void apply() override {
        ...
        force_out->set<zeno::vec3f>(
            zeno::vec3f(force.x(), force.y(), force.z()));
        set_output("force", std::move(force_out));
    }
};

ZENDEFNODE(BulletMultiBodyGetLinkForce,
           {{"object", "linkIndex"}, {"force"}, {}, {"Bullet"}});
...
struct BulletMultiBodyGetLinkTorque : zeno::INode {
    virtual void apply() override {
        ...
        torque_out->set<zeno::vec3f>(
            zeno::vec3f(torque.x(), torque.y(), torque.z()));
        set_output("torque", std::move(torque_out));
    }
};

ZENDEFNODE(BulletMultiBodyGetLinkTorque,
           {{"object", "linkIndex"}, {"torque"}, {}, {"Bullet"}});
...
ZENDEFNODE(BulletMultiBodyGetJointVelPos,
           {{"object", "linkIndex"}, {"vel", "pos"}, {}, {"Bullet"}});
