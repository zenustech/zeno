...
struct ZSLevelSetBinaryOperator : INode {
    ...
    void binaryOp(SplsT &lsa, const SplsT &lsb, T a, T b, Op op) {
        ...
        cudaPol(range(numInBlocks), [taba = proxy<execspace_e::cuda>(lsa._table),
                                     tabb = proxy<execspace_e::cuda>(lsb._table)] __device__(auto bi) mutable {
        ...
        cudaPol(Collapse{numCurBlocks - numPrevBlocks, lsa.block_size},
                [numPrevBlocks, lsa = proxy<execspace_e::cuda>(lsa)] __device__(auto bi, auto ci) mutable {
                    using ls_t = RM_CVREF_T(lsa);
                    auto block = lsa._grid.block(bi + numPrevBlocks);
                    for (typename ls_t::channel_counter_type chn = 0; chn != lsa.numChannels(); ++chn)
        ...
        cudaPol(Collapse{numCurBlocks, lsa.block_size},
                [lsa = proxy<execspace_e::cuda>(lsa), lsb = proxy<execspace_e::cuda>(lsb), a, b,
                 op] __device__(auto bi, auto ci) mutable {
                    using ls_t = RM_CVREF_T(lsa);
                    auto coord = lsa._table._activeKeys[bi] + ls_t::grid_view_t::cellid_to_coord(ci);
                    auto [bnoB, cnoB] = lsb.decompose_coord(coord);
                    auto block = lsa._grid.block(bi);
                    for (typename ls_t::channel_counter_type propNo = 0; propNo != lsa.numProperties(); ++propNo) {
                        ...
                        for (typename ls_t::channel_counter_type chn = 0; chn != propSize; ++chn) {
                            ...
                            const auto &B = b * lsb.value_or(propOffsetB + chn, bnoB, cnoB, 0);
    ...
    void apply() override {
        fmt::print(fg(fmt::color::green), "begin executing ZSLevelSetBinaryOperator\n");
        ...
        match(
            [this, a, b](auto &lsPtr, const auto &lsPtrB, auto op)
                ...
                lsPtr->_min = TV::init([&a = lsPtr->_min, &b = lsPtrB->_min](int i) { return std::min(a(i), b(i)); });
                lsPtr->_max = TV::init([&a = lsPtr->_max, &b = lsPtrB->_max](int i) { return std::max(a(i), b(i)); });
            ...
            [](...) { throw std::runtime_error("both fields are of different spls categories!"); })(fielda, fieldb, op);
        fmt::print(fg(fmt::color::cyan), "done executing ZSLevelSetBinaryOperator\n");
...
struct ResampleZSLevelSet : INode {
    ...
    void resample(SplsT &ls, const RefSplsT &refLs, const zs::PropertyTag &tag) {
        ...
        cudaPol(Collapse{ls.numBlocks(), ls.block_size},
                [ls = proxy<execspace_e::cuda>(ls), refLs = proxy<execspace_e::cuda>(refLs), tag] __device__(
                    typename RM_CVREF_T(ls)::size_type bi, typename RM_CVREF_T(ls)::cell_index_type ci) mutable {
            ...
            if ((int)ls("mark", bi, ci) == 0)
              return; // skip inactive voxels
                    ...
                    if constexpr (ls_t::category == grid_e::staggered) {
                        ...
                        for (typename ls_t::channel_counter_type chn = 0; chn != tag.numChannels; ++chn) {
                            if constexpr (refls_t::category == grid_e::staggered) {
                                auto arena =
                                    refLs.arena(ls.indexToWorld(coord, chn % 3), chn % 3, kernel_linear_c, wrapv<0>{});
                                ls._grid(propOffset + chn, bi, ci) = arena.isample(refPropOffset + chn, 0);
                            } else {
                                auto arena = refLs.arena(ls.indexToWorld(coord, chn % 3), kernel_linear_c, wrapv<0>{});
                                ls._grid(propOffset + chn, bi, ci) = arena.isample(refPropOffset + chn, 0);
                            }
                        }
                        ...
                        if constexpr (refls_t::category == grid_e::staggered) {
                            for (typename ls_t::channel_counter_type chn = 0; chn != tag.numChannels; ++chn) {
                                auto arena = refLs.arena(X, chn % 3, kernel_linear_c, wrapv<0>{}, false_c);
                                ls._grid(propOffset + chn, bi, ci) = arena.isample(refPropOffset + chn, 0);
                            }
                            // ls._grid(propOffset + chn, bi, ci) =
    ...
    void apply() override {
        ...
        match(
            ...
            [](...) { throw std::runtime_error("not both fields are spls!"); })(field, refField);
...
struct AdvectZSLevelSet : INode {
    ...
    int advect(SplsT &lsOut, const VelSplsT &velLs, const float dt) {
        ...
        auto nlayers = std::max((nvoxels + lsOut.side_length - 1) / lsOut.side_length, 2);
        ...
        cudaPol(Collapse{lsOut.numBlocks(), lsOut.block_size},
                [ls = proxy<execspace_e::cuda>(ls), lsOut = proxy<execspace_e::cuda>(lsOut),
                 velLs = proxy<execspace_e::cuda>(velLs),
                 dt] __device__(typename RM_CVREF_T(lsOut)::size_type bi,
                                typename RM_CVREF_T(lsOut)::cell_index_type ci) mutable {
                    using ls_t = RM_CVREF_T(ls);
                    auto coord = lsOut._table._activeKeys[bi] + ls_t::grid_view_t::cellid_to_coord(ci);
                    if constexpr (ls_t::category == grid_e::staggered) {
                        for (int d = 0; d != 3; ++d) {
    ...
    void apply() override {
        ...
        match(
            ...
            [](...) { throw std::runtime_error("not both fields are spls!"); })(field, velField);
...
struct ClampZSLevelSet : INode {
    ...
    void clamp(SplsT &ls, const RefSplsT &refLs, const VelSplsT &velLs, const float dt) {
        ...
        cudaPol(Collapse{ls.numBlocks(), ls.block_size},
                [ls = proxy<execspace_e::cuda>(ls), refLs = proxy<execspace_e::cuda>(refLs),
                 velLs = proxy<execspace_e::cuda>(velLs), dt] __device__(auto bi, auto ci) mutable {
                    using ls_t = RM_CVREF_T(ls);
                    ...
                    if constexpr (ls_t::category == grid_e::staggered) {
                        for (int d = 0; d != 3; ++d) {
                            ...
                            if constexpr (ref_ls_t::category == grid_e::staggered) {
                                auto pad = refLs.arena(xn, d, kernel_linear_c, wrapv<0>{}, true_c);
                        ...
                        if constexpr (ref_ls_t::category == grid_e::staggered) {
                            for (typename ls_t::channel_counter_type chn = 0; chn != ls.numChannels(); ++chn) {
                                int f = chn % 3;
                                auto pad = refLs.arena(xn, f, kernel_linear_c, wrapv<0>{}, true_c);
    ...
    void apply() override {
        ...
        match(
            ...
            [](...) { throw std::runtime_error("not all fields are spls!"); })(field, reffield, velField);
