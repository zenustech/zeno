...
struct ZSLevelSetTopologyUnion : INode {
    ...
    void topologyUnion(SplsT &ls, const TableT &refTable) {
        ...
        cudaPol(range(numInBlocks),
                [ls = proxy<execspace_e::cuda>(ls), refTable = proxy<execspace_e::cuda>(refTable)] __device__(
        ...
        cudaPol(Collapse{numCurBlocks - numPrevBlocks, ls.block_size},
                [numPrevBlocks, ls = proxy<execspace_e::cuda>(ls)] __device__(auto bi, auto ci) mutable {
                    using ls_t = RM_CVREF_T(ls);
    ...
    void apply() override {
        fmt::print(fg(fmt::color::green), "begin executing ZSLevelSetTopologyUnion\n");
        ...
        match(
            [this](auto &lsPtr,
                   const auto &refLsPtr) -> std::enable_if_t<is_spls_v<typename RM_CVREF_T(lsPtr)::element_type> &&
                ...
                lsPtr->_min = TV::init([&a = lsPtr->_min, &b = refLsPtr->_min](int i) { return std::min(a(i), b(i)); });
                lsPtr->_max = TV::init([&a = lsPtr->_max, &b = refLsPtr->_max](int i) { return std::max(a(i), b(i)); });
        ...
        fmt::print(fg(fmt::color::cyan), "done executing ZSLevelSetTopologyUnion\n");
        set_output("ZSField", std::move(zsfield));
    }
};
...
struct ZSLevelSetFloodFill : INode {
    void apply() override {
        ...
        match(
            [&](auto &lsPtr,
                const auto &sdfLs) -> std::enable_if_t<is_spls_v<typename RM_CVREF_T(lsPtr)::element_type>> {
                ...
                if constexpr (is_same_v<sdf_ls_t, basic_ls_t>) {
                    match([&cudaPol, &lsPtr](const auto &sdfLsPtr) {
                        extend_level_set_domain(cudaPol, *lsPtr, get_level_set_view<execspace_e::cuda>(sdfLsPtr));
                    })(sdfLs._ls);
            ...
            [](auto &lsPtr, ...) {
                throw std::runtime_error(fmt::format("levelset [{}] is not a spls!", get_var_type_str(lsPtr)));
