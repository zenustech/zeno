...
struct ZSParticleToZSLevelSet : INode {
    ...
    void p2g(zs::CudaExecutionPolicy &cudaPol, const typename ZenoParticles::particles_t &pars, LsT &ls) {
        using namespace zs;
        ...
        for (auto &&parObjPtr : parObjPtrs) {
            ...
            match(
                [&](auto &lsPtr) -> std::enable_if_t<is_spls_v<typename RM_CVREF_T(lsPtr)::element_type>> {
                    auto &ls = *lsPtr;
                    fmt::print("[par2ls] dx: {}, npars: {}\n", ls._grid.dx, pars.size());
                    p2g(cudaPol, pars, ls);
                },
                [](...) { throw std::runtime_error("field to be transfered must be a sparse levelset!"); })(field);
        }
...
struct PrimitiveToZSLevelSet : INode {
    ...
    void iterate(zs::CudaExecutionPolicy &cudaPol, int nvoxels, const zs::Vector<TV> &pos, SpLsT &ls,
                 zs::Vector<IV> &mi, zs::Vector<IV> &ma) {
        using namespace zs;
        cudaPol(range(pos.size()), [pos = proxy<execspace_e::cuda>(pos), mi = proxy<execspace_e::cuda>(mi),
                                    ma = proxy<execspace_e::cuda>(ma), ls = proxy<execspace_e::cuda>({}, ls),
    ...
    void p2g(zs::CudaExecutionPolicy &cudaPol, int nvoxels, const zs::Vector<TV> &pos, SpLsT &ls) {
        using namespace zs;
        cudaPol(range(pos.size()), [pos = proxy<execspace_e::cuda>(pos), ls = proxy<execspace_e::cuda>({}, ls),
                                    nvoxels] __device__(int pi) mutable {
            using lsv_t = RM_CVREF_T(ls);
            auto x = pos[pi];
            auto coord = ls.worldToCell(x);
    ...
    void apply() override {
        ...
        cudaPol(Collapse{nblocks, ls.block_size},
                [grid = proxy<execspace_e::cuda>({}, ls._grid), thickness,
                 backgroundValue = ls._backgroundValue] __device__(int bi, int ci) mutable {
        ...
        cudaPol(Collapse{nblocks, ls.block_size},
                [grid = proxy<execspace_e::cuda>({}, ls._grid), thickness,
                 backgroundValue = ls._backgroundValue] __device__(int bi, int ci) mutable {
                    if (auto sdf = grid("sdf", bi, ci); sdf < thickness) // touched by particles
