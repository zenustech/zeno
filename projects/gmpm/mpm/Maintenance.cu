        ...
        auto probeSize = [&]() { // side effects: (prev) vert/eleCnt
          ...
          cudaPol(
              ...
              [edgeEles = proxy<execspace_e::cuda>(edgeEles),
               ...
               dx] __device__(int ei) mutable {
                auto edge = edges[ei];
                float3 xs[2] = {pars.pack<3>("x", edge[0]),
                                pars.pack<3>("x", edge[1])};
          ...
          cudaPol(
              ...
              [edgeEles = proxy<execspace_e::cuda>(edgeEles),
               ...
               dx] __device__(int ei) mutable {
                ...
                pars.tuple<3>("v", vi) =
                    (pars.pack<3>("v", edge[0]) + pars.pack<3>("v", edge[1])) *
                    0.5f;
                // F, C
