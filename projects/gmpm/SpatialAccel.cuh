...
#else
#error                                                                         \
    "only include this header when ZS_ENABLE_CUDA is enabled and include only in cu source files."
#endif
...
template <int dim_ = 3, int lane_width_ = 32, typename Index = int,
          typename ValueT = zs::f32, typename AllocatorT = zs::ZSPmrAllocator<>>
struct ZenoLBvh {
  ...
  static_assert(std::is_floating_point_v<value_type>,
                "value_type should be floating point");
  static_assert(std::is_integral_v<index_type>,
                "index_type should be an integral");
  ...
  void build(zs::CudaExecutionPolicy &,
             const zs::Vector<zs::AABBBox<dim, value_type>> &primBvs,
             zs::wrapv<Refit> = {});

  void refit(zs::CudaExecutionPolicy &,
             const zs::Vector<zs::AABBBox<dim, value_type>> &primBvs);
  ...
  template <typename Policy> Box getTotalBox(Policy &&pol) const {
    ...
    if (numLeaves <= 2) {
      ...
      box.setVal(Box{TV::uniform(limits<value_type>::max()),
                     TV::uniform(limits<value_type>::lowest())});
      pol(Collapse{numLeaves},
          [bvh = proxy<space>(*this),
           box = proxy<space>(box)] ZS_LAMBDA(int vi) mutable {
  ...
  bvs_t orderedBvs;
  indices_t parents{}, levels{}, leafInds{},
      auxIndices{}; // escape ids/ prim ids
};

template <zs::execspace_e, typename LBvhT, typename = void> struct LBvhView;

/// proxy to work within each backends
template <zs::execspace_e space, typename LBvhT>
struct LBvhView<space, const LBvhT> {
  ...
  explicit constexpr LBvhView(const LBvhT &lbvh)
      : _orderedBvs{zs::proxy<space>(lbvh.orderedBvs)},
        ...
        _levels{zs::proxy<space>(lbvh.levels)}, _leafInds{zs::proxy<space>(
                                                    lbvh.leafInds)},
  ...
  zs::VectorView<space, const indices_t> _parents, _levels, _leafInds,
      _auxIndices;
  index_t _numNodes;
};

template <zs::execspace_e space, int dim, int lane_width, typename Ti,
          typename T, typename Allocator>
constexpr decltype(auto)
proxy(const ZenoLBvh<dim, lane_width, Ti, T, Allocator> &lbvh) {
  return LBvhView<space, const ZenoLBvh<dim, lane_width, Ti, T, Allocator>>{
      lbvh};
}
...
template <int dim, int lane_width, typename Index, typename Value,
          typename Allocator>
template <bool Refit>
void ZenoLBvh<dim, lane_width, Index, Value, Allocator>::build(
    zs::CudaExecutionPolicy &policy,
    const zs::Vector<zs::AABBBox<dim, Value>> &primBvs, zs::wrapv<Refit>) {
  using namespace zs;
  using T = value_type;
  using Ti = index_type;
  ...
  const auto defaultBox = Box{TV::uniform(limits<value_type>::max()),
                              TV::uniform(limits<value_type>::lowest())};
  Vector<Box> wholeBox{primBvs.get_allocator(), 1};
  wholeBox.setVal(defaultBox);
  policy(primBvs, [box = proxy<space>(wholeBox),
                   execTag] ZS_LAMBDA(const Box &bv) mutable {
    for (int d = 0; d != dim; ++d) {
      atomic_min(execTag, &box(0)._min[d],
                 bv._min[d] - 10 * limits<T>::epsilon());
      atomic_max(execTag, &box(0)._max[d],
                 bv._max[d] + 10 * limits<T>::epsilon());
    }
  });
  ...
  // policy(zip(range(numLeaves), primBvs, mcs, indices),
  // ComputeMortonCodes{wholeBox.getVal()});
  policy(range(numLeaves),
         [wholeBox = wholeBox.getVal(), primBvs = proxy<space>(primBvs),
          mcs = proxy<space>(mcs),
          indices = proxy<space>(indices)] ZS_LAMBDA(auto id) mutable {
           ...
           auto coord = wholeBox.getUniformCoord(c)
                            .template cast<f32>(); // this is a vec<T, dim>
  ...
  radix_sort_pair(policy, mcs.begin(), indices.begin(), sortedMcs.begin(),
                  sortedIndices.begin(), numLeaves);
#endif
...
// build + refit
...
#endif
  {
    ...
    policy(range(numLeaves), [indices = proxy<space>(sortedIndices), pInds,
                              lDepths, numTrunk] ZS_LAMBDA(Ti idx) mutable {
    ...
    /// origin: efficient BVH-based collision detection scheme with ordering and
    /// restructuring
    policy(range(numTrunk), [mcs = proxy<space>(sortedMcs), tPars, tLcs, tRcs,
                             tLs, tRs, lPars, lDepths,
      ...
      if (idx == 0) {
        ...
        if (preCode == curCode && curCode == nxtCode) {
          ...
          for (step = 2; right = left + step * dir,
              (right < num_leaves && right >= 0
                   ? __clz(mcs(right) ^ curCode) > minLZ
                   : false);
               step <<= 1)
            ;
          Ti len;
          ...
          // preCode, curCode, nxtCode, dir, minLZ, len);
          if (dir == 1) {
            i = left;
            j = left + len;
          } else {
...
#else
  exclusive_scan(policy, std::begin(leafDepths), std::end(leafDepths),
                 std::begin(leafOffsets));
#endif
  ...
  policy(range(numLeaves),
         [levels = proxy<space>(levels), lDepths, lOffsets, lPars, lLcas, tLs,
  ...
  if constexpr (false) {
    // check right childs
    policy(range(numTrunk), [lDepths, tLs, tRs, tLcs, tRcs, tPars, lPars, lLcas,
                             tDst, auxIndices = proxy<space>(auxIndices),
                             levels = proxy<space>(levels),
                             parents = proxy<space>(parents),
                             numTrunk] ZS_LAMBDA(Ti idx) mutable {
      const auto l = tLs[idx];
      const auto r = tRs[idx];
      const auto lc = tLcs[idx];
      const auto rc = tRcs[idx];
      auto chkPar = [&](int ch, int par) {
        if (ch < numTrunk) {
          if (int p = tPars[ch]; p != par) {
            printf("trunk child %d <%d, %d>\' parent %d <%d, %d> not %d <%d, "
                   "%d>!\n",
                   ch, tLs[ch], tRs[ch], p, tLs[p], tRs[p], par, tLs[par],
                   tRs[par]);
          }
        } else {
      ...
      if (idx == 0 || !(zs::get<0>(range) == zs::get<0>(lRange) &&
                        zs::get<1>(range) == zs::get<1>(rRange) &&
                        zs::get<1>(lRange) + 1 == zs::get<0>(rRange)))
        printf("<%d, %d> != <%d, %d> + <%d, %d>\n", zs::get<0>(range),
               zs::get<1>(range), zs::get<0>(lRange), zs::get<1>(lRange),
               zs::get<0>(rRange), zs::get<1>(rRange));
      auto level = 2;
      // lca
      Ti lcc = lc;
      while (lcc < numTrunk) {
  ...
  // reorder leaf
  policy(range(numLeaves),
         [lOffsets, lPars, lLcas, auxIndices = proxy<space>(auxIndices),
  ...
  if constexpr (false) {
    ...
    Vector<u64> tab{primBvs.get_allocator(), numNodes},
        tmp{primBvs.get_allocator(), 1};
    ...
    policy(enumerate(trunkDst), [tab = proxy<space>(tab)] ZS_LAMBDA(auto id,
                                                                    auto dst) {
      if (atomic_cas(exec_cuda, &tab[dst], (u64)(dst + 1), (u64)0) != dst + 1)
        printf("\t%d-th trunk node (dst %d) invalid\n", (int)id, (int)dst);
    });
    policy(enumerate(leafInds), [tab = proxy<space>(tab)] ZS_LAMBDA(auto id,
                                                                    auto dst) {
      if (atomic_cas(exec_cuda, &tab[dst], (u64)(dst + 1), (u64)0) != dst + 1)
  ...
  policy(range(numTrunk),
         [lLcas, lOffsets, auxIndices = proxy<space>(auxIndices),
          parents = proxy<space>(parents), tRs, tPars, tDst,
          numTrunk] ZS_LAMBDA(Ti idx) mutable {
           auto dst = tDst[idx];
           auto r = tRs[idx];
           // aux (esc)
    ...
    policy(range(numTrunk), [lDepths, lOffsets, tLs, tRs, tLcs, tRcs, tPars,
      ...
      auto triggered = [&]() {
        ...
        for (auto &&i : zs::range(cnt)) {
          ...
          int lch = tLcs[id];
          printf(
              "tk node %d <%d (depth %d), %d> cur depth %d, level %d, lch %d\n",
              id, tLs[id], lDepths[tLs[id]], tRs[id], depth++, levels[tDst[id]],
              lch);
        }
      };
      auto chkPar = [&](int ch, int par) {
      ...
      if (lDepths[lcc] != levels[tDst[lca]] + 1) {
        printf("depth and level misalign. leaf %d, depth %d, lca level %d !\n",
               (int)lcc, (int)lDepths[lcc], (int)(levels[tDst[lca]] + 1));
      }
      if (false) { // l == 4960 && levels[dst] > 2
        printf("leaf %d branch, lca %d. depth %d, level %d, trunk node %d (%d "
               "ordered)\n",
               (int)l, (int)lca, (int)lDepths[l], (int)levels[tDst[lca]],
               (int)idx, (int)dst);
        for (int n = lca; n < numTrunk; n = tLcs[n]) {
          auto lc_ = tLcs[n];
          printf("node %d <%d, %d>, level %d, lc %d (%d ordered)\n", n,
                 (int)tLs[n], (int)tRs[n], (int)levels[tDst[n]], (int)lc_,
...
template <int dim, int lane_width, typename Index, typename Value,
          typename Allocator>
void ZenoLBvh<dim, lane_width, Index, Value, Allocator>::refit(
    zs::CudaExecutionPolicy &policy,
...
// refit
  ...
  policy(Collapse{numLeaves},
         [primBvs = proxy<space>(primBvs),
          orderedBvs = proxy<space>(orderedBvs),
          auxIndices = proxy<space>(auxIndices),
          leafInds = proxy<space>(leafInds), parents = proxy<space>(parents),
          levels = proxy<space>(levels), flags = proxy<space>(refitFlags),
          numTrunk = numLeaves - 1] ZS_LAMBDA(Ti idx) mutable {
