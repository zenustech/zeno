...
struct MakeSparseGrid : INode {
    void apply() override {
        ...
        int numExpectedBlocks = nbx * nby * nbz;
        ...
        pol(zs::range(bcnt * 512), [spgv = zs::proxy<space>(spg), sphere_c, sphere_r] __device__(int cellno) mutable {
			...
			auto dx = spgv._transform(0, 0);

			float dist2c = zs::sqrt(float(zs::sqr(ccoord[0] - sphere_c[0])
										+ zs::sqr(ccoord[1] - sphere_c[1])
										+ zs::sqr(ccoord[2] - sphere_c[2])));
			
			//spgv("sdf", bno, cno) = ;
			spgv("sdf", ccoord) = dist2c > sphere_r ? dx : -dx;
        });
