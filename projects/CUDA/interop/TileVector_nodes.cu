    ...
    zs::SmallString tagName, int dim, int index)
{
    ...
    pol(range(1),
        [tv = proxy<space>({}, tv),
         res = proxy<space>(res),
         tagName = SmallString{tagName},
         index = index,
         dim = dim] __device__(int i) mutable
        {
            res[0] = tv(tagName, dim, index);
        });
    return res.getVal();
}
...
namespace zeno
{
    struct MakeZsTileVector : INode
    {
        void apply() override
        {
            ...
            auto input_prop_tags = get_input<DictObject>("prop_tags")
                                       ->getLiterial<int>();
...
#define MAKE_TILEVECTOR_OBJ_T(T)                                                               \
    ...
    {                                                                                          \
        ...
        tvObj->set(zs::TileVector<T, 32, zs::ZSPmrAllocator<false>>{allocator, tags, 0});       \
            ...
            std::visit([input_size](auto &tv)
                       { tv.resize(input_size); },
                       tvObj->value);
