...
#define DEFINE_MAT_WITH_ROWS(rows, type, tail)  DEFINE_MAT(rows, 1, type, tail) DEFINE_MAT(rows, 2, type, tail) \
    DEFINE_MAT(rows, 3, type, tail) DEFINE_MAT(rows, 4, type, tail) 
#define DEFINE_SQR_MAT(dim, type, tail) using mat##dim##tail = zs::vec<type, dim, dim>; 
#define DEFINE_MAT_VEC_TYPE(type, tail) DEFINE_VEC(1, type, tail) \
    DEFINE_VEC(2, type, tail) DEFINE_VEC(3, type, tail) DEFINE_VEC(4, type, tail) \
    DEFINE_MAT_WITH_ROWS(1, type, tail) DEFINE_MAT_WITH_ROWS(2, type, tail) \
    DEFINE_MAT_WITH_ROWS(3, type, tail) DEFINE_MAT_WITH_ROWS(4, type, tail) \
    DEFINE_SQR_MAT(1, type, tail) DEFINE_SQR_MAT(2, type, tail) \
...
#define LIST_MAT_VEC_TYPE(tail) mat11##tail, mat12##tail, mat13##tail, mat14##tail, \
    ...
    using SmallVecValue = std::variant<
        LIST_MAT_VEC_TYPE(i), LIST_MAT_VEC_TYPE(f), LIST_MAT_VEC_TYPE(d)
    >; 
}
