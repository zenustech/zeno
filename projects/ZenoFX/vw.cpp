...
void vdb_wrangle(zfx::x64::Executable *exec, GridPtr &grid, bool modifyActive) {
    auto velman =
        openvdb::tree::LeafManager<std::decay_t<decltype(grid->tree())>>(
            grid->tree());
...
struct VDBWrangle : zeno::INode {
    virtual void apply() override {
        ...
        for (auto const &[key_, par] :
             params->getLiterial<zeno::NumericValue>()) {
            auto key = '$' + key_;
            auto dim = std::visit(
                [&](auto const &v) {
        ...
        auto modifyActive =
            (get_input<zeno::StringObject>("ModifyActive")->get()) == "true";
