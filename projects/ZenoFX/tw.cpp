...
struct TrianglesWrangle : zeno::INode {
    virtual void apply() override {
        auto prim = get_input<zeno::PrimitiveObject>("prim");
        auto type = get_input<zeno::StringObject>("faceType")->get();

        if (type == "points") {
            trueapply(prim->points);
        } else if (type == "lines") {
            trueapply(prim->lines);
        } else if (type == "tris") {
            trueapply(prim->tris);
        } else if (type == "quads") {
            ...
            throw zeno::Exception("invalid faceType: " + type);
        }
    ...
    template <class Tris>
    void trueapply(Tris &tris) {
        auto prim = get_input<zeno::PrimitiveObject>("prim");
        auto code = get_input<zeno::StringObject>("zfxCode")->get();

        // BEGIN张心欣快乐自动加@IND
        if (auto pos = code.find("@IND"); pos != code.npos && (code.size() <= pos + 4 || !(isalpha(code[pos + 4]) || strchr("_@$", code[pos + 4]))) && (pos == 0 || !(isalpha(code[pos - 1]) || strchr("_@$", code[pos - 1])))) {
            auto &indatt = tris.template add_attr<float>("IND");
        ...
        constexpr int npoly = is_vec_n<std::decay_t<decltype(tris[0])>>;
	static_assert(npoly <= 9);
	static_assert(std::is_same_v<decay_vec_t<std::decay_t<decltype(tris[0])>>, int>);
        tris.foreach_attr([&] (auto const &key, auto const &attr) {
	...
	//}
		...
		err_printf("ERROR: cannot assign new attribute to verts in triswrangle!");
	    }
...
ZENDEFNODE(TrianglesWrangle, {
    {{"PrimitiveObject", "prim", "", PrimarySocket},
     {"string", "zfxCode"}, {"DictObject:NumericObject", "params", "", PrimarySocket},
...
//struct PrimWrangle : TrianglesWrangle {
//};
