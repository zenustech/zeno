...
struct rigid_body_surface_voxel_mask_reducer {
    ...
    rigid_body_surface_voxel_mask_reducer(const rigid_body_surface_voxel_mask_reducer& other, tbb::split):
...
struct rigid_pressure_force_reducer {
    rigid_pressure_force_reducer(const std::vector<openvdb::BoolTree::LeafNodeType*>& inMaskLeaves,
        const openvdb::Vec3f& inMassCenter,
        openvdb::FloatGrid::Ptr inPressure,
        openvdb::Vec3fGrid::Ptr inFaceWeights,
        float indt):
        myMaskLeaves(inMaskLeaves),
    ...
    rigid_pressure_force_reducer(const rigid_pressure_force_reducer& other, tbb::split):
        myMaskLeaves(other.myMaskLeaves), 
        ...
        zeno::vec3f &totalTorc)
    {
        ...
        tbb::parallel_reduce(tbb::blocked_range<size_t>(0, pos.size(),/*grain size*/1), mask_reducer);
        ...
        rigid_pressure_force_reducer    ForceReducer(mask_leaves,
            masscenter,
            pressure,
            faceWeights, dt);

        tbb::parallel_reduce(tbb::blocked_range<size_t>(0, mask_leaves.size(),/*grain size*/1), ForceReducer);
