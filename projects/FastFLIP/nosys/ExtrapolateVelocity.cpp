...
class vdb_velocity_extrapolator {
  public:
    ...
    static void union_extrapolate(int n_layer, openvdb::FloatGrid::Ptr vx, openvdb::FloatGrid::Ptr vy,
                                  openvdb::FloatGrid::Ptr vz, const openvdb::FloatTree *target_topo = nullptr);
};
...
class velocity_extrapolate_functor {
  ...
  public:
    velocity_extrapolate_functor(const std::vector<vel_tree_node_t *> &in_vel_tree_node_ptrs,
                                 openvdb::Vec3SGrid::Ptr in_vel_grid, openvdb::BoolGrid::Ptr in_empty_indicator)
            ...
            for (int ii = -1; ii <= 1 && !found_empty_neib; ii++) {
                for (int jj = -1; jj <= 1 && !found_empty_neib; jj++) {
                    for (int kk = -1; kk <= 1 && !found_empty_neib; kk++) {
                        ...
                        found_empty_neib =
                            found_empty_neib | const_empty_indicator_axr.getValue(openvdb::Coord{
                                                   leaf_coord[0] + ii, leaf_coord[1] + jj, leaf_coord[2] + kk});
                    }
                }
            }
            if (found_empty_neib) {
        ...
        for (size_t i_node = r.begin(); i_node != r.end(); ++i_node) {
            ...
            if (found_empty_neib) {
                ...
                for (openvdb::Index offset = 0; offset < vel_node.size(); offset++) {
                    ...
                    if (ijk[0] == 0 || ijk[1] == 0 || ijk[2] == 0 || ijk[0] == Nm1 || ijk[1] == Nm1 || ijk[2] == Nm1) {
                        on_border = true;
                    }
        ...
        for (auto leaf = rhs.m_extrapolate_vel_grid->tree().beginLeaf(); leaf; ++leaf) {
            ...
            if (!newLeaf) {
                ...
                m_extrapolate_vel_grid->tree().addLeaf(
                    tree.template stealNode<openvdb::Vec3fGrid::TreeType::LeafNodeType>(leaf->origin(),
                                                                                        openvdb::Vec3f{0}, false));
            } else {
        ...
        for (auto leaf = rhs.m_extrapolate_weight_grid->tree().beginLeaf(); leaf; ++leaf) {
            ...
            if (!newLeaf) {
                ...
                m_extrapolate_weight_grid->tree().addLeaf(
                    tree.template stealNode<openvdb::Int32Grid::TreeType::LeafNodeType>(leaf->origin(), 0, false));
            } else {
                // otherwise increment existing values
                for (auto iter = leaf->cbeginValueOn(); iter; ++iter) {
                    auto val = newLeaf->getValue(iter.offset());
...
struct add_extrapolated_vel_op {
    ...
    add_extrapolated_vel_op(std::vector<openvdb::Vec3fGrid::TreeType::LeafNodeType *> &in_result_vel_nodes,
...
void vdb_velocity_extrapolator::extrapolate(int n_layer, openvdb::Vec3fGrid::Ptr in_out_vel_grid) {
    ...
    for (; n_layer > 0; n_layer--) {
        ...
        auto extrapolator_leafmanager =
            openvdb::tree::LeafManager<openvdb::Vec3fTree>(vel_extrapolator.m_extrapolate_vel_grid->tree());
        ...
        auto set_nodes = [&](openvdb::Vec3SGrid::TreeType::LeafNodeType &extrapolated_vel_leaf,
                             openvdb::Index leafpos) {
            auto *vel_leaf = in_out_vel_grid->tree().touchLeaf(extrapolated_vel_leaf.origin());
            auto *count_leaf =
                vel_extrapolator.m_extrapolate_weight_grid->tree().touchLeaf(extrapolated_vel_leaf.origin());
...
void vdb_velocity_extrapolator::extrapolate(int n_layer, openvdb::FloatGrid::Ptr in_out_scalar_grid) {
    ...
    for (; n_layer > 0; n_layer--) {
        ...
        new_layer_mask->setTree(
            std::make_shared<openvdb::BoolTree>(in_out_scalar_grid->tree(), false, openvdb::TopologyCopy()));
        new_layer_mask->pruneGrid();
        auto original_topology_mask = new_layer_mask->deepCopy();
        openvdb::tools::dilateActiveValues(new_layer_mask->tree(), 1, openvdb::tools::NearestNeighbors::NN_FACE,
                                           openvdb::tools::TilePolicy::IGNORE_TILES);
        ...
        extrapolated_narrowband_grid->setTree(
            std::make_shared<openvdb::FloatTree>(new_layer_mask->tree(), 0.f, openvdb::TopologyCopy()));
...
void vdb_velocity_extrapolator::union_extrapolate(int n_layer, openvdb::FloatGrid::Ptr vx, openvdb::FloatGrid::Ptr vy,
                                                  openvdb::FloatGrid::Ptr vz, const openvdb::FloatTree *target_topo) {
    ...
    for (int channel = 0; channel < 3; channel++) {
        ...
        for (int ilayer = 0; ilayer < n_layer; ilayer++) {
            ...
            valid_vel_topo->setTree(
                std::make_shared<openvdb::BoolTree>(v[channel]->tree(), false, openvdb::TopologyCopy()));
            //called for each new narrow band leaf
            auto value_extrapolator = [&](const tbb::blocked_range<size_t> &r) {
