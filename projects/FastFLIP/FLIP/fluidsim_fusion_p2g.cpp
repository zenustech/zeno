...
struct particle_sorter_body_t {
  ...
  particle_sorter_body_t(
      const std::vector<FLIP_particle> &in_minimum_FLIP_particles,
      size_t number_of_bulks, sparse_fluid8x8x8 &sparse_bulk, float in_dx)
      : m_unsorted_particles(in_minimum_FLIP_particles), m_sparse_bulk{
                                                             sparse_bulk} {
  ...
  particle_sorter_body_t(particle_sorter_body_t &x, tbb::split)
      : m_unsorted_particles(x.m_unsorted_particles), m_sparse_bulk{
                                                          x.m_sparse_bulk} {
  ...
  void operator()(const tbb::blocked_range<size_t> &r) {
    ...
    for (auto i = r.begin(); i != r.end(); i++) {
      ...
      if (is_in_all_bulks) {
        ...
        extrabulk[0].clear();
        extrabulk[0].push_back(0);
        extrabulk[1].clear();
        extrabulk[1].push_back(0);
        extrabulk[2].clear();
        extrabulk[2].push_back(0);

        influencing_bulk_ijk.assign(8, LosTopos::Vec3i{-1, -1, -1});
        auto tmp = p - m_sparse_bulk.bmin;
        LosTopos::Vec3f pos = LosTopos::Vec3f(tmp[0], tmp[1], tmp[2]);
        LosTopos::Vec3i bulk_ijk =
            LosTopos::Vec3i((int)floor(pos[0] / m_sparse_bulk.bulk_size),
        ...
        for (const auto &xx : extrabulk[0]) {
          for (const auto &yy : extrabulk[1]) {
            for (const auto &zz : extrabulk[2]) {
              LosTopos::Vec3i extra_bulk_ijk =
                  bulk_ijk + LosTopos::Vec3i{xx, yy, zz};
              FLUID::Vec3i key{extra_bulk_ijk[0], extra_bulk_ijk[1],
                               extra_bulk_ijk[2]};
              auto found_bulk = m_sparse_bulk.index_mapping.find(key);
              if (found_bulk != m_sparse_bulk.index_mapping.end()) {
                auto idx = found_bulk->second;
                m_bin_bulk_particle[idx].push_back(i);
              }
              // auto idx = (*m_sparse_bulk).get_bulk_index(extra_bulk_ijk[0],
              // extra_bulk_ijk[1], extra_bulk_ijk[2]);
  ...
  void join(const particle_sorter_body_t &other) {
    tbb::parallel_for(
        (size_t)0, m_bin_bulk_particle.size(), [&](size_t bulk_idx) {
          size_t old_size = m_bin_bulk_particle[bulk_idx].size();
          m_bin_bulk_particle[bulk_idx].resize(
              old_size + other.m_bin_bulk_particle[bulk_idx].size());
  ...
  const std::vector<FLIP_particle> &m_unsorted_particles;

  // reduction result
  //[index of bulk][posvel influencing this bulk]
  // note that the beginning of each bin may not be properly aligned with 32bit
  // location
  std::vector<std::vector<int>> m_bin_bulk_particle;
  sparse_fluid8x8x8 &m_sparse_bulk;
  float m_dx;
};
...
struct p2g_body_t {
  p2g_body_t(const std::vector<std::vector<int>> &in_bin_pidx,
             sparse_fluid8x8x8 &in_sparse_bulk,
             const std::vector<FLIP_particle> &in_particles, float in_dx,
             float in_particle_radius)
      : m_bin_pidx(in_bin_pidx), m_particles(in_particles), m_dx(in_dx),
        m_particle_radius(in_particle_radius), m_sparse_bulk{in_sparse_bulk} {
    set_offsets();
  };
  ...
  void set_offsets() {
    ...
    // when later extracting using _mm_store_ps, we get float[0] = arg0
    // correctly see
  ...
  void operator()(const tbb::blocked_range<size_t> &bulk_range) const {
    ...
    for (size_t bulkidx = bulk_range.begin(); bulkidx < bulk_range.end();
         bulkidx++) {

      // clear the temporary
      u.data.assign(512, 0.f);
      wtu.data.assign(512, 0.f);
      v.data.assign(512, 0.f);
      wtv.data.assign(512, 0.f);
      w.data.assign(512, 0.f);
      wtw.data.assign(512, 0.f);
      phi.data.assign(512, 0.f);
      wtphi.data.assign(512, 0.f);

      // the bulk of data to write to
      auto &the_bulk = m_sparse_bulk.fluid_bulk[bulkidx];
      const LosTopos::Vec3i &bulk_corner_ijk{the_bulk.tile_corner[0],
                                             the_bulk.tile_corner[1],
                                             the_bulk.tile_corner[2]};
      ...
      //	for (int j = 0; j < 3; j++) {
      //		aligned_posvel[pidx].pos[j] =
      // m_particles[m_bin_pidx[bulkidx][pidx]].pos[j];
      //		aligned_posvel[pidx].vel[j] =
      // m_particles[m_bin_pidx[bulkidx][pidx]].vel[j];
      //	}
      ...
      for (size_t i_p = 0; i_p < np; i_p++) {
        ...
        const auto pvol = m_particles[m_bin_pidx[bulkidx][i_p]].volm;
        // calculate the weights at the 27 voxels around p
        // note some voxels are outside the current bulk and its weights is not
        // use at all
        ...
        for (size_t ivoxel = 0; ivoxel < 27; ivoxel++) {
          ...
          x_dist_particle_to_sample[ivoxel] = _mm_and_ps(
              absmask,
              _mm_sub_ps(x_offset_to_center_voxel_origin[ivoxel], particle_x));
          y_dist_particle_to_sample[ivoxel] = _mm_and_ps(
              absmask,
              _mm_sub_ps(y_offset_to_center_voxel_origin[ivoxel], particle_y));
          z_dist_particle_to_sample[ivoxel] = _mm_and_ps(
              absmask,
              _mm_sub_ps(z_offset_to_center_voxel_origin[ivoxel], particle_z));

          // transfer the distance to weight at the 27 voxels
          //(1-dist)
          // the far points now becomes negative
          x_dist_particle_to_sample[ivoxel] =
              _mm_sub_ps(float1x4, x_dist_particle_to_sample[ivoxel]);
          y_dist_particle_to_sample[ivoxel] =
              _mm_sub_ps(float1x4, y_dist_particle_to_sample[ivoxel]);
          z_dist_particle_to_sample[ivoxel] =
              _mm_sub_ps(float1x4, z_dist_particle_to_sample[ivoxel]);
          ...
          x_dist_particle_to_sample[ivoxel] =
              _mm_max_ps(float0x4, x_dist_particle_to_sample[ivoxel]);
          y_dist_particle_to_sample[ivoxel] =
              _mm_max_ps(float0x4, y_dist_particle_to_sample[ivoxel]);
          z_dist_particle_to_sample[ivoxel] =
              _mm_max_ps(float0x4, z_dist_particle_to_sample[ivoxel]);

          // turn them into weights reduce to x
          x_dist_particle_to_sample[ivoxel] =
              _mm_mul_ps(x_dist_particle_to_sample[ivoxel],
                         y_dist_particle_to_sample[ivoxel]);
          x_dist_particle_to_sample[ivoxel] =
              _mm_mul_ps(x_dist_particle_to_sample[ivoxel],
                         z_dist_particle_to_sample[ivoxel]);
        ...
        // at this point
        // x_dist_particle_to_sample contains the weights of this particle to
        // the u,v,w,phi sampling point write to the actual result
        ...
        for (size_t ivoxel = 0; ivoxel < 27; ivoxel++) {
          ...
          if (in_bulk) {
            ...
            u.data[mem_offset] += packed_weight[0] * pvel[0] * pvol;
            wtu.data[mem_offset] += packed_weight[0] * pvol;

            v.data[mem_offset] += packed_weight[1] * pvel[1] * pvol;
            wtv.data[mem_offset] += packed_weight[1] * pvol;

            w.data[mem_offset] += packed_weight[2] * pvel[2] * pvol;
            wtw.data[mem_offset] += packed_weight[2] * pvol;
      ...
      for (size_t i_flat_voxel = 0; i_flat_voxel < 512; i_flat_voxel++) {
        ...
        if (wtu.data[i_flat_voxel] > 0) {
          the_bulk.u.data[i_flat_voxel] =
              u.data[i_flat_voxel] / (wtu.data[i_flat_voxel] + 1.0e-4f);
        ...
        if (wtv.data[i_flat_voxel] > 0) {
          the_bulk.v.data[i_flat_voxel] =
              v.data[i_flat_voxel] / (wtv.data[i_flat_voxel] + 1.0e-4f);
        ...
        if (wtw.data[i_flat_voxel] > 0) {
          the_bulk.w.data[i_flat_voxel] =
              w.data[i_flat_voxel] / (wtw.data[i_flat_voxel] + 1.0e-4f);
        ...
        // write to the phi
        // 0.5*dx + 2.0*res[gidx];
        the_bulk.liquid_phi.data[i_flat_voxel] =
            0.5 * m_dx +
            2.0 * phi.data[i_flat_voxel] / (1.0e-5f + wtphi.data[i_flat_voxel]);
      }
  ...
  // the 27 offsets in each channel are ordered according their positions
  // relative to the minimum of the 27 voxel the 000 voxel has (-1,-1,-1) offset
  // to the center voxel the 001 voxel has (-1,-1, 1) offset to the center voxel
  //...
  ...
  sparse_fluid8x8x8 &m_sparse_bulk;
  const std::vector<FLIP_particle> &m_particles;
  const std::vector<std::vector<int>> &m_bin_pidx;
  std::vector<LosTopos::Vec3i> loop_order;
  const float m_dx;
  const float m_particle_radius;
  // dangerous raw pointer to store all the positions and velocities of
  // particles associated with a bulk it will be re-sized for different bulk.
};

} // namespace

void FluidSim::fusion_p2g_liquid_phi(sparse_fluid8x8x8 &_eulerian_fluids,
                                     std::vector<FLIP_particle> &_particles,
                                     float _dx, float _particle_radius) {
  auto sort_body = particle_sorter_body_t{
     _particles, _eulerian_fluids.fluid_bulk.size(), _eulerian_fluids, _dx};
  tbb::parallel_reduce(tbb::blocked_range<size_t>(0, _particles.size()),
                       sort_body);
  // sort_body(tbb::blocked_range<size_t>(0, particles.size()));

  auto p2gbody = p2g_body_t(sort_body.m_bin_bulk_particle, _eulerian_fluids,
                            _particles, _dx, _particle_radius);
  tbb::parallel_for(
      tbb::blocked_range<size_t>(0, _eulerian_fluids.fluid_bulk.size()),
      p2gbody);
  // p2gbody(tbb::blocked_range<size_t>(0, eulerian_fluids->fluid_bulk.size()));
  // extend liquids slightly into solids
  tbb::parallel_for(
      (size_t)0, (size_t)_eulerian_fluids.fluid_bulk.size(), (size_t)1,
      [&](size_t index) {
        for (uint i = 0; i < _eulerian_fluids.n_perbulk; i++) {
          auto const &tmp_ijk = _eulerian_fluids.loop_order[i];
          LosTopos::Vec3i ijk{tmp_ijk[0], tmp_ijk[1], tmp_ijk[2]};
          if (_eulerian_fluids.liquid_phi(index, ijk[0], ijk[1], ijk[2]) <
              0.5 * _dx) {
            float solid_phi =
                0.125 *
                (_eulerian_fluids.solid_phi(index, ijk[0], ijk[1], ijk[2]) +
                 _eulerian_fluids.solid_phi(index, ijk[0] + 1, ijk[1], ijk[2]) +
                 _eulerian_fluids.solid_phi(index, ijk[0], ijk[1] + 1, ijk[2]) +
                 _eulerian_fluids.solid_phi(index, ijk[0] + 1, ijk[1] + 1,
                                           ijk[2]) +
                 _eulerian_fluids.solid_phi(index, ijk[0], ijk[1], ijk[2] + 1) +
                 _eulerian_fluids.solid_phi(index, ijk[0] + 1, ijk[1],
                                           ijk[2] + 1) +
                 _eulerian_fluids.solid_phi(index, ijk[0], ijk[1] + 1,
                                           ijk[2] + 1) +
                 _eulerian_fluids.solid_phi(index, ijk[0] + 1, ijk[1] + 1,
                                           ijk[2] + 1));
            if (solid_phi < 0) {
              _eulerian_fluids.liquid_phi(index, ijk[0], ijk[1], ijk[2]) =
                  -0.5 * _dx;
            }
          }
          if (_eulerian_fluids.liquid_phi(index, ijk[0], ijk[1], ijk[2])>0)
          {
              _eulerian_fluids.fluid_bulk[index].has_hole = true;
          }
        }
      });
}
