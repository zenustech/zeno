...
T interpolate_value(const FLUID::Vec<3,S>& point, const Array3<T, Array1<T> >& grid) {
   int i,j,k;
   S fi,fj,fk;
...
template<class S,class T>
T interpolate_gradient(FLUID::Vec<3,T>& gradient, const FLUID::Vec<3,S>& point, const Array3<T, Array1<T> >& grid) {
   int i,j,k;
   S fx,fy,fz;
