...
// 	//construct the index minimum and maximum that defines the simulation
// domain 	m_domain_index_begin = floorVec3(in_arg.m_domain_vecf_min /
// m_dx); 	m_domain_index_end = ceilVec3(in_arg.m_domain_vecf_max / m_dx);

// 	m_voxel_center_transform =
// openvdb::math::Transform::createLinearTransform(m_dx);
// 	m_voxel_vertex_transform =
// openvdb::math::Transform::createLinearTransform(m_dx);
// 	m_voxel_vertex_transform->postTranslate(openvdb::Vec3d{ -0.5,-0.5,-0.5
// }*double(m_dx));
...
// 	auto first_leaf =
// m_particles->treePtr()->touchLeaf(openvdb::Coord(m_domain_index_begin));

// 	//add descriptors
// 	first_leaf->initializeAttributes(pos_descriptor(), 0);
// 	first_leaf->appendAttribute(first_leaf->attributeSet().descriptor(),
// m_pv_attribute_descriptor, 1);
...
// 		float estimated_steps = std::ceil((target_t - stepped) /
// cfl_dt); 		float estimated_dt = (target_t - stepped) /
// estimated_steps * 1.1; 		float dt = std::min(estimated_dt,
// cfl_dt);
...
// 		//emit_liquid(m_particles,m_source_sdfvel,
// m_domain_index_begin*m_dx, m_domain_index_end*m_dx);
// printf("3\n");
...
// 	auto save_vdb = [&]() {
// 		/*auto diff_pressure = [&](openvdb::FloatTree::LeafNodeType&
// leaf, openvdb::Index leafpos) { 			for (auto iter =
// leaf.beginValueOn(); iter; ++iter)
// { 				iter.setValue(iter.getValue() + 9.81 *
// iter.getCoord()[1] * m_dx);
// 			}
// 		};
...
// 		//openvdb::io::File(s.str()).write({ m_particles, m_velocity,
// m_velocity_weights, m_liquid_sdf,  m_solid_sdf, m_pressure, m_solid_velocity
// });
// 		//openvdb::io::File(s.str()).write({ copied_particles ,
// copied_liquid_sdf,copied_solid_sdf, copied_velocity, copied_old_velocity,
// copied_pressure, copied_face_weights, copied_update, copied_RHS
// ,m_solid_velocity->deepCopy() }); openvdb::io::File(s.str()).write({
// copied_particles ,m_shrinked_liquid_sdf });
...
// 	auto update_FLIP_velocity =
// [&](openvdb::points::PointDataTree::LeafNodeType& leaf, openvdb::Index
// leafpos) {
// 		// Attribute reader
// 		// Extract the position attribute from the leaf by name (P is
// position). 		const openvdb::points::AttributeArray& positionArray =
// 			leaf.attributeArray("P");
// 		// Extract the velocity attribute from the leaf by name (v is
// velocity). 		openvdb::points::AttributeArray& velocityArray =
...
// 			openvdb::Vec3R index_gpos = iter.getCoord().asVec3d() +
// positionHandle.get(*iter); 			auto original_vel =
// openvdb::tools::StaggeredBoxSampler::sample(ovaxr, index_gpos);
// auto updated_vel = openvdb::tools::StaggeredBoxSampler::sample(vaxr,
// index_gpos); 			auto old_pvel =
// velocityHandle.get(*iter); 			velocityHandle.set(*iter,
// (pic_component)*updated_vel + (1.0f
// - pic_component) * (updated_vel - original_vel + old_pvel));
// 		}

// 	};//end update flip velocity

// 	auto particle_man =
// openvdb::tree::LeafManager<openvdb::points::PointDataTree>(m_particles->tree());
...
// 	//openvdb::points::advectPoints(*m_particles, *m_velocity, /*RK order*/
// 1, /*time step*/dt, /*number of steps*/1);
// 	//std::cout << " particle mem " << m_particles->memUsage() << std::endl;
// 	custom_move_points_and_set_flip_vel(*m_particles, *m_velocity,
// *m_velocity_after_p2g, pic_component, dt, /*RK order*/ 1);
// 	//openvdb::points::advectPoints(*m_particles, *m_velocity, /*RK order*/
// 4, /*time step*/dt, /*number of steps*/1);
...
//     const openvdb::Vec3fGrid &in_old_velocity, float PIC_component, float dt,
//     int RK_order) {
//   std::vector<openvdb::points::PointDataTree::LeafNodeType *>
//   particle_leaves; in_out_points.tree().getNodes(particle_leaves);

//   // CSim::TimerMan::timer("Sim.step/vdbflip/advection/reduce").start();
//   auto reducer = std::make_unique<point_to_counter_reducer>(
//       dt, m_dx, in_velocity_field, in_old_velocity, PIC_component,
//       particle_leaves, RK_order);
//   tbb::parallel_reduce(
//       tbb::blocked_range<openvdb::Index>(0, particle_leaves.size(), 10),
//       *reducer);
...
//         // according to the index space leaf position, assign the particles
//         to
//         // the final attribute list
//         // printf("leafcounter %d, attrcounter:%d\n", *index_ends.rbegin(),
//         // reducer.toffset_oindex_oleafpos_hashmap[leaf.origin()]->size());
...
// 	openvdb::tree::LeafManager<openvdb::points::PointDataTree>
// particle_manager(m_particles->tree());

// 	//maybe it's not necessary to allocate these
// 	//particle_manager.foreach(touch_velocity_phi_leaves, false);

// 	auto p2gop = particle_to_grid_reducer(m_velocity, m_velocity_weights,
// m_liquid_sdf, m_particle_radius);
...
// 	tbb::parallel_for(tbb::blocked_range<openvdb::Index>(0,
// fill_kill_nodes.size(), 8), filler); 	m_particles->pruneGrid(0);
...
// void FLIP_vdb::seed_liquid(openvdb::FloatGrid::Ptr in_sdf,
//                            const openvdb::Vec3f &init_vel) {
...
//   // check bounding box
//   auto particle_bbox =
//       openvdb::CoordBBox(openvdb::Coord(m_domain_index_begin - 8),
//                          openvdb::Coord(m_domain_index_end - 1 + 8));

//   auto insdfman =
//       openvdb::tree::LeafManager<openvdb::FloatTree>(in_sdf->tree());
//   auto mark_active_point_leaves = [&](openvdb::FloatTree::LeafNodeType &leaf,
//                                       openvdb::Index leafpos) {
//     using namespace openvdb::tools::local_util;
...
//     for (auto target_ipos = target_iposbegin; target_ipos < target_iposend;
//          target_ipos += 8) {
//       if (target_ipos[0] >= particle_bbox.min()[0] &&
//           target_ipos[0] <= particle_bbox.max()[0] &&
//           target_ipos[1] >= particle_bbox.min()[1] &&
//           target_ipos[2] <= particle_bbox.max()[1] &&
//           target_ipos[2] >= particle_bbox.min()[2] &&
//           target_ipos[2] <= particle_bbox.max()[2]) {
...
//           target_point_leaves.insert(
//               std::make_pair(ceil_leaf->origin(), ceil_leaf));
//         }

//         if (floor_leaf || ceil_leaf) {
//           return;
//         } else {
//           auto newleafc = new openvdb::points::PointDataTree::LeafNodeType(
//               gcoordc, 0, true);
//           auto new_origin = newleafc->origin();
//           bool success = target_point_leaves.insert(
//               std::make_pair(newleafc->origin(), newleafc));
//           if (!success) {
//             delete newleafc;
//           }
...
//             auto newleaff = new openvdb::points::PointDataTree::LeafNodeType(
//                 gcoordf, 0, true);
//             bool success = target_point_leaves.insert(
...
//           leaf.attributeArray("P");
//       // Extract the velocity attribute from the leaf by name (v is
//       velocity). const openvdb::points::AttributeArray &velocityArray =
//           leaf.attributeArray("v");
...
//         auto voxelwpos =
//             m_particles->indexToWorld(leaf.offsetToGlobalCoord(offset));
...
//                                           randomTable[(index++) % 21474836]};
//             auto particlewpos =
//                 m_particles->indexToWorld(particle_pipos) + voxelwpos;
...
//   tbb::parallel_for(tbb::blocked_range<size_t>(0, leafarray.size()),
//   seed_leaf); printf("added particles:%zd\n", size_t(added_particles));
//   printf("original leaf count:%d\n", m_particles->tree().leafCount());
...
// void FLIP_vdb::emit_liquid(openvdb::points::PointDataGrid::Ptr
// in_out_particles, 	const std::vector<sdf_vel_pair>& in_sdf_vels, 	const
// openvdb::Vec3f& in_emit_world_min, 	const openvdb::Vec3f& in_emit_world_max)
// {
// 	for (const auto& sv : in_sdf_vels) {
// 		emit_liquid(in_out_particles, sv, in_emit_world_min,
// in_emit_world_max);
// 	}
// }

// void FLIP_vdb::emit_liquid(openvdb::points::PointDataGrid::Ptr
// in_out_particles, const sdf_vel_pair& in_sdf_vel, const openvdb::Vec3f&
// in_emit_world_min, const openvdb::Vec3f& in_emit_world_max)
// {
// 	using  namespace openvdb::tools::local_util;
...
// 	auto worldinputbbox =
// openvdb::BBoxd(in_sdf->indexToWorld(inputbbox.min()),
// in_sdf->indexToWorld(inputbbox.max())); 	for (int ic = 0; ic < 3; ic++) {
...
// 	auto pispace_range = tbb::blocked_range3d<int, int, int>(loopbegin.x(),
// loopend.x(), loopbegin.y(), loopend.y(), loopbegin.z(), loopend.z());

// 	float dx = in_out_particles->transform().voxelSize()[0];
// 	auto mark_active_point_leaves = [&](const
// tbb::blocked_range3d<int,int,int>& r) { 		auto solid_axr{
// in_sdf->getConstAccessor() };

// 		for (int i = r.pages().begin(); i < r.pages().end(); i +=1) {
// 			for (int j = r.rows().begin(); j < r.rows().end(); j
// +=1) { 				for (int k = r.cols().begin(); k <
// r.cols().end(); k +=1) {
// 					//test if any of the corner of this box
// touches the 					int has_solid = 0;
// int i8 = i * 8, j8 = j * 8, k8 = k * 8; for (int ii
// =
// 0; ii <= 8 && !has_solid; ii += 8) {
// for (int jj = 0; jj <= 8 && !has_solid; jj += 8) { for (int kk = 0; kk <= 8
// && !has_solid; kk += 8) { auto particle_idx_pos = openvdb::Vec3i(i8 + ii, j8
// + jj, k8 + kk); has_solid = (openvdb::tools::BoxSampler::sample(solid_axr,
// in_sdf->worldToIndex(in_out_particles->indexToWorld(particle_idx_pos))) < 0);
// 							}
// 						}
// 					}
...
// 					if (has_solid) {
// 						auto floor_leaf =
// in_out_particles->tree().probeLeaf(openvdb::Coord(i8,j8,k8));
// if (floor_leaf) {
// target_point_leaves.insert(std::make_pair(floor_leaf->origin(), floor_leaf));
// 						}
// 						else {
// 							auto newleaff = new
// openvdb::points::PointDataTree::LeafNodeType(openvdb::Coord(i8, j8, k8), 0,
// true); 							auto new_origin
// =
// newleaff->origin(); 							bool
// success = target_point_leaves.insert(std::make_pair(newleaff->origin(),
// newleaff)); if
// (!success) { delete newleaff;
// 							}
// 						}//end else found existing
// particle leaf
// 					}//end if touch solid
// 				}//loop k
// 			}//loop j
...
// it->attributeSet().descriptorPtr(); 				break;
// 			}
// 		}
// 	}
...
// 			if (leaf.getLastValue() == 0) {
// 				//initialize the attribute descriptor
// 				auto local_pv_descriptor =
// m_pv_attribute_descriptor;
// 				leaf.initializeAttributes(m_position_attribute_descriptor,
// 0);
// leaf.appendAttribute(leaf.attributeSet().descriptor(), local_pv_descriptor,
// 1);
// 			}
...
// 			const openvdb::points::AttributeArray& positionArray =
// 				leaf.attributeArray("P");
// 			// Extract the velocity attribute from the leaf by name
// (v is velocity). 			const openvdb::points::AttributeArray&
// velocityArray = 				leaf.attributeArray("v");
// auto p_handle_ptr = openvdb::points::AttributeHandle<openvdb::Vec3f,
// FLIP_vdb::PositionCodec>::create(positionArray); 			auto
// v_handle_ptr = openvdb::points::AttributeHandle<openvdb::Vec3f,
// FLIP_vdb::VelocityCodec>::create(velocityArray);
...
// 				//used to indicate sub_voxel(8 cells) occupancy
// by
// particles 				unsigned char sub_voxel_occupancy = 0;

// 				//first emit original particles
// 				openvdb::Index32 this_voxel_emitted = 0;
// 				for (auto idx = idxbegin; idx < idxend; ++idx) {
// 					auto p = p_handle_ptr->get(idx);
// 					unsigned char sv_pos = ((p[2] > 0.f) <<
// 2)
// |
// ((p[1] > 0.f) << 1) | (p[0] > 0.f);
...
// auto voxelipos = in_sdf->worldToIndex(voxelwpos); if
// (openvdb::tools::BoxSampler::sample(in_sdf_axr, voxelipos) < dx) {
// const int
// max_emit_trial = 16; 					for (int trial =
// 0; this_voxel_emitted < 8 && trial<max_emit_trial; trial++) { openvdb::Vec3d
// particle_pipos{ distribution(generator) ,distribution(generator)
// ,distribution(generator) }; 						auto& p
// =
// particle_pipos; 						unsigned char
// sv_pos
// =
// ((p[2] > 0) << 2) |
// ((p[1] > 0) << 1) | (p[0] > 0);
// 						//only emit uniformly, otherwise
// skip it 						if (sub_voxel_occupancy
// & (1 << sv_pos)) {
// 							//that bit is active,
// there is already an particle
// 							//skip it
// 							continue;
// 						}
// 						auto particlewpos =
...
// auto particle_sdfipos = in_sdf->worldToIndex(particlewpos);
// if (openvdb::tools::BoxSampler::sample(in_sdf_axr, particle_sdfipos) <
// sdf_threshold) {
// sub_voxel_occupancy
// |=
// 1
...
// emitted_particle);
// leaf.appendAttribute(leaf.attributeSet().descriptor(), local_pv_descriptor,
// 1); 			leaf.setOffsets(new_idxend);

// 			//set the positions and velocities
// 			//get the new attribute arrays
// 			openvdb::points::AttributeArray& posarray =
// leaf.attributeArray("P"); 			openvdb::points::AttributeArray&
// varray = leaf.attributeArray("v");

// 			// Create read handles for position and velocity
// 			openvdb::points::AttributeWriteHandle<openvdb::Vec3f,
// FLIP_vdb::PositionCodec> posWHandle(posarray);
// 			openvdb::points::AttributeWriteHandle<openvdb::Vec3f,
// FLIP_vdb::VelocityCodec> vWHandle(varray);
...
// void FLIP_vdb::sink_liquid(const std::vector<openvdb::FloatGrid::Ptr>&
// in_sdfs)
// {
// 	//remove all particles inside solids, in sink zone, and outside of the
// bounding box 	auto partman =
// openvdb::tree::LeafManager<openvdb::points::PointDataTree>(m_particles->tree());

// 	std::vector<openvdb::points::PointDataTree::LeafNodeType*> leaves;
// 	partman.getNodes(leaves);

// 	openvdb::Vec3d idx_domain_min = m_domain_index_begin - openvdb::Vec3d{
// 0.5 }, 		idx_domain_max= m_domain_index_end - openvdb::Vec3d{ 0.5
// };

// 	auto sink_leaf = [&](const tbb::blocked_range<size_t>& r) {
// 		std::vector<openvdb::tree::ValueAccessor<const
// openvdb::FloatTree>> sink_axrs; 		for (size_t isink = 0; isink <
// in_sdfs.size(); isink++) {
// 			sink_axrs.push_back(openvdb::tree::ValueAccessor<const
// openvdb::FloatTree>{ in_sdfs[isink]->tree() });
// 		}

// 		auto particle_voxel_inside_domain = [&](const openvdb::Coord&
// voxel_coord) { 			return (voxel_coord[0] >=
// m_domain_index_begin[0] && 				voxel_coord[1] >=
// m_domain_index_begin[1] && 				voxel_coord[2] >=
// m_domain_index_begin[2] && 				voxel_coord[0] <
// m_domain_index_end[0] && 				voxel_coord[1] <
// m_domain_index_end[1] && 				voxel_coord[2] <
// m_domain_index_end[2]);
// 		};

// 		auto touch_sink = [&](const openvdb::Vec3d& pipos, float delta =
// 0)
// {
// 			//check domain boundary in index space
// 			if (pipos[0] <= idx_domain_min[0] || pipos[0] >=
// idx_domain_max[0]
// 				|| pipos[1] <= idx_domain_min[1] || pipos[1] >=
// idx_domain_max[1]
// 				|| pipos[2] <= idx_domain_min[2] || pipos[2] >=
// idx_domain_max[2]) { 				return true;
// 			}
...
// 				openvdb::Vec3d sinkipos =
// in_sdfs[isink]->worldToIndex(wpos); 				if
// (openvdb::tools::BoxSampler::sample(sink_axrs[isink], sinkipos) < delta) {
// 					return true;
// 				}
// 			}
...
// 			//anyone of its corners touch sink zone then it is worth
// processing 			for (int i = 0; i <= 8 && !corner_sink; i += 8)
// { for (int j = 0; j <= 8 && !corner_sink; j += 8) { for (int k = 0; k <= 8 &&
// !corner_sink; k
// += 8) { 						corner_sink |=
// touch_sink(leaforigin.offset(i, j, k).asVec3d() - openvdb::Vec3d{ 0.5 });
// 					}//k
// 				}//j
// 			}//i

// 			if (!corner_sink) { continue; }
// 			//below this line, at least one corner of the particle
// leaf touches sink zone
...
// 			float voxeldelta = 0.5 *
// m_particles->transform().voxelSize()[0];

// 			//loop over all voxels
// 			for (auto offset = 0; offset <
// leaves[leafpos]->SIZE;++offset) { 				int idxend =
// leaves[leafpos]->getValue(offset); 				int idxbegin =
// 0; if (offset != 0) { 					idxbegin =
// leaves[leafpos]->getValue(offset - 1);
// 				}
...
// 				auto voxelipos
// =leaves[leafpos]->offsetToGlobalCoord(offset);
...
// 					for (auto idx = idxbegin; idx < idxend;
// idx++) { 						auto particleipos =
// positionHandle.get(idx) + voxelipos;
// if
// (!touch_sink(particleipos)) {
...
// 				auto local_pv_descriptor =
// m_pv_attribute_descriptor;
// 				leaves[leafpos]->initializeAttributes(m_position_attribute_descriptor,
// new_p.size());
// 				leaves[leafpos]->appendAttribute(leaves[leafpos]->attributeSet().descriptor(),
// local_pv_descriptor, 1);
...
// 				for (auto iter =
// leaves[leafpos]->beginIndexOn(); iter;
// ++iter) { 					posWHandle.set(*iter,
// new_p[*iter]); vWHandle.set(*iter, new_v[*iter]);
// 				}
// 			}//end if has points to write to attribute
...
// void FLIP_vdb::narrow_band_particles(openvdb::points::PointDataGrid::Ptr
// out_nb_particles, 	openvdb::FloatGrid::Ptr out_interior_sdf,
// 	openvdb::points::PointDataGrid::Ptr in_particles, int nlayer)
// {
// 	//particles are stored in particle voxels
// 	//1 dilate particle voxels and mark existing part as inactive to get the
// surface mask
...
// 	//4 use the erosion mask to generate interior sdf (every voxel has
// -0.5dx)
...
// 	surface_mask->setTree(std::make_shared<openvdb::BoolTree>(in_particles->tree(),
// /*inactive value*/false,/*active value*/true, openvdb::TopologyCopy()));
// 	openvdb::tools::dilateActiveValues(surface_mask->tree(), 1,
// openvdb::tools::NearestNeighbors::NN_FACE_EDGE_VERTEX);

// 	//1.2 hollow the surface mask
// 	auto hollow_surface_mask_lambda =
// [in_particles](openvdb::BoolTree::LeafNodeType& leaf, openvdb::Index leafpos)
// { 		auto particle_leaf =
// in_particles->tree().probeConstLeaf(leaf.origin()); 		if
// (!particle_leaf) { 			return;
// 		}
...
// 	auto surface_mask_man =
// openvdb::tree::LeafManager<openvdb::BoolTree>(surface_mask->tree());
...
// 	//4 use the erosion mask to generate interior sdf (every voxel has
// -0.5dx) 	float dx = in_particles->transform().voxelSize()[0];

// 	auto interior_sdf = openvdb::FloatGrid::create(0.5 * dx);
// 	interior_sdf->setTree(std::make_shared<openvdb::FloatTree>(in_particles->tree(),
// /*inactive value*/0.5f*dx,/*active value*/-0.5f*dx,
// openvdb::TopologyCopy()));

// 	auto erosion_lambda = [erosion_mask,
// dx](openvdb::FloatTree::LeafNodeType& leaf, openvdb::Index leafpos) {
...
// 		}
// 	};//end erosion lambda
// 	auto interior_sdf_man =
// openvdb::tree::LeafManager<openvdb::FloatTree>(interior_sdf->tree());
...
// 	auto populate_nb_particle_lambda =
// [in_particles](openvdb::points::PointDataTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		auto original_leaf =
// in_particles->tree().probeConstLeaf(leaf.origin()); 		if
// (!original_leaf) { 			leaf.setValuesOff();
// return;
// 		}

// 		if (original_leaf->getLastValue() == 0) {
// 			//original leaf is empty, possibly not having any
// attribute sets 			leaf.setValuesOff();
// return;
// 		}
...
// 		//copy the attributes
// 		leaf.replaceAttributeSet(new
// openvdb::points::AttributeSet(original_leaf->attributeSet(), emitted), true);
// 		for (const auto& it :
// original_leaf->attributeSet().descriptor().map()) { 			const
// auto attributeIndex = static_cast<openvdb::Index>(it.second);

// 			auto& target_array =
// leaf.attributeArray(attributeIndex);

// 			narrow_band_copy_iterator
// copyiter(original_particle_idx);

// 			target_array.copyValues(original_leaf->attributeArray(attributeIndex),
// copyiter);
// 		}
// 	};//end populate narrow band particles lambda
// 	auto nb_manager =
// openvdb::tree::LeafManager<openvdb::points::PointDataTree>(nb_particles->tree());
...
// { 		auto* p =
// m_boundary_velocity_volume->tree().touchLeaf(iter->origin());
// 		p->fill(init_vel);
// 	}
// }
...
// 				vtx[ii * 4 + jj * 2 + kk] =
// 					boxmin + openvdb::Vec3f{ ii *
// box_size[0],jj
// * box_size[1],kk * box_size[2] };
// 			}
// 		}
// 	}
...
// 	m_boundary_fill_kill_volume =
// openvdb::tools::meshToLevelSet<openvdb::FloatGrid>(*m_voxel_center_transform,
// vtx, faces, 3.0f); 	m_boundary_fill_kill_volume->setName("boundary_volume");
// 	m_boundary_fill_kill_volume->setGridClass(openvdb::GridClass::GRID_LEVEL_SET);
// 	//openvdb::io::File("boundary_volume.vdb").write({
// m_boundary_fill_kill_volume });
// }
...
// 	auto shrinked_domain_begin = m_domain_index_begin + openvdb::Vec3i{
// narrow_band_width }; 	auto shrinked_domain_end = m_domain_index_end -
// openvdb::Vec3i{ narrow_band_width };

// 	auto extended_domain_begin = m_domain_index_begin - openvdb::Vec3i{
// narrow_band_width }; 	auto extended_domain_end = m_domain_index_end +
// openvdb::Vec3i{ narrow_band_width };

// 	//first node
// 	auto* first_node = m_domain_solid_sdf->tree().touchLeaf(openvdb::Coord{
// extended_domain_begin }); 	auto touch_domain_begin = first_node->origin();
...
// 					for (int ic = 0; ic < 3; ic++) {
// 						if (leaf_origin[ic] +
// openvdb::Coord{ ii,jj,kk
// }[ic] < shrinked_domain_begin[ic]) {
// return true;
// 						}
// 						if (leaf_origin[ic] +
// openvdb::Coord{ ii,jj,kk
// }[ic] > shrinked_domain_end[ic]) {
// return true;
// 						}
// 					}
// 				}
...
// 		for (int y = touch_domain_begin[1]; y <= touch_domain_end[1];
// y++) { 			for (int z = touch_domain_begin[2]; z <=
// touch_domain_end[2]; z++) { 				if
// (has_point_outside_domain(openvdb::Coord{ x,y,z })) {
// 					m_domain_solid_sdf->tree().touchLeaf(openvdb::Coord{
// x,y,z });
// 				}
// 			}
// 		}
...
// 	auto leafman =
// openvdb::tree::LeafManager<openvdb::FloatTree>(m_domain_solid_sdf->tree());

// 	openvdb::Vec3f domain_center = 0.5f * (m_domain_index_begin +
// m_domain_index_end); 	openvdb::Vec3f domain_extend = openvdb::Vec3f{
// m_domain_index_end } - domain_center;

// 	auto set_leaf_sdf = [&](openvdb::FloatTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		for (auto offset = 0; offset <
// leaf.SIZE; offset++)
// {
...
// 				//but its solid vertex happens to be on the
...
// 			if (voxel_is_outside) {
// 				//outside of the domain means it's inside the
// solid container, set negaive distance
// leaf.setValueOn(offset, -m_dx * (ppos - cpos).asVec3s().length());
// 			}
// 			else {
// 				//find the closest way to move this point to the
// boundary 				int effort[6];
// 				//negx
...
// 		for (auto leafiter = solidsdfptr->tree().cbeginLeaf(); leafiter;
// ++leafiter) { 			auto source_ipos = leafiter->origin();
// auto source_wpos =
// solidsdfptr->indexToWorld(source_ipos); 			auto target_ipos
// = openvdb::Coord(floorVec3(m_solid_sdf->worldToIndex(source_wpos)));
// 			m_solid_sdf->tree().touchLeaf(target_ipos);
// 		}
// 		/*auto toucher = solid_leaf_tile_toucher(m_solid_sdf,
// solidsdfptr); 		solidsdfptr->tree().visit(toucher);*/
// 	}
...
// 	auto collect_liquid_origins =
// [&](openvdb::points::PointDataTree::LeafNodeType& leaf, openvdb::Index
// leafpos) { 		for (auto& solidsdfptr : m_moving_solids) {
// auto axr{
// solidsdfptr->getConstUnsafeAccessor() }; 			for (int ii = 0;
// ii
// <= 8; ii += 8) { 				for (int jj = 0; jj <= 8; jj +=
// 8) { for (int kk = 0; kk <=
// 8; kk += 8) { 						auto at_coord =
// leaf.origin().offsetBy(ii, jj, kk); auto wpos =
// m_particles->indexToWorld(at_coord); auto ipos =
// solidsdfptr->worldToIndex(wpos); if (openvdb::tools::BoxSampler::sample(axr,
// ipos) < 0) { neib_liquid_origins.push_back(leaf.origin()); return;
// 						}
// 					}//end kk
// 				}//end jj
// 			}//end ii
...
// 	auto update_solid_op = [&](openvdb::FloatTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		for (const auto& solidsdfptr :
// m_moving_solids) { 			auto source_solid_axr{
// solidsdfptr->getConstAccessor()
// };
...
// 				float current_sdf = leaf.getValue(offset);
// 				auto current_Ipos =
// leaf.offsetToGlobalCoord(offset); 				auto
// current_wpos = m_solid_sdf->indexToWorld(current_Ipos);
// 				//interpolate the value at the
// 				float source_sdf =
// openvdb::tools::BoxSampler::sample(
// source_solid_axr, solidsdfptr->worldToIndex(current_wpos));
...
// 	auto solid_leafman =
// openvdb::tree::LeafManager<openvdb::FloatTree>(m_solid_sdf->tree());
...
// for (auto viter = iter->cbeginValueOn(); viter != iter->cendValueOn();
// ++viter) { 			leaf->setValueOn(viter.offset(),
// 				std::min(leaf->getValue(viter.offset()),
// viter.getValue()));
// 		}
// 	}
// }
...
// 				if (voxel_solid_sdf<0 && leaf.getValue(offset) <
// (-voxel_solid_sdf)) {
...
// 	auto phi_manager =
// openvdb::tree::LeafManager<openvdb::FloatTree>(m_liquid_sdf->tree());
...
// 	//this is to be called by a solid manager
// 	auto immerse_liquid_into_solid = [&](openvdb::FloatTree::LeafNodeType&
// leaf, openvdb::Index leafpos) {
// 		//detech if there is solid
// 		if (m_solid_sdf->tree().probeConstLeaf(leaf.origin())) {
// 			auto pushed_out_liquid_axr{
// m_pushed_out_liquid_sdf->getConstAccessor() }; 			auto
// const_solid_axr{
// m_solid_sdf->getConstAccessor() }; 			auto shift{
// openvdb::Vec3R{0.5} };

// 			for (auto iter = leaf.beginValueOn(); iter; ++iter) {
// 				auto voxel_solid_sdf =
// openvdb::tools::BoxSampler::sample(
// m_solid_sdf->tree(), leaf.offsetToGlobalCoord(iter.offset()).asVec3d() +
// shift);

// 				if (voxel_solid_sdf < 0) {
// 					bool found_liquid_neib = false;
// 					for (int i_neib = 0; i_neib < 6 &&
// !found_liquid_neib; i_neib++) { int component = i_neib / 2;
// int positive_dir = (i_neib % 2 == 0);
...
// 	auto solid_vel_set_op = [&](openvdb::Vec3fTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		auto weight_axr{
// m_face_weight->getConstAccessor()
// }; 		for (auto offset = 0; offset < leaf.SIZE; ++offset) {
// if (weight_axr.getValue(leaf.offsetToGlobalCoord(offset)) != openvdb::Vec3f{
// 1
// }) {
// 				//has solid component
...
// 	auto leafman =
// openvdb::tree::LeafManager<openvdb::Vec3fTree>(m_solid_velocity->tree());

// 	leafman.foreach(solid_vel_set_op);

// 	auto set_moving_solid_vel = [&](openvdb::Vec3fTree::LeafNodeType& leaf,
// openvdb::Index leafpos) {
// 		//check if it is close to a moving objects
// 		//if so, set its velocity to the object's velocity, because the
// velocity volume may
...
// 			for (auto iter = leaf.beginValueOn(); iter; ++iter) {
// 				auto wpos =
// m_solid_velocity->indexToWorld(iter.getCoord()); auto ipos =
// solid->worldToIndex(wpos); 				if
// (openvdb::tools::BoxSampler::sample(solid_axr,ipos) <=2*m_dx) {
...
// 	invmat.col(1) = invdx2 * Eigen::Vector4f{ 0, 1.0f / 3.0f ,0 ,-1.0f
// / 6.0f }; 	invmat.col(2) = invdx2 * Eigen::Vector4f{ 0, 0, 1.0f / 3.0f
// ,-1.0f / 6.0f }; 	invmat.col(3) = invdx2 * Eigen::Vector4f{ 0, -1.0f
// / 6.0f, -1.0f / 6.0f, 1.0f / 4.0f };
...
// 	//this is to be used by the velocity(post_pressure) leaf manager
// 	auto velocity_update_op = [&](openvdb::Vec3fTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		auto phi_axr{
// m_liquid_sdf->getConstAccessor() }; 		auto true_phi_axr{
// m_pushed_out_liquid_sdf->getConstAccessor()
// }; 		auto weight_axr{ m_face_weight->getConstAccessor() };
// auto solid_vel_axr{ m_solid_velocity->getConstAccessor() }; 		auto
// solid_sdf_axr{ m_solid_sdf->getConstAccessor() }; 		auto
// pressure_axr{ m_pressure->getConstAccessor() };
// 		//auto update_axr{ m_velocity_update->getAccessor() };
...
// 				continue;
// 			}
// 			//we are looking at a velocity sample previously
// obtained from p2g
// 			//possibly with extrapolation
...
// 					//does it have any dof on its side?
// 					auto phi_this =
// phi_axr.getValue(gcoord); 					auto phi_below =
// phi_axr.getValue(lower_gcoord); 					float
// p_this = pressure_axr.getValue(gcoord); float p_below =
// pressure_axr.getValue(lower_gcoord);
...
// 					}
// 					else {
// 						if (phi_this > 0 && phi_below >
// 0)
// { 							update_this_velocity =
// false; } // end if all outside liquid
// else
// {
...
// {
// 								//this point is
// outside the liquid, possibly free air if
// (openvdb::tools::BoxSampler::sample(solid_sdf_axr,
// gcoord.asVec3d()+openvdb::Vec3d(0.5)) > 0) {
// 									//if so,
// set the pressure to be ghost value if (true_phi_axr.getValue(lower_gcoord) <
// 0) { update_this_velocity = true;
// 									}
// 									//p_this
// = p_below / std::min(phi_below, - m_dx*1e-3f) * phi_this;
// 								}
// 							}
...
// 								//this point is
// outside the liquid, possibly free air if
// (openvdb::tools::BoxSampler::sample(solid_sdf_axr, lower_gcoord.asVec3d()
// + openvdb::Vec3d(0.5)) > 0) {
// 									//if so,
// set the pressure to be ghost value if (true_phi_axr.getValue(gcoord) < 0) {
// update_this_velocity = true;
// 									}
// 									//p_below
// = p_this / std::min(phi_this, - m_dx * 1e-3f) * phi_below;
// 								}
// 							}
// 						} //end else all outside liquid
// 					}//end all inside liquid
...
// 						if (phi_this >= 0 || phi_below
// >= 0) { 							theta =
// fraction_inside(phi_below,
// phi_this); 							if (theta <
// 0.02f) theta = 0.02f;
// 						}

// 						original_vel[ic] -= in_dt *
// (float)(p_this
// -
// p_below) / m_dx / theta; vel_update[ic] = in_dt
// * (float)(p_this - p_below) / m_dx / theta;
// if (face_weights[ic] < 1) {
// 							//use the voxel center
// solid sdf
// gradient 							openvdb::Vec3f
// grad_solid{
// 0
// }; openvdb::Vec3f evalpos{ 0.5 };

// 							//evaluate the four sdf
// on this face Eigen::VectorXf neib_sdf = Eigen::VectorXf::Zero(12);

// 							//four_sdf[0] =
// solid_sdf_axr.getValue(gcoord);
// 							//calculate the in-plane
// solid normal
// 							//then deduce the
// off-plane
// solid normal 							for (int
// iclevel = -1; iclevel <= 1; iclevel++) {
// int ic4 = (iclevel + 1) * 4;
// switch (ic) { case 0:
// 									//   ^z
// 									//(0,0,1)------(0,1,1)
...
// 									//u
// 									neib_sdf[0
// + ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(iclevel, 0, 0)); neib_sdf[1 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(iclevel, 1, 0)); neib_sdf[2 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(iclevel, 1, 1)); neib_sdf[3 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(iclevel, 0, 1)); break;
// case 1:
// 									//   ^x
// 									//(1,0,0)------(1,0,1)
// 									//   | |
...
// 									//v
// 									neib_sdf[0
// + ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(0, iclevel, 0)); neib_sdf[1 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(0, iclevel, 1)); neib_sdf[2 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(1, iclevel, 1)); neib_sdf[3 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(1, iclevel, 0)); break;
// case 2:
// 									//   ^y
// 									//(0,1,0)------(1,1,0)
// 									//   | |
...
// 									//w
// 									neib_sdf[0
// + ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(0, 0, iclevel)); neib_sdf[1 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(1, 0, iclevel)); neib_sdf[2 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(1, 1, iclevel)); neib_sdf[3 +
// ic4] = solid_sdf_axr.getValue(gcoord.offsetBy(0, 1, iclevel)); break;
// 								}
// 							}
...
// * mT
// *
// neib_sdf; 							float normal_a =
// std::max(-1.f, std::min(1.f, abcd[0]));

// 							//make sure on the
// normal direction, the velocity has the same component

// 							openvdb::Vec3f facev{
// 0,0,0
// }; 							grad_solid[ic] =
// normal_a; facev[ic] = original_vel[ic]; if
// (1||(original_vel[ic]-solid_vel[ic]) * normal_a < 0) { facev -= grad_solid *
// grad_solid.dot(facev); facev += grad_solid * grad_solid.dot(solid_vel);
// 							}
// 							original_vel[ic] =
// facev[ic];
// 							/*float penetrating =
...
// -= penetrating;
// original_vel[ic]
// += grad_solid[ic] * solid_vel[ic];
// 							}*/

// 							//mix the solid velocity
// and fluid velocity 							float
// solid_fraction = (1 - face_weights[ic])*0.8;
// original_vel[ic] = (1-solid_fraction) * original_vel[ic] + (solid_fraction) *
// solid_vel[ic];
// 						}
// 						has_any_update = true;
// 					}//end if any dofs on two sides
...
// 			else {
// 				leaf.setValueOn(offset, original_vel);
// 				//update_axr.setValue(leaf.offsetToGlobalCoord(offset),
// vel_update);
// 			}
// 		}//end for all voxels
// 	};//end velocity_update_op
...
// 	auto set_acceleration_field = [&](openvdb::Vec3fTree::LeafNodeType&
// leaf, openvdb::Index leafpos) { 		leaf.fill(openvdb::Vec3f(0, 0,
// 0)); for (auto propeller : m_propeller_sdf) { 			auto
// propaxr{ propeller->getConstAccessor() }; 			bool
// propeller_not_found = true; 			for (int ii = 0; ii <= 8 &&
// propeller_not_found; ii += 4) { for (int jj = 0; jj <= 8 &&
// propeller_not_found; jj += 4) { for (int kk = 0; kk <= 8 &&
// propeller_not_found; kk += 4) { auto wpos =
// m_acceleration_fields->indexToWorld(leaf.origin().offsetBy(ii, jj, kk));
// auto ipos = propeller->worldToIndex(wpos); if
// (openvdb::tools::BoxSampler::sample(propaxr,ipos)<0) {
...
// 				for (auto iter = leaf.beginValueOn(); iter;
// ++iter) { 					auto wpos =
// m_acceleration_fields->indexToWorld(iter.getCoord());
// auto ipos = propeller->worldToIndex(wpos); if
// (openvdb::tools::BoxSampler::sample(propaxr, ipos) < 0) {
...
// 	};
// 	auto accman =
// openvdb::tree::LeafManager<openvdb::Vec3fTree>(m_acceleration_fields->tree());
// 	accman.foreach(set_acceleration_field);

// 	auto add_gravity = [&](openvdb::Vec3fTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		auto face_weight_axr{
// m_face_weight->getConstAccessor() }; 		for (auto iter =
// leaf.beginValueOn(); iter != leaf.endValueOn(); ++iter) { if
// (face_weight_axr.getValue(iter.getCoord())[1] > 0) {
// 				iter.modifyValue([dt](openvdb::Vec3f& v) {v[1]
// -= dt
// * 9.81; });
// 			}
// 		}
// 		auto accaxr{ m_acceleration_fields->getConstAccessor() };
// 		for (auto iter = leaf.beginValueOn(); iter != leaf.endValueOn();
// ++iter) { 			if (face_weight_axr.getValue(iter.getCoord())[1]
// >
// 0) { 				iter.modifyValue([&](openvdb::Vec3f& v)
// {v
// += dt
// * accaxr.getValue(iter.getCoord()); });
// 			}
// 		}
// 	};
...
// openvdb::Index leafpos) { 		float max_v = 0; 		for
// (auto offset = 0; offset < leaf.SIZE; ++offset) { 			if
// (leaf.isValueOn(offset)) { 				for (int ic = 0; ic < 3;
// ic++) { 					if (max_v <
// std::abs(leaf.getValue(offset)[ic])) {
...
// 	auto leafman = openvdb::tree::LeafManager
// <openvdb::Vec3fTree>(m_velocity->tree());
...
// 	std::nth_element(max_v_per_leaf.begin(), max_v_per_leaf.begin() + nleaf
// - 1, max_v_per_leaf.end()); 	std::nth_element(max_v_per_leaf.begin(),
// max_v_per_leaf.begin() + top90, max_v_per_leaf.end());
...
// 	auto regularize_vel = [&](openvdb::Vec3fTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		float max_v = 0; 		auto
// old_vel_leaf = m_velocity_after_p2g->tree().probeLeaf(leaf.origin());
// for (auto iter = leaf.beginValueOn(); iter; ++iter) {
// openvdb::Vec3f current_val = iter.getValue(); 			for (int
// ic = 0; ic < 3; ic++) { 				if (current_val[ic] >
// max_allowed_vel) { 					current_val[ic] =
// max_allowed_vel;
// 				}
...
// 		if (old_vel_leaf) {
// 			for (auto iter = old_vel_leaf->beginValueOn(); iter;
// ++iter)
// { 				openvdb::Vec3f current_val = iter.getValue();
// for (int ic = 0; ic < 3;
// ic++) { 					if (current_val[ic] >
// max_allowed_vel) { current_val[ic] = max_allowed_vel;
// 					}
// 					if (current_val[ic] < -max_allowed_vel)
// { 						current_val[ic] =
// -max_allowed_vel;
...
void FLIP_vdb::Advect(float dt, float dx,
  ...
  // openvdb::tools::StaggeredBoxSampler::sample(ovaxr, index_gpos);
  // auto updated_vel = openvdb::tools::StaggeredBoxSampler::sample(vaxr,
  // index_gpos); 		auto old_pvel = velocityHandle.get(*iter);
  // 		velocityHandle.set(*iter, (pic_component)*updated_vel + (1.0f -
  // pic_component) * (updated_vel - original_vel + old_pvel));
  // 	}
  ...
  // auto particle_man =
  // openvdb::tree::LeafManager<openvdb::points::PointDataTree>(particles->tree());

  // openvdb::io::File(std::to_string(m_framenumber)+"vn.vdb").write({
  // voxel_center_solid_normal, voxel_center_solid_vn });

  // advect them in the velocity field
  // openvdb::points::advectPoints(*m_particles, *m_velocity, /*RK order*/ 1,
  // /*time step*/dt, /*number of steps*/1); std::cout << " particle mem " <<
  // m_particles->memUsage() << std::endl;
...
// {
// 	using point_leaf_t = openvdb::points::PointDataTree::LeafNodeType;
// 	//auto narrow_leaves = tbb::concurrent_hash_map<openvdb::Coord,
// point_leaf_t*>();
...
// 	auto keep_one_layer_mask = [&](openvdb::FloatTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		auto point_axr{
// m_particles->getConstAccessor() }; 		auto pointleaf =
// m_particles->tree().probeConstLeaf(leaf.origin()); 		auto maskaxr{
// point_voxel_mask->getAccessor() };
...
// { 					for (int kk = -1; kk < 2 &&
// all_in_liquid; kk++) { all_in_liquid &=
...
// 			/*if (itercoord[0] <
// m_domain_index_begin[0]||itercoord[0]>=m_domain_index_end[0]||
// itercoord[1] < m_domain_index_begin[1] || itercoord[1] >=
// m_domain_index_end[1] || 				itercoord[2] <
// m_domain_index_begin[2] || itercoord[2] >= m_domain_index_end[2]) {
// iter.setValueOff();
// 			}*/
// 		}
// 	};
// 	auto maskman =
// openvdb::tree::LeafManager<openvdb::FloatTree>(point_voxel_mask->tree());
// 	maskman.foreach(keep_one_layer_mask);
// 	printf("keep_one_layer_mask_done\n");
// 	auto smaller_mask = point_voxel_mask->deepCopy();
// 	auto small_maskman =
// openvdb::tree::LeafManager<openvdb::FloatTree>(smaller_mask->tree());

// 	int nlayer = 2;
// 	//dilate the mask
// 	openvdb::tools::dilateVoxels(maskman, nlayer,
// openvdb::tools::NearestNeighbors::NN_FACE_EDGE_VERTEX);
// 	openvdb::tools::dilateVoxels(small_maskman, nlayer - 1,
// openvdb::tools::NearestNeighbors::NN_FACE_EDGE_VERTEX);
// 	printf("dilate_voxel_mask_done\n");
// 	//use the mask to shrink the sdf so that it only shows the interior
// liquid
// 	//to be used by the shrinked sdf manager
// 	auto shrink_sdf_op = [&](openvdb::FloatTree::LeafNodeType& leaf,
// openvdb::Index leafpos) { 		auto smaller_mask_axr{
// smaller_mask->getConstAccessor() }; 		auto smaller_mask_leaf=
// smaller_mask->tree().probeConstLeaf(leaf.origin()) ;
...
// 		for (auto iter = smaller_mask_leaf->beginValueOn(); iter;
// ++iter) { 			if (leaf.isValueMaskOn(iter.offset())) {
...
// 	auto sdfman =
// openvdb::tree::LeafManager<openvdb::FloatTree>(m_shrinked_liquid_sdf->tree());
// 	sdfman.foreach(shrink_sdf_op);
// 	printf("shrink_sdf_done\n");
// 	openvdb::tools::pruneLevelSet(m_shrinked_liquid_sdf->tree(), 0.5f *
// m_dx, -0.5f * m_dx);
...
// 		auto maskleaf =
// point_voxel_mask->tree().probeConstLeaf(leaf.origin());
...
// 		auto phandle = openvdb::points::AttributeHandle<openvdb::Vec3f,
// PositionCodec>(positionarray);

// 		auto& velarray = leaf.attributeArray("v");
// 		auto vhandle = openvdb::points::AttributeHandle<openvdb::Vec3f,
// VelocityCodec>(velarray); 		for (auto iter = leaf.cbeginValueAll();
// iter;
// ++iter) { 			if (!iter.isValueOn()) {
...
// 		auto p_write_handle =
// openvdb::points::AttributeWriteHandle<openvdb::Vec3f,
// PositionCodec>(new_positionarray);

// 		auto& new_velarray = leaf.attributeArray("v");
// 		auto v_write_handle =
// openvdb::points::AttributeWriteHandle<openvdb::Vec3f,
// VelocityCodec>(new_velarray);
...
// 	auto resultman =
// openvdb::tree::LeafManager<openvdb::points::PointDataTree>(result->tree());
...
// 			delete
// result->tree().stealNode<point_leaf_t>(iter->origin(), 0, false);
// 		}
// 	}
