...
struct ZSPartitionForZSParticles : INode {
    void apply() override {
        using namespace zs;
        fmt::print(fg(fmt::color::green), "begin executing ZSPartitionForZSParticles\n");
        ...
        if (!table->requestRebuild && cached && table->hasTags()) {
            ...
            cudaPol(range(table->numBoundaryEntries()), // table->getTags(),
                    [tags = proxy<execspace_e::cuda>(table->getTags()),
                     flag = proxy<execspace_e::cuda>(bRebuild)] __device__(auto i) mutable {
                        auto tag = tags[i];
                        if (tag == 1 && flag[0] == 0) {
            ...
            if (bRebuild.getVal() == 0) {
                table->rebuilt = false;
                fmt::print(fg(fmt::color::cyan), "done executing ZSPartitionForZSParticles (skipping full "
                                                 "rebuild)\n");
        ...
        for (auto &&parObjPtr : parObjPtrs) {
            if (parObjPtr->category != ZenoParticles::bending)
                cnt += (std::size_t)std::ceil(parObjPtr->getParticles().size() / get_input2<float>("ppb"));
            if (parObjPtr->isMeshPrimitive())
                cnt += (std::size_t)std::ceil(parObjPtr->numElements() / get_input2<float>("ppb"));
        }
        ...
        for (auto &&parObjPtr : parObjPtrs) {
            ...
            if (parObjPtr->isMeshPrimitive()) { // including tracker, but not bending
                ...
                cudaPol(range(eles.size()),
                        [eles = proxy<execspace_e::cuda>({}, eles), table = proxy<execspace_e::cuda>(partition),
        ...
        fmt::print(fg(fmt::color::cyan), "done executing ZSPartitionForZSParticles\n");
        set_output("ZSPartition", table);
    }
};
...
struct ExpandZSPartition : INode {
    ...
    template <typename VecT, zs::enable_if_t<std::is_integral_v<typename VecT::value_type>> = 0>
    ...
    void registerNewBlockEntries(zs::CudaExecutionPolicy &policy, typename ZenoPartition::table_t &table,
                                 zs::Vector<zs::i32> &dirTags, std::size_t offset, std::size_t numNewEntries) const {
        using namespace zs;
        policy(range(numNewEntries), [table = proxy<execspace_e::cuda>(table),
                                      dirTags = proxy<execspace_e::cuda>(dirTags), offset] __device__(int bi) mutable {
            ...
            for (auto ijk : ndrange<3>(3)) {
                ...
                if (auto neighborNo = table.query(bcoord + dir * (int)grid_traits<grid_t>::side_length);
                    ...
                    atomic_or(exec_cuda, &dirTags[neighborNo], (i32)(1 << getDirIndex(-dir)));
            }
        });
    }
    void apply() override {
        ...
        fmt::print("expect {} iterations to complete partition expansion of {} entries.\n", niters, prevCnt);
        ...
        for (int iter = 0; iter != niters; ++iter) {
            cudaPol(range(prevCnt), [dirs = proxy<execspace_e::cuda>(dirs),
                                     table = proxy<execspace_e::cuda>(partition)] __device__(auto bi) mutable {
                using table_t = RM_CVREF_T(table);
            ...
            registerNewBlockEntries(cudaPol, partition, dirs, prevCnt, curCnt - prevCnt);
        ...
        if (table->hasTags()) {
            ...
            cudaPol(Collapse{prevCnt},
                    [dirs = proxy<execspace_e::cuda>(dirs), marks = proxy<execspace_e::cuda>(marks)] __device__(
                        Ti bi) mutable { marks[bi] = dirs[bi] != (i32)27; });
            exclusive_scan(cudaPol, std::begin(marks), std::end(marks), std::begin(offsets));
            auto bouCnt = offsets.getVal(prevCnt);

            auto &boundaryIndices = table->getBoundaryIndices();
            boundaryIndices.resize(bouCnt);
            cudaPol(range(prevCnt), [marks = proxy<execspace_e::cuda>(marks),
                                     boundaryIndices = proxy<execspace_e::cuda>(boundaryIndices),
                                     offsets = proxy<execspace_e::cuda>(offsets)] __device__(Ti bi) mutable {
                if (marks[bi])
...
struct ZSGridFromZSPartition : INode {
    void apply() override {
        fmt::print(fg(fmt::color::green), "begin executing ZSGridFromZSPartition\n");
...
struct SpringSystemTimeStepping : INode {
    ...
    void filter(zs::CudaExecutionPolicy &cudaPol, tiles_t &vertData, const zs::SmallString tagSrc,
                const zs::SmallString tagDst) {
        using namespace zs;
        constexpr auto space = execspace_e::cuda;
        cudaPol(range(vertData.size()), [data = proxy<space>({}, vertData), tagSrc, tagDst] __device__(int pi) mutable {
            auto v = vec3::zeros();
            if (data("x0", 1, pi) < 0.5)
    ...
    float dot(zs::CudaExecutionPolicy &cudaPol, tiles_t &vertData, const zs::SmallString tag0,
        ...
        cudaPol(range(vertData.size()),
                [data = proxy<space>({}, vertData), res = proxy<space>(res), tag0, tag1] __device__(int pi) mutable {
    ...
    float evalEps(zs::CudaExecutionPolicy &cudaPol, tiles_t &vertData, float dt) {
        ...
        res.setVal(0);
        cudaPol(range(vertData.size()),
                [data = proxy<space>({}, vertData), res = proxy<space>(res)] __device__(int pi) mutable {
        ...
        res.setVal(0);
        cudaPol(range(vertData.size()),
                [data = proxy<space>({}, vertData), res = proxy<space>(res), dt] __device__(int pi) mutable {
    ...
    float computeSpringEnergy(zs::CudaExecutionPolicy &cudaPol, const tiles_t &springs, tiles_t &vertData, float alpha,
                              float dt) {
        using namespace zs;
        constexpr auto space = execspace_e::cuda;
        Vector<double> Esum{vertData.get_allocator(), 1};
        Esum.setVal(0);
        cudaPol(range(springs.size()), [Esum = proxy<space>(Esum), eles = proxy<space>({}, springs),
                                        data = proxy<space>({}, vertData), dt, alpha] __device__(int ei) mutable {
            auto inds = eles.pack<2>("inds", ei).reinterpret_bits<int>();
            auto v0 = data.pack<3>("v", inds[0]);
            auto dv0 = data.pack<3>("dv", inds[0]) + data.pack<3>("p", inds[0]) * alpha;
            v0 += dv0;
            auto x0 = data.pack<3>("x0", inds[0]) + v0 * dt;

            auto v1 = data.pack<3>("v", inds[1]);
            auto dv1 = data.pack<3>("dv", inds[1]) + data.pack<3>("p", inds[1]) * alpha;
            v1 += dv1;
    ...
    void computeSpringForce(zs::CudaExecutionPolicy &cudaPol, const tiles_t &springs, tiles_t &vertData,
                            const zs::SmallString vtag, const zs::SmallString ftag, float dt, float eps,
                            bool clear = false) {
        using namespace zs;
        constexpr auto space = execspace_e::cuda;
        if (clear)
            cudaPol(range(vertData.size()), [data = proxy<space>({}, vertData), ftag] __device__(int pi) mutable {
                data.tuple<3>(ftag, pi) = vec3::zeros();
            });
        cudaPol(range(springs.size()), [eles = proxy<space>({}, springs), data = proxy<space>({}, vertData), vtag, ftag,
                                        eps, dt] __device__(int ei) mutable {
            auto inds = eles.pack<2>("inds", ei).reinterpret_bits<int>();
  ...
  void computeCollisionForce(zs::CudaExecutionPolicy &cudaPol,
...
#endif
    void SAp(zs::CudaExecutionPolicy &cudaPol, const tiles_t &verts, const tiles_t &springs, tiles_t &vertData,
             float dt, float eps) {
        using namespace zs;
        constexpr auto space = execspace_e::cuda;
        computeSpringForce(cudaPol, springs, vertData, "p", "f", dt, eps, true);
        // computeCollisionForce(cudaPol, springs, vertData, "p", "f", dt, eps);
        cudaPol(range(vertData.size()), [verts = proxy<space>({}, verts), data = proxy<space>({}, vertData), dt,
                                         eps] __device__(int pi) mutable {
            auto m = verts("m", pi);
    ...
    void apply() override {
        using namespace zs;
        fmt::print(fg(fmt::color::green), "begin executing SpringSystemTimeStepping\n");
        vec3 accel{vec3::zeros()};
        if (has_input("Accel"))
            accel = vec3::from_array(get_input<NumericObject>("Accel")->get<zeno::vec3f>());
        ...
        tiles_t vertData{verts.get_allocator(),
                         {{"x0", 3}, {"dv", 3}, {"v", 3}, {"f", 3}, {"f0", 3}, {"b", 3}, {"r", 3}, {"s", 3}, {"p", 3}},
                         verts.size()};
        ...
        // clear dv
        cudaPol(range(vertData.size()), [data = proxy<space>({}, vertData)] __device__(int pi) mutable {
            data.tuple<3>("dv", pi) = vec3::zeros();
        });
        // f_initial
        cudaPol(range(numVerts),
                [verts = proxy<space>({}, verts), data = proxy<space>({}, vertData)] __device__(int pi) mutable {
        ...
        // compute b
        cudaPol(range(numVerts), [verts = proxy<space>({}, verts), data = proxy<space>({}, vertData), accel, dt,
                                  eps] __device__(int pi) mutable {
            auto m = verts("m", pi);
            auto f0 = data.pack<3>("f0", pi);
            auto f = data.pack<3>("f", pi);
        ...
        while (deltaNew > tol * delta0 * tol && deltaNew > limits<float>::epsilon() * 8 && iter != 100) {
            // s = S(Ap)
      ...
      for (; E > Eprev;) {
        ...
        E = computeSpringEnergy(cudaPol, springs, vertData, alpha, dt);
            ...
            cudaPol(range(numVerts), [data = proxy<space>({}, vertData), alpha] __device__(int pi) mutable {
                data.tuple<3>("dv", pi) = data.pack<3>("dv", pi) + alpha * data.pack<3>("p", pi);
                data.tuple<3>("r", pi) = data.pack<3>("r", pi) - alpha * data.pack<3>("s", pi);
            });
            deltaOld = deltaNew;
            ...
            //
            cudaPol(range(numVerts),
                    [data = proxy<space>({}, vertData), deltaNew, deltaOld] __device__(int pi) mutable {
                        data.tuple<3>("p", pi) = data.pack<3>("r", pi) + (deltaNew / deltaOld) * data.pack<3>("p", pi);
                    });
        ...
        cudaPol(range(numVerts),
                [verts = proxy<space>({}, verts), data = proxy<space>({}, vertData), dt] __device__(int pi) mutable {
        ...
        cudaPol(range(numVerts), [verts = proxy<space>({}, verts), data = proxy<space>({}, vertData), accel,
        ...
        fmt::print(fg(fmt::color::cyan), "done executing SpringSystemTimeStepping\n");
        // getchar();
...
struct UpdateZSGrid : INode {
    void apply() override {
        ...
        if (zsgrid->isPicStyle())
                    ...
                    [grid = proxy<execspace_e::cuda>({}, grid), stepDt, accel,
                     ptr = velSqr.data()] __device__(auto bi, auto ci) mutable {
                        ...
                        if (mass != 0.f) {
                            ...
                            auto vstar = block.pack<3>("vstar", ci) * mass + vel + accel * stepDt;
            ...
            cudaPol(Collapse{partition.size(), ZenoGrid::grid_t::block_space()},
                    [grid = proxy<execspace_e::cuda>({}, grid)] __device__(auto bi, auto ci) mutable {
                        auto block = grid.block(bi);
                        auto mass = block("m", ci);
                        if (mass != 0.f) {
                            {
...
struct ZSReturnMapping : INode {
    ...
    void returnMapping(zs::CudaExecutionPolicy &cudaPol, typename ZenoParticles::particles_t &pars,
                       const zs::StvkWithHencky<float> &elasticModel, const PM &plasticModel) const {
        using namespace zs;
        cudaPol(range(pars.size()), [pars = proxy<execspace_e::cuda>({}, pars), elasticModel = elasticModel,
                                     plasticModel = plasticModel] __device__(size_t pi) mutable {
            auto FeHat = pars.pack<3, 3>("F", pi);
            if constexpr (is_same_v<zs::NonAssociativeCamClay<float>, RM_CVREF_T(plasticModel)>) {
    ...
    void return_mapping_surface(zs::CudaExecutionPolicy &cudaPol, typename ZenoParticles::particles_t &eles,
                                const float gamma, const float k, const float fc) const {
        using namespace zs;
        cudaPol(range(eles.size()),
                [eles = proxy<execspace_e::cuda>({}, eles), gamma, k, fc] __device__(size_t pi) mutable {
    ...
    void return_mapping_curve(zs::CudaExecutionPolicy &cudaPol, const zs::StvkWithHencky<float> &stvkModel,
                              typename ZenoParticles::particles_t &eles, const float gamma, const float fa) const {
        using namespace zs;
        ...
        cudaPol(range(eles.size()), [eles = proxy<execspace_e::cuda>({}, eles), stvkModel = stvkModel,
                                     materialParamOverride, gamma, fa] __device__(size_t pi) mutable {
            // hard code ftm
            ...
            if (eps_trace < 0) {
                ...
                auto dgp = eps_hat_norm + (stvkModel.mu + stvkModel.lam) / stvkModel.mu * eps_trace * alpha;
        ...
        for (auto &&parObjPtr : parObjPtrs) {
            ...
            if (parObjPtr->category == ZenoParticles::mpm) {
                if (parObjPtr->getModel().hasPlasticity()) {
                    match(
                        [this, &cudaPol, &pars](const zs::StvkWithHencky<float> &elasticModel, const auto &plasticModel)
                            -> std::enable_if_t<!is_same_v<RM_CVREF_T(plasticModel), std::monostate>> {
                            returnMapping(cudaPol, pars, elasticModel, plasticModel);
                        },
                        [](...) { throw std::runtime_error("unsupported elasto-plasticity models"); })(
                        parObjPtr->getModel().getElasticModel(), parObjPtr->getModel().getPlasticModel());
                }
                ...
                if (parObjPtr->category == ZenoParticles::surface)
                    return_mapping_surface(cudaPol, eles, models.retrieve("gamma"), models.retrieve("k"),
                                           std::tan(models.retrieve("fa") / g_pi));
                else if (parObjPtr->category == ZenoParticles::curve) {
                    match(
                        [this, &eles, &cudaPol, &models](const StvkWithHencky<float> &stvkModel) {
                            // use drucker prager plasticity for friction handling
                            return_mapping_curve(cudaPol, stvkModel, eles, models.retrieve("gamma"),
                                                 models.retrieve("fa"));
                        },
                        [](...) {
                            // do nothing
