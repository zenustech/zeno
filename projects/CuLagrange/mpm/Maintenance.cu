    ...
    void apply() override {
        ...
        for (auto &&parObjPtr : parObjPtrs) {
            if (parObjPtr->category != ZenoParticles::mpm)
                continue;
            auto &pars = parObjPtr->getParticles();
            cudaPol(range(pars.size()), [pars = proxy<execspace_e::cuda>({}, pars), ibs = proxy<execspace_e::cuda>(ibs),
                                         density = parObjPtr->getModel().density,
                                         cellVol = grid.dx * grid.dx * grid.dx] __device__(size_t pi) mutable {
                auto pos = pars.template pack<3>("x", pi);
...
struct PushOutZSParticles : INode {
    ...
    void pushout(zs::CudaExecutionPolicy &cudaPol, typename ZenoParticles::particles_t &pars, LsView lsv, float dis) {
        using namespace zs;
        cudaPol(range(pars.size()), [pars = proxy<execspace_e::cuda>({}, pars), lsv,
                                     eps = limits<float>::epsilon() * 128, dis] __device__(size_t pi) mutable {
            auto x = pars.pack<3>("x", pi);
            bool updated = false;
            int cnt = 5;
#if 0
      if (pi < 10) {
...
#endif
            for (auto sd = lsv.getSignedDistance(x), sdabs = zs::abs(sd); sd < dis && cnt--;) {
                auto diff = x.zeros();
                for (int i = 0; i != 3; i++) {
                    auto v1 = x;
                    auto v2 = x;
                    v1[i] = x[i] + eps;
                    v2[i] = x[i] - eps;
                    diff[i] = (lsv.getSignedDistance(v1) - lsv.getSignedDistance(v2)) / (eps + eps);
                }
        ...
        for (auto &&parObjPtr : parObjPtrs) {
            ...
            match(
                ...
                [&](const_sdf_vel_ls_t &ls) {
                    match([&](auto lsv) { pushout(cudaPol, pars, SdfVelFieldView{lsv}, dis); })(
          ...
          cudaPol(range(prevEleCnt),
                  [eles = proxy<execspace_e::cuda>({}, eles),
                   ...
                   dx] __device__(int ei) mutable {
                    ...
                    zs::vec<float, 3> xs[3]{pars.pack<3>("x", inds[0]),
                                            pars.pack<3>("x", inds[1]),
                                            pars.pack<3>("x", inds[2])};
                    auto area =
                    ...
                    if (auto vertId = vertOffsets[ei]; vertId >= 0) {
                      ...
                      for (int i = 0; i != 3; ++i) {
                        atomic_add(exec_cuda, &pars("vol", inds[i]),
                                   -vole_div_3);
                        atomic_add(exec_cuda, &pars("m", inds[i]),
                                   -vole_div_3 * rho);
                      }

                      /// spawn new vertex
                      pars("vol", vertId) = vole_div_3;
                      pars("m", vertId) = vole_div_3 * rho;
                      pars.tuple<3>("x", vertId) = c;
                      pars.tuple<3>("v", vertId) = vele;
...
#define CONSTRUCT_ELEMENT(e, ia, ib, a, b)                                        \
    {                                                                             \
        ...
        eles("m", e) = vole * rho;                                                \
        eles("vol", e) = vole;                                                    \
        eles.tuple<3>("x", e) = (a + b + c) / 3.f;                                \
        eles.tuple<3>("v", e) = vele;                                             \
        eles.tuple<9>("C", e) = Ce;                                               \
        eles.tuple<9>("F", e) = Fe;                                               \
        mat3 d{};                                                                 \
        auto d0 = b - a;                                                          \
        ...
        atomic_add(exec_cuda, &pars("m", ia), vole *rho / 3.f);                   \
        atomic_add(exec_cuda, &pars("vol", ib), vole / 3.f);                      \
        atomic_add(exec_cuda, &pars("m", ib), vole *rho / 3.f);                   \
    }
...
struct RefineMeshParticles : INode {
    void apply() override {
        ...
        for (auto &&parObjPtr : parObjPtrs) {
            if (parObjPtr->prim.get() == nullptr || parObjPtr->category == ZenoParticles::mpm)
                continue;
            ...
            if (parObjPtr->category == ZenoParticles::surface) {
                ...
                auto probeSize = [&]() { // side effects: (prev) vert/eleCnt
                    ...
                    cudaPol(Collapse{prevEleCnt},
                            [eles = proxy<execspace_e::cuda>({}, eles),
                             table = proxy<execspace_e::cuda>(edgeTable)] __device__(int ei) mutable {
                                int inds[3] = {reinterpret_bits<int>(eles("inds", 0, ei)),
                                               reinterpret_bits<int>(eles("inds", 1, ei)),
                                               reinterpret_bits<int>(eles("inds", 2, ei))};
                                for (int e = 0; e != 3; ++e) {
                    ...
                    zs::memset(mem_device, edgeLocalNosInEle.data(), -1, sizeof(char2) * numEdges);
                    cudaPol(Collapse{prevEleCnt},
                            [eles = proxy<execspace_e::cuda>({}, eles), edgeEles = proxy<execspace_e::cuda>(edgeEles),
                    ...
                    zs::memset(mem_device, eleSplitVertNos.data(), -1, sizeof(int3) * prevEleCnt);
                    cudaPol(range(numEdges),
                            [edgeEles = proxy<execspace_e::cuda>(edgeEles),
                             ...
                             pars = proxy<execspace_e::cuda>({}, pars), vertCnt = proxy<execspace_e::cuda>(vertCnt),
                             eleCnt = proxy<execspace_e::cuda>(eleCnt), dx] __device__(int ei) mutable {
                                auto edge = edges[ei];
                                float3 xs[2] = {pars.pack<3>("x", edge[0]), pars.pack<3>("x", edge[1])};
                                if ((xs[1] - xs[0]).l2NormSqr() > dx * dx) {
                                    ...
                                    int nEleInc = (eleNos[0] != -1 ? 1 : 0) + (eleNos[1] != -1 ? 1 : 0);
                    ...
                    pars.resize(vertCnt.getVal());
                    eles.resize(eleCnt.getVal());
                    fmt::print("verts from {} to {}, {} added\n", prevVertCnt, vertCnt.getVal(),
                               vertCnt.getVal() - prevVertCnt);
                    fmt::print("eles from {} to {}, {} added\n", prevEleCnt, eleCnt.getVal(),
                               eleCnt.getVal() - prevEleCnt);
                    fmt::print("{} edges in total\n", numEdges);
                    /// init additional verts
                    int vertOffset = vertCnt.getVal();
                    int eleOffset = eleCnt.getVal();
                    cudaPol(range(numEdges), [edgeEles = proxy<execspace_e::cuda>(edgeEles),
                                              edgeLocalNosInEle = proxy<execspace_e::cuda>(edgeLocalNosInEle),
                                              eleSplitVertNos = proxy<execspace_e::cuda>(eleSplitVertNos),
                                              edges = proxy<execspace_e::cuda>(edgeTable._activeKeys),
                                              pars = proxy<execspace_e::cuda>({}, pars),
                        ...
                        pars("vol", vi) = 0.f;
                        // pos, vel, F, C
                        pars.tuple<3>("x", vi) = (xs[0] + xs[1]) * 0.5f;
                        pars.tuple<3>("v", vi) = (pars.pack<3>("v", edge[0]) + pars.pack<3>("v", edge[1])) * 0.5f;
                        // F, C
                    ...
                    if (curVertCnt = vertCnt.getVal(); prevVertCnt != curVertCnt) {
                        ...
                        if (hasNormalProperty) {
                            ...
                            cudaPol(range(curVertCnt), [pars = proxy<execspace_e::cuda>({}, pars)] __device__(int pi) {
                ...
                while (probeSize()) {
                    eleCnt.setVal(prevEleCnt);
                    cudaPol(range(prevEleCnt),
                            [eles = proxy<execspace_e::cuda>({}, eles), pars = proxy<execspace_e::cuda>({}, pars),
                             eleSplitVertNos = proxy<execspace_e::cuda>(eleSplitVertNos),
                             ...
                             rho = model.density] __device__(int ei) mutable {
                                ...
                                int inds[3] = {reinterpret_bits<int>(eles("inds", 0, ei)),
                                               reinterpret_bits<int>(eles("inds", 1, ei)),
                                               reinterpret_bits<int>(eles("inds", 2, ei))};
                                zs::vec<float, 3> xs[3]{pars.pack<3>("x", inds[0]), pars.pack<3>("x", inds[1]),
                                                        pars.pack<3>("x", inds[2])};
                                // vol, vel, C, F, d
...

                                {
                                    ...
                                    for (int i = 0; i != 3; ++i) {
                                        ...
                                        atomic_add(exec_cuda, &pars("m", inds[i]), -vole_div_3 * rho);
                                    }
                                }

                                using mat3 = zs::vec<double, 3, 3>;
                                auto constructElement = [&](int e, int3 is, const auto &p0, const auto &p1,
                                                            const auto &p2, float vole) {
                                    eles("m", e) = vole * rho;
                                    eles("vol", e) = vole;
                                    eles.tuple<3>("x", e) = (p0 + p1 + p2) / 3.f;
                                    eles.tuple<3>("v", e) = vele;
                                    eles.tuple<9>("C", e) = Ce;
                                    eles.tuple<9>("F", e) = Fe;
                                    if (hasNormalProperty)
                                    ...
                                    for (int i = 0; i != 3; ++i) {
                                        ...
                                        atomic_add(exec_cuda, &pars("m", is[i]), mass_div3);
                                    }
                                    if (hasNormalProperty)
                                        for (int i = 0; i != 3; ++i)
                                            for (int j = 0; j != 3; ++j)
                                ...
                                if (numSplits == 1) {
                                    if (int d = splitVerts[0]; d != -1) { // [d, -1, -1]
                                        auto pd = pars.pack<3>("x", d);
                                        constructElement(ei, {inds[2], inds[0], d}, xs[2], xs[0], pd, vol * 0.5f);
                                        constructElement(eleId, {inds[1], inds[2], d}, xs[1], xs[2], pd, vol * 0.5f);
                                    } else if (int d = splitVerts[1]; d != -1) { // [-1, d, -1]
                                        auto pd = pars.pack<3>("x", d);
                                        constructElement(ei, {inds[0], inds[1], d}, xs[0], xs[1], pd, vol * 0.5f);
                                        constructElement(eleId, {inds[2], inds[0], d}, xs[2], xs[0], pd, vol * 0.5f);
                                    ...
                                    if (splitVerts[0] == -1) { // [-1, d, e]
                                        ...
                                        auto e = splitVerts[2];
                                        auto pd = pars.pack<3>("x", d);
                                        auto pe = pars.pack<3>("x", e);
                                        constructElement(ei, {inds[0], inds[1], d}, xs[0], xs[1], pd, vol * 0.5f);
                                        constructElement(eleId, {inds[2], e, d}, xs[2], pe, pd, vol * 0.25f);
                                        constructElement(eleId + 1, {inds[0], d, e}, xs[0], pd, pe, vol * 0.25f);
                                    } else if (splitVerts[1] == -1) { // [d, -1, e]
                                        auto d = splitVerts[0];
                                        auto e = splitVerts[2];
                                        auto pd = pars.pack<3>("x", d);
                                        auto pe = pars.pack<3>("x", e);
                                        constructElement(ei, {inds[1], inds[2], d}, xs[1], xs[2], pd, vol * 0.5f);
                                        constructElement(eleId, {inds[0], d, e}, xs[0], pd, pe, vol * 0.25f);
                                        constructElement(eleId + 1, {inds[2], e, d}, xs[2], pe, pd, vol * 0.25f);
                                        ...
                                        auto pe = pars.pack<3>("x", e);
                                        constructElement(ei, {inds[2], inds[0], d}, xs[2], xs[0], pd, vol * 0.5f);
                                        constructElement(eleId, {inds[1], e, d}, xs[1], pe, pd, vol * 0.25f);
                                        constructElement(eleId + 1, {inds[2], d, e}, xs[2], pd, pe, vol * 0.25f);
                                    }
                                } else {
                                    auto d = splitVerts[0];
                                    auto e = splitVerts[1];
                                    ...
                                    auto pd = pars.pack<3>("x", d);
                                    auto pe = pars.pack<3>("x", e);
                                    auto pf = pars.pack<3>("x", f);
                                    constructElement(ei, {inds[0], d, f}, xs[0], pd, pf, vol * 0.25f);
                                    constructElement(eleId, {inds[1], e, d}, xs[1], pe, pd, vol * 0.25f);
                                    constructElement(eleId + 1, {inds[2], f, e}, xs[2], pf, pe, vol * 0.25f);
                                    constructElement(eleId + 2, {d, e, f}, pd, pe, pf, vol * 0.25f);
                                }
                            });
                    if (hasNormalProperty)
...
struct UpdateZSPrimitiveSequence : INode {
    void apply() override {
        using namespace zs;
        fmt::print(fg(fmt::color::green), "begin executing UpdateZSPrimitiveSequence\n");
        ...
        cudaPol(Collapse{numV},
                [pars = proxy<execspace_e::cuda>({}, zsprimseq->getParticles()), dt] __device__(int pi) mutable {
                    pars.tuple<3>("nrm", pi) = zs::vec<float, 3>::zeros();
                    pars.tuple<3>("x", pi) = pars.pack<3>("x", pi) + pars.pack<3>("v", pi) * dt;
                });
        cudaPol(Collapse{numE},
                [eles = proxy<execspace_e::cuda>({}, zsprimseq->getQuadraturePoints()), dt] __device__(int ei) mutable {
                    eles.tuple<3>("x", ei) = eles.pack<3>("x", ei) + eles.pack<3>("v", ei) * dt;
                });
        // normal
        cudaPol(Collapse{numE}, [pars = proxy<execspace_e::cuda>({}, zsprimseq->getParticles()),
                                 eles = proxy<execspace_e::cuda>({}, zsprimseq->getQuadraturePoints()),
                                 dt] __device__(int ei) mutable {
            int inds[3] = {reinterpret_bits<int>(eles("inds", 0, ei)), reinterpret_bits<int>(eles("inds", 1, ei)),
                           reinterpret_bits<int>(eles("inds", 2, ei))};
            zs::vec<float, 3> xs[3]{pars.pack<3>("x", inds[0]), pars.pack<3>("x", inds[1]), pars.pack<3>("x", inds[2])};
        ...
        cudaPol(Collapse{size},
                [pars = proxy<execspace_e::cuda>({}, zsprimseq->getParticles())] __device__(int pi) mutable {
                    pars.tuple<3>("nrm", pi) = pars.pack<3>("nrm", pi).normalized();
                });
        if (sprayedSize != 0)
            cudaPol(Collapse{sprayedSize}, [pars = proxy<execspace_e::cuda>({}, zsprimseq->getParticles()),
                                            eles = proxy<execspace_e::cuda>({}, zsprimseq->getQuadraturePoints()),
        ...
        fmt::print(fg(fmt::color::cyan), "done executing UpdateZSPrimitiveSequence\n");
