...
struct ApplyGridBoundaryOnZSGrid : INode {
    void apply() override {
        fmt::print(fg(fmt::color::green), "begin executing ApplyGridBoundaryOnZSGrid\n");
        ...
        auto &boundaryPartition = get_input<ZenoPartition>("BoundaryZSPartition")->get();
        ...
        cudaPol(Collapse{boundaryPartition.size(), boundaryGrid.block_size},
                [grid = proxy<execspace_e::cuda>({}, grid), boundaryGrid = proxy<execspace_e::cuda>({}, boundaryGrid),
        ...
        fmt::print(fg(fmt::color::cyan), "done executing ApplyGridBoundaryOnZSGrid\n");
        set_output("ZSGrid", zsgrid);
    }
};

ZENDEFNODE(ApplyGridBoundaryOnZSGrid, {
                                          {"ZSPartition", "ZSGrid", "BoundaryZSPartition", "BoundaryZSGrid"},
                                          {"ZSGrid"},
...
struct ApplyBoundaryOnZSGrid : INode {
    ...
    constexpr void projectBoundary(zs::CudaExecutionPolicy &cudaPol, LsView lsv, const ZenoBoundary &boundary,
        ...
        if (isFlipStyle)
            cudaPol(Collapse{partition.size(), ZenoGrid::grid_t::block_space()},
                    [grid = proxy<execspace_e::cuda>({}, grid), table = proxy<execspace_e::cuda>(partition),
                     boundary = collider] __device__(int bi, int ci) mutable {
                        auto block = grid.block(bi);
                        auto mass = block("m", ci);
                        if (mass != 0.f) {
                            auto pos = (table._activeKeys[bi] + grid.cellid_to_coord(ci)) * grid.dx;
    ...
    void apply() override {
        fmt::print(fg(fmt::color::green), "begin executing ApplyBoundaryOnZSGrid\n");
                ...
                match([&](const auto &ls) {
                    if constexpr (is_same_v<RM_CVREF_T(ls), basic_ls_t>) {
                        match([&](const auto &lsPtr) {
                            ...
                            projectBoundary(cudaPol, lsv, *boundary, partition, grid, zsgrid->isFlipStyle());
                        })(ls._ls);
                    } else if constexpr (is_same_v<RM_CVREF_T(ls), const_sdf_vel_ls_t>) {
                        match([&](auto lsv) {
                            projectBoundary(cudaPol, SdfVelFieldView{lsv}, *boundary, partition, grid,
                                            zsgrid->isFlipStyle());
                        ...
                        match([&](auto fieldPair) {
                            ...
                            projectBoundary(cudaPol,
                                            ...
                                            *boundary, partition, grid, zsgrid->isFlipStyle());
...
struct ComputeParticleBeta : INode {
    void determine_beta(zs::CudaExecutionPolicy &cudaPol, typename ZenoParticles::particles_t &pars,
                        const typename ZenoPartition::table_t &partition, const float dt,
                        const typename ZenoGrid::grid_t &grid, float JpcDefault = 1.f) {
        using namespace zs;
        cudaPol(range(pars.size()),
                [pars = proxy<execspace_e::cuda>({}, pars), table = proxy<execspace_e::cuda>(partition),
                 grid = proxy<execspace_e::cuda>({}, grid), dt, JpcDefault] __device__(size_t pi) mutable {
    ...
    void determine_beta(zs::CudaExecutionPolicy &cudaPol, typename ZenoParticles::particles_t &pars,
                        typename ZenoParticles::particles_t &eles, const typename ZenoPartition::table_t &partition,
                        const float dt, const typename ZenoGrid::grid_t &grid, float JpcDefault = 1.f) {
        using namespace zs;
        cudaPol(range(eles.size()),
                [pars = proxy<execspace_e::cuda>({}, pars), eles = proxy<execspace_e::cuda>({}, eles),
                 table = proxy<execspace_e::cuda>(partition), grid = proxy<execspace_e::cuda>({}, grid), dt,
    ...
    template <typename LsView>
    void determine_beta_sdf(zs::CudaExecutionPolicy &cudaPol, LsView lsv, const ZenoBoundary &boundary, const float dt,
                            typename ZenoParticles::particles_t &pars, float JpcDefault = 1.f) {
        using namespace zs;
        auto collider = boundary.getBoundary(lsv);
        cudaPol(range(pars.size()), [pars = proxy<execspace_e::cuda>({}, pars), boundary = collider, JpcDefault,
            ...
            if (boundary.levelset.getSignedDistance(X) < 0) {
                ...
                auto vel = boundary.R * boundary.levelset.getMaterialVelocity(X) + boundary.dbdt;
                if ((vp - vel).dot(nrm) < 0) {
                    pars("beta", pi) = 0.f;
                    return;
                }
    ...
    template <typename LsView>
    void determine_beta_sdf(zs::CudaExecutionPolicy &cudaPol, LsView lsv, const ZenoBoundary &boundary, const float dt,
                            typename ZenoParticles::particles_t &pars, typename ZenoParticles::particles_t &eles,
                            float JpcDefault = 1.f) {
        using namespace zs;
        auto collider = boundary.getBoundary(lsv);
        cudaPol(range(eles.size()),
                [pars = proxy<execspace_e::cuda>({}, pars), eles = proxy<execspace_e::cuda>({}, eles),
                 boundary = collider, JpcDefault, dt] __device__(size_t ei) mutable {
                    auto pos = eles.pack<3>("x", ei);
                    auto vp = eles.pack<3>("v", ei);
                    pos += dt * vp;
                    auto tri = eles.pack<3>("inds", ei).reinterpret_bits<int>();
    ...
    void apply() override {
        ...
        if (has_input<ZenoGrid>("ZSBoundary(Grid)")) {
            ...
            for (auto &&parObjPtr : parObjPtrs) {
                ...
                fmt::print("[ComputeParticleBeta] dx: {}, dt: {}, npars: {}\n", grid.dx, stepDt, pars.size());

                if (parObjPtr->category == ZenoParticles::mpm || parObjPtr->category == ZenoParticles::surface) {
                    bool isMeshPrimitive = parObjPtr->isMeshPrimitive();
                    match([&](auto &elasticModel, auto &anisoElasticModel) {
                        float JpcDefault = 1.f;
                        if constexpr (is_same_v<RM_CVREF_T(anisoElasticModel), NonAssociativeCamClay<float>>)
                            JpcDefault = 1.1f;
                        if (isMeshPrimitive)
                            determine_beta(cudaPol, pars, parObjPtr->getQuadraturePoints(), partition, stepDt, grid,
                                           JpcDefault);
                        else
                            ...
                            if constexpr (is_same_v<RM_CVREF_T(ls), basic_ls_t>) {
                                match([&](const auto &lsPtr) {
                                    ...
                                    if (isMeshPrimitive)
                                        ...
                                        determine_beta_sdf(cudaPol, lsv, *boundary, stepDt, pars, JpcDefault);
                                })(ls._ls);
                            } else if constexpr (is_same_v<RM_CVREF_T(ls), const_sdf_vel_ls_t>) {
                                match([&](auto lsv) {
                                    if (isMeshPrimitive)
                                        determine_beta_sdf(cudaPol, SdfVelFieldView{lsv}, *boundary, stepDt, pars,
                                                           parObjPtr->getQuadraturePoints(), JpcDefault);
                                    else
                                        determine_beta_sdf(cudaPol, SdfVelFieldView{lsv}, *boundary, stepDt, pars,
                                                           JpcDefault);
                                ...
                                match([&](auto fieldPair) {
                                    ...
                                    if (isMeshPrimitive)
                                        determine_beta_sdf(
                                            ...
                                            TransitionLevelSetView{SdfVelFieldView{fvSrc}, SdfVelFieldView{fvDst},
                                                                   ls._stepDt, ls._alpha},
                                            *boundary, stepDt, pars, parObjPtr->getQuadraturePoints(), JpcDefault);
                                    else
                                        determine_beta_sdf(cudaPol,
...
struct ApplyWindImpulseOnZSGrid : INode {
    ...
    void
    computeWindImpulse(zs::CudaExecutionPolicy &cudaPol, float windDragCoeff, float windDensity, VelSplsViewT velLs,
                       const typename ZenoParticles::particles_t &pars, const typename ZenoParticles::particles_t &eles,
                       const typename ZenoPartition::table_t &partition, typename ZenoGrid::grid_t &grid, float dt) {
        using namespace zs;
        cudaPol(range(eles.size()),
                [windDragCoeff, windDensity, velLs, pars = proxy<execspace_e::cuda>({}, pars), // for normal compute
                 eles = proxy<execspace_e::cuda>({}, eles), table = proxy<execspace_e::cuda>(partition),
                 grid = proxy<execspace_e::cuda>({}, grid), Dinv = 4.f / grid.dx / grid.dx,
                 dt] __device__(size_t ei) mutable {
                    using grid_t = RM_CVREF_T(grid);
                    zs::vec<float, 3> n{};
                    float area{};
                    {
                        auto inds = eles.pack<3>("inds", ei).template reinterpret_bits<int>();
                        auto p0 = pars.pack<3>("x", inds[0]);
                        auto p1 = pars.pack<3>("x", inds[1]);
                        auto p2 = pars.pack<3>("x", inds[2]);
                    ...
                    auto arena = make_local_arena<grid_e::collocated, kernel_e::quadratic>(grid.dx, pos);
    ...
    void apply() override {
        fmt::print(fg(fmt::color::green), "begin executing ApplyWindImpulseOnZSGrid\n");
        ...
        match([&](const auto &velLsPtr) {
            ...
            for (auto &&parObjPtr : parObjPtrs) {
                ...
                if (parObjPtr->category == ZenoParticles::surface || parObjPtr->category == ZenoParticles::tracker) {
                    auto &eles = parObjPtr->getQuadraturePoints();
                    computeWindImpulse(cudaPol, windDrag, windDensity, get_level_set_view<execspace_e::cuda>(velLsPtr),
                                       pars, eles, partition, grid, stepDt);
                }
            }
        })(velField);

        fmt::print(fg(fmt::color::cyan), "done executing ApplyWindImpulseOnZSGrid\n");
        set_output("ZSGrid", zsgrid);
    }
};
        ...
        if (has_input("translation")) {
            ...
            match(
                ...
                [](auto &ls) {
                    ...
                    throw std::runtime_error(fmt::format("levelset of special type [{}] are const-.", msg));
                })(ls);
        }
