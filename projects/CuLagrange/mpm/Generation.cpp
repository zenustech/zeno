...
#else
        auto sampled = zs::sample_from_levelset(gridPtr, dx, get_input2<float>("ppc"));
#endif
...
struct ZSPoissonDiskSample : INode {
    void apply() override {
        ...
        match([&prim, &sampled, dx, ppc, this](auto &ls) {
            ...
            } else if constexpr (is_same_v<LsT, typename ZenoLevelSet::const_transition_ls_t>) {
                match([&sampled = sampled, &ls, dx = dx, ppc = ppc](auto fieldPair) {
                    sampled = zs::sample_from_levelset(TransitionLevelSetView{SdfVelFieldView{get<0>(fieldPair)},
...
struct SurfaceToSprings : INode {
    // element
    std::shared_ptr<PrimitiveObject> build_springs(zs::OmpExecutionPolicy &ompPol, const PrimitiveObject &surf) {
        if (surf.tris.size() == 0)
            return {};
        using namespace zs;
        ...
        //
        ompPol(range(numE), [table = proxy<space>(edgeTable), &tris = surfEles](int ei) mutable {
            auto tri = tris[ei];
            auto vi = tri[2];
            for (int v = 0; v != 3; ++v) {
                auto vj = tri[v];
        ...
        ompPol(range(numE), [table = proxy<space>(edgeTable), &edgeToEles = edgeToEles, &tris = surfEles](int ei) {
            auto tri = tris[ei];
            auto vi = tri[2];
            for (int v = 0; v != 3; ++v) {
                auto vj = tri[v];
        ...
        ompPol(range(numE), [table = proxy<space>(edgeTable), eleTable = proxy<space>(eleTable), &edgeToEles, &cnt,
                             &elePairs, &tris = surfEles](int ei) mutable {
            ...
            for (int v = 0; v != 3; ++v) {
                ...
                if (vi > vj) { // check opposite
                    if (auto edgeNo = table.query(vec2i{vj, vi}); edgeNo != table_t::sentinel_v) {
        ...
        ompPol(range(numSpringVerts), [&springPos, &surfPars, &surfEles, eleTable = proxy<space>(eleTable)](int pi) {
        ...
        ompPol(range(numElePairs),
               [&springPos, &springLines, &surfPars, &surfEles, &elePairs, eleTable = proxy<space>(eleTable)](int ei) {
                   auto vinds = elePairs[ei];
                   springLines[ei] = zeno::vec2i{eleTable.query(vec1i{vinds[0]}), eleTable.query(vec1i{vinds[1]})};
               });
    ...
    void apply() override {
        ...
        if (has_input<PrimitiveObject>("prim")) {
            set_output("prim", build_springs(ompPol, *get_input<PrimitiveObject>("prim")));
            ...
            for (auto &&objSharedPtr : objSharedPtrLists.get()) {
                if (auto ptr = dynamic_cast<PrimitiveObject *>(objSharedPtr.get()); ptr != nullptr)
...
struct ToZSSprings : INode {
    void apply() override {
        ...
        const auto toTV3 = [](const zeno::vec3f &x) { return TV3{x[0], x[1], x[2]}; };
        const auto fromTV3 = [](const TV3 &x) { return zeno::vec3f{(float)x[0], (float)x[1], (float)x[2]}; };
        {
        ...
        fmt::print("{} springs in process. pending {} particles with these attributes.\n", eleSize, size);
        for (auto tag : tags)
            fmt::print("tag: [{}, {}]\n", tag.name, tag.numChannels);

        springsPrim->particles = std::make_shared<typename ZenoParticles::particles_t>(tags, size, memsrc_e::host);
        auto &pars = springsPrim->getParticles(); // tilevector
        {
            ompExec(zs::range(size), [pars = proxy<execspace_e::openmp>({}, pars), &obj, velsPtr, category, &prim,
                                      &auxAttribs, &dofVol, rho](int pi) mutable {
                ...
                for (auto &prop : auxAttribs) {
                    if (prop.numChannels == 3)
                        pars.tuple<3>(prop.name, pi) = prim->attr<vec3f>(std::string{prop.name})[pi];
                    else
        ...
        {
            springsPrim->elements = typename ZenoParticles::particles_t{eleTags, eleSize, memsrc_e::host};
            auto &eles = springsPrim->getQuadraturePoints(); // tilevector
            ompExec(zs::range(eleSize), [eles = proxy<execspace_e::openmp>({}, eles), velsPtr, &eleVol, &obj, &lines,
                                         &toTV3, rho, stiffness](int ei) mutable {
...
struct ToZSCloth : INode {
    void apply() override {
...

        {
            ...
            // surface
            {
                ...
                for (std::size_t i = 0; i != eleSize; ++i) {
                    ...
                    auto normal = cross(toTV3(eleD[i][0]).normalized(), toTV3(eleD[i][1]).normalized()).normalized();
        ...
        // attributes
        std::vector<zs::PropertyTag> tags{{"m", 1}, {"x", 3}, {"v", 3}, {"vol", 1}, {"C", 9}, {"beta", 1}};
        std::vector<zs::PropertyTag> eleTags{{"m", 1}, {"x", 3}, {"v", 3},     {"vol", 1}, {"C", 9},
                                             {"F", 9}, {"d", 9}, {"DmInv", 9}, {"inds", 3}};

        eleTags.emplace_back(zs::PropertyTag{"mu", 1});
        eleTags.emplace_back(zs::PropertyTag{"lam", 1});

        fmt::print("{} elements in process. pending {} particles with these attributes.\n", eleSize, size);
        for (auto tag : tags)
            fmt::print("tag: [{}, {}]\n", tag.name, tag.numChannels);

        zscloth->particles = std::make_shared<typename ZenoParticles::particles_t>(tags, size, memsrc_e::host);
        auto &pars = zscloth->getParticles(); // tilevector
        {
            ompExec(zs::range(size), [pars = proxy<execspace_e::openmp>({}, pars), &model, &obj, velsPtr, &dofVol,
                                      category](size_t pi) mutable {
                using vec3 = zs::vec<float, 3>;
                using mat3 = zs::vec<float, 3, 3>;
        ...
        ompExec(zs::range(eleSize), [eles = proxy<execspace_e::openmp>({}, eles), &model, &mu, &lam, velsPtr, &eleVol,
                                     &elePos, &obj, &eleVel, &eleD, category, &tris](size_t ei) mutable {
            using vec3 = zs::vec<double, 3>;
            using mat3 = zs::vec<double, 3, 3>;
            // vol, mass
            ...
            auto Dmat = mat3{D[0][0], D[1][0], D[2][0], D[0][1], D[1][1], D[2][1], D[0][2], D[1][2], D[2][2]};
            ...
            // file, You can obtain one at http://mozilla.org/MPL/2.0/.
            {
                ...
                if (std::abs(rv(1)) <= 10 * limits<float>::epsilon() ||
                    ...
                    fmt::print(fg(fmt::color::red),
                               ...
                               "\n\tt0[{}, {}, {}], \n\tt1[{}, {}, {}], \n\tnormal[{}, "
                               "{}, {}]\n",
                               ei, t0[0], t0[1], t0[2], t1[0], t1[1], t1[2], normal[0], normal[1], normal[2]);
#if 1
...
struct ToZSParticles : INode {
    void apply() override {
        ...
        if (bindMesh) {
            ...
            case ZenoParticles::tet: {
                ...
                for (std::size_t i = 0; i != eleSize; ++i) {
                    ...
                    eleVol[i] = v;
                    elePos[i] = (obj[quad[0]] + obj[quad[1]] + obj[quad[2]] + obj[quad[3]]) / 4;
                    if (velsPtr)
                        eleVel[i] = (velsPtr[quad[0]] + velsPtr[quad[1]] + velsPtr[quad[2]] + velsPtr[quad[3]]) / 4;
        ...
        // attributes
        std::vector<zs::PropertyTag> tags{{"m", 1}, {"x", 3},   {"v", 3},   {"vol", 1},
                                          {"C", 9}, {"vms", 1}, {"beta", 1} /*asflip, for positional adjustment*/};
        ...
        if (!bindMesh) {
            if (hasF)
                ...
                if (category != ZenoParticles::mpm)
                    throw std::runtime_error("mesh particles should not use the 'J' attribute.");
            }
        } else {
            eleTags.emplace_back(zs::PropertyTag{"mu", 1});
            eleTags.emplace_back(zs::PropertyTag{"lam", 1});
        ...
        auto &pars = outParticles->getParticles(); // tilevector
        {
            ompExec(zs::range(size),
                    [pars = proxy<execspace_e::openmp>({}, pars), hasLogJp, hasOrientation, hasF, &model, &obj, velsPtr,
                     nrmsPtr, bindMesh, &dofVol, category, &inParticles, &auxAttribs](size_t pi) mutable {
                        using vec3 = zs::vec<float, 3>;
                        using mat3 = zs::vec<float, 3, 3>;
                        ...
                        for (auto &prop : auxAttribs) {
                            if (prop.numChannels == 3)
                                pars.tuple<3>(prop.name, pi) = inParticles->attr<vec3f>(std::string{prop.name})[pi];
                            else
        ...
        if (bindMesh) {
            outParticles->elements = typename ZenoParticles::particles_t{eleTags, eleSize, memsrc_e::host};
            auto &eles = outParticles->getQuadraturePoints(); // tilevector
            ompExec(zs::range(eleSize), [eles = proxy<execspace_e::openmp>({}, eles), &model, &mu, &lam, velsPtr,
                                         nrmsPtr, &eleVol, &elePos, &obj, &eleVel, &eleD, category, &quads, &tris,
          ...
          getchar();
                    ...
                    if (Dstar(0, 0) <= limits<float>::epsilon()) {
                        fmt::print(fg(fmt::color::red),
                                   ...
                                   "\n\tbinormal[{}, "
                                   "{}, {}]\n",
                                   ei, tangent[0], tangent[1], tangent[2], Dmat(0, 1), Dmat(1, 1), Dmat(2, 1),
                                   Dmat(0, 2), Dmat(1, 2), Dmat(2, 2));
                        eles("mu", ei) = 0.f;
                        eles("lam", ei) = 0.f;
                    }
                    eles.tuple<9>("DmInv", ei) = invDstar;
...
struct ToBoundaryParticles : INode {
    void apply() override {
        ...
        default: throw std::runtime_error("boundary primitive is not a surface mesh!");
        } // end switch
        ...
        if (sprayed) { // compute volumes for remaining verts
            ...
            ompExec(zs::range(sprayedSize),
                    [table = proxy<execspace_e::openmp>(table), trisPtr, &sprayedVertToElement,
                     flag = proxy<execspace_e::openmp>(flag),
                     numSprayedParsPerElement = proxy<execspace_e::openmp>(numSprayedParsPerElement)](int pi) {
                        using IV = zs::vec<int, 3>;
                        int eid = table.query(IV{(int)(trisPtr[pi][0]), (int)(trisPtr[pi][1]), (int)(trisPtr[pi][2])});
                        if (eid == -1) {
                            printf("%d-th spray par face <%d, %d, %d> not found in entry!\n", pi, (int)(trisPtr[pi][0]),
                                   (int)(trisPtr[pi][1]), (int)(trisPtr[pi][2]));
                            flag[0] = 0;
                            return;
                        }
            ...
            ompExec(zs::range(sprayedSize),
                    [sprayedOffset, &sprayedVertToElement,
                     numSprayedParsPerElement = proxy<execspace_e::openmp>(numSprayedParsPerElement), &eleVol,
                     &dofVol](auto pi) {
                        using IV = zs::vec<int, 3>;
                        int eid = sprayedVertToElement[pi];
                        dofVol[pi + sprayedOffset] = eleVol[eid] / numSprayedParsPerElement[eid];
                    });
        }

        // attributes
        std::vector<zs::PropertyTag> tags{{"m", 1}, {"x0", 3}, {"x", 3}, {"v", 3}, {"nrm", 3}};
        std::vector<zs::PropertyTag> eleTags{{"m", 1}, {"x", 3}, {"v", 3}, {"nrm", 3}, {"inds", (int)category + 1}};
        if (sprayed) {
        ...
        outParticles->particles =
            std::make_shared<typename ZenoParticles::particles_t>(tags, totalSize, memsrc_e::host);
        auto &pars = outParticles->getParticles(); // tilevector
        ompExec(zs::range(size),
                [pars = proxy<execspace_e::host>({}, pars), &pos, velsPtr, &dofVol, sprayed](int pi) mutable {
                    using vec3 = zs::vec<float, 3>;
        ...
        if (sprayed)
            ompExec(zs::range(sprayedSize), [pars = proxy<execspace_e::host>({}, pars), &sprayedVertToElement, trisPtr,
                                             weightsPtr, &dofVol, category, &inParticles,
                ...
                if (tmp[0] >= sprayedOffset || tmp[1] >= sprayedOffset || tmp[2] >= sprayedOffset)
                    printf("damn! pids wrong!\n");

                // pos, vel, nrm all updated on-the-fly from original mesh verts
                pars.tuple<3>("x", dst) = ws[0] * pars.pack<3>("x", pids[0]) + ws[1] * pars.pack<3>("x", pids[1]) +
                                          ws[2] * pars.pack<3>("x", pids[2]);
                pars.tuple<3>("v", dst) = ws[0] * pars.pack<3>("v", pids[0]) + ws[1] * pars.pack<3>("v", pids[1]) +
        ...
        ompExec(zs::range(eleSize),
                [pars = proxy<execspace_e::openmp>({}, pars), eles = proxy<execspace_e::openmp>({}, eles), &eleVol,
                 &elePos, &eleVel, &tris](size_t ei) mutable {
                    using vec3 = zs::vec<float, 3>;
                    using mat3 = zs::vec<float, 3, 3>;
                    // mass
                    eles("m", ei) = eleVol[ei];
                    ...
                    // nrm
                    {
                        zs::vec<float, 3> xs[3] = {pars.pack<3>("x", tri[0]), pars.pack<3>("x", tri[1]),
                                                   pars.pack<3>("x", tri[2])};
                        auto n = (xs[1] - xs[0]).normalized().cross((xs[2] - xs[0]).normalized()).normalized();
        ...
        // vert normal
        ompExec(zs::range(size), [pars = proxy<execspace_e::openmp>({}, pars)](size_t pi) mutable {
            pars.tuple<3>("nrm", pi) = pars.pack<3>("nrm", pi).normalized();
        });
        if (sprayed)
            // reassign nrm to corresponding surface element
            ompExec(zs::range(sprayedSize), [sprayedOffset, pars = proxy<execspace_e::openmp>({}, pars),
        ...
        /// extract surface edges
        {
            ...
            auto &surfEdges = (*outParticles)[ZenoParticles::s_surfEdgeTag];
...
struct ScalePrimitiveAlongNormal : zeno::INode {
    virtual void apply() override {
        ...
        auto ompExec = zs::omp_exec();
        ompExec(zs::range(pos.size()), [&](size_t pi) { pos[pi] += nrm[pi] * dis; });
    }
};
...
struct ComputePrimitiveSequenceVelocity : zeno::INode {
    virtual void apply() override {
        ...
        if (prim0->size() != prim1->size())
            throw std::runtime_error("consecutive sequence objs with different topo!");
...
struct ComposeSdfVelField : INode {
    void apply() override {
        ...
        if (velLsPtr) {
            ...
            ls->getLevelSet() = const_sdf_vel_ls_t{sdfLsPtr->getBasicLevelSet(), velLsPtr->getBasicLevelSet()};
        } else {
            if (!sdfLsPtr->holdsBasicLevelSet()) {
                auto msg =
                    fmt::format("sdfField is {}a basic levelset.\n", sdfLsPtr->holdsBasicLevelSet() ? "" : "not ");
                throw std::runtime_error(msg);
            }
...
struct EnqueueLevelSetSequence : INode {
    void apply() override {
        using namespace zs;
        fmt::print(fg(fmt::color::green), "begin executing EnqueueLevelSetSequence\n");
        ...
        if (has_input<ZenoLevelSet>("ZSLevelSet")) {
            ...
            match([&lsseq](basic_ls_t &basicLs) { lsseq.push(const_sdf_vel_ls_t{basicLs}); },
                  [&lsseq](const_sdf_vel_ls_t &field) { // recommend
                      lsseq.push(field);                // also reset alpha in the meantime
                  },
                  [&lsseq](const_transition_ls_t &seq) {
                      lsseq._fields.insert(lsseq._fields.end(), seq._fields.begin(), seq._fields.end());
                  })(ls);
        }

        fmt::print(fg(fmt::color::cyan), "done executing EnqueueLevelSetSequence\n");
...
struct MakeZSString : INode {
    void apply() override {
        ...
        n->value =
            fmt::format(get_input<StringObject>("fmt_str")->get(), get_input<NumericObject>("frameid")->get<int>(),
...
struct UpdateLevelSetSequence : INode {
    void apply() override {
        using namespace zs;
        fmt::print(fg(fmt::color::green), "begin executing UpdateLevelSetSequence\n");
