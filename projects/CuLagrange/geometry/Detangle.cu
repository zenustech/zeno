        ...
        if(has_input<ZenoParticles>("kboundary") && detangle_with_boundary) {
            ...
            cudaExec(zs::range(kttemp.size()),[
                ...
                kvtemp = proxy<space>({},kvtemp)] ZS_LAMBDA(int kti) mutable {
                    ...
                    auto knrm = (ktvs[1] - ktvs[0]).cross(ktvs[2] - ktvs[0]);
                    auto nknrm = knrm.norm();
                    ...
                    if(nknrm < eps) {
                        ...
                        kttemp.tuple(dim_c<3>,"nrm",kti) = knrm / nknrm;
        ...
        for(int iter = 0;iter != nm_iters;++iter) {
            ...
            cudaExec(zs::range(tris.size()),[
                ...
                xtag = zs::SmallString(xtag)] ZS_LAMBDA(int ti) mutable {
                    ...
                    auto tnrm = (tvs[1] - tvs[0]).cross(tvs[2] - tvs[0]);
                    auto ntnrm = tnrm.norm();
                    // auto tnrm = LSL_GEO::facet_normal(tvs[0],tvs[1],tvs[2]);
                    if(ntnrm < eps) {
                        tris.tuple(dim_c<3>,"nrm",ti) = vec3::zeros();
                        tris("d",ti) = 0;
                    } else {
                        tnrm = tnrm / ntnrm;
            ...
            auto post_filter_the_update = get_input2<bool>("post_filter_the_update");
...
ZENDEFNODE(Detangle2, {
    {
        ...
        {gParamType_String,"collision_group",""},
        {gParamType_Bool,"skip_too_close_intersections_at_rest","0"},
        {gParamType_Int,"nm_detangle_restart_iters","1"},
        {gParamType_Float,"skip_distance","0"},
        {gParamType_Int,"nm_iters","1"},
        {gParamType_Bool,"use_global_scheme","0"},
        {gParamType_Float,"maximum_correction","0.1"},
        {gParamType_Float,"progressive_slope","0.1"},
        {gParamType_Float,"relaxation_rate","1"},
        {"kboundary"},
        {gParamType_Int,"substep_id","0"},
        {gParamType_Int,"nm_substeps","1"},
        {gParamType_Bool,"refit_kboundary_bvh","1"},
        {gParamType_Bool,"mark_intersection","0"},
        {gParamType_Bool,"use_cur_kine_configuration","1"},
        // {gParamType_Bool,"use_barycentric_interpolator","0"},
        {gParamType_Bool,"post_filter_the_update","1"},
        {gParamType_Bool,"enforce_boundary_normal","0"},
        {gParamType_Bool,"enforce_self_intersection_normal","0"},
        {gParamType_Bool,"detangle_with_boundary","1"},
        {gParamType_Bool,"do_self_detangle","1"},
        {gParamType_Bool,"skip_animation_intersection","1"},
        {gParamType_Bool,"among_same_group","1"},
        {gParamType_Bool,"among_different_group","1"}
    },
    {
        {"zsparticles"}
    },
