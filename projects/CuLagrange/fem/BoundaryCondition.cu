...
struct ApplyBoundaryOnVertices : INode {
    ...
    constexpr void projectBoundary(zs::CudaExecutionPolicy &cudaPol, LsView lsv, const ZenoBoundary &boundary,
                                   TileVecT &verts, float dt) {
        using namespace zs;
        auto collider = boundary.getBoundary(lsv);
        cudaPol(Collapse{verts.size()},
                [verts = proxy<execspace_e::cuda>({}, verts), boundary = collider, dt] __device__(int vi) mutable {
                    ...
                    if (boundary.queryInside(pos)) {
                        ...
                        if (boundary.type == collider_e::Sticky) {
                            ...
                            verts("BCorder", vi) = 3;
                            verts.template tuple<3>("BCtarget", vi) = verts.template pack<3>("BCtarget", vi) + dt * v;
    ...
    void apply() override {
        ...
        if (has_input<ZenoBoundary>("ZSBoundary")) {
            ...
            if (boundary->zsls)
                match([&](const auto &ls) {
                    if constexpr (is_same_v<RM_CVREF_T(ls), basic_ls_t>) {
                        ...
                        match([&](auto lsv) { projectBoundary(cudaPol, SdfVelFieldView{lsv}, *boundary, verts, dt); })(
            ...
            if (boundary->type != collider_e::Sticky) {
                cudaPol(Collapse{verts.size()},
                        [verts = proxy<execspace_e::cuda>({}, verts)] __device__(int vi) mutable {
                            ...
                            if (BCorder != 0) {
                                ...
                                verts.tuple<3>("BCtarget", vi) = BCbasis.transpose() * verts.pack<3>("BCtarget", vi);
                            }
                        });
            }
        }
...
struct MoveTowards : INode {
    ...
    void setupVelocity(zs::CudaExecutionPolicy &pol, VertsT &bouVerts, typename ZenoParticles::particles_t &next,
                       double dt) {
        using namespace zs;
        // update mesh verts
        bouVerts.append_channels(pol, {{"BCtarget", 3}});
        pol(Collapse{bouVerts.size()}, [prev = proxy<execspace_e::cuda>({}, bouVerts),
                                        next = proxy<execspace_e::cuda>({}, next), dt] __device__(int pi) mutable {
            auto newX = next.pack<3>("x", pi);
            prev.template tuple<3>("BCtarget", pi) = newX;
            prev.template tuple<3>("v", pi) = (newX - prev.template pack<3>("x", pi)) / dt;
        });
    }
    void apply() override {
        using namespace zs;
        ...
        if (has_input<ZenoParticles>("ZSBoundaryPrimitive")) {
            ...
            if (zsprimseq->hasImage(ZenoParticles::s_particleTag))
                setupVelocity(cudaPol, zsprimseq->getParticles<true>(), next->getParticles(), dt);
            setupVelocity(cudaPol, zsprimseq->getParticles(), next->getParticles(), dt);
        }
