...
#pragma omp parallel for
            for (int i = 0; i < attarr.size(); i++) {
                ...
                for (int i = 0; i < numprim->verts.size(); i++) {
                    auto &v = numprim->verts[i];
                    v = v * scale + pos;
                }
...
#pragma omp parallel for
                for (int i = 0; i < attarr.size(); i++) {
                    ...
                    for (int i = 0; i < offprim->verts.size(); i++) {
                        offprim->verts[i] += pos;
                    }
                    outprim2[i + attarr.size()] = offprim.get();
                    outprim[i + attarr.size()] = std::move(offprim);
                }
            }
        });
        if (includeSelf) {
