    ...
    struct mgData:IObject{
        ...
        void fillInner(){
            ...
            tag->setTree(std::make_shared<openvdb::FloatTree>(
                    sdf[0]->tree(), /*bgval*/ float(1),
                    openvdb::TopologyCopy()));
            int activeNum = sdf[0]->activeVoxelCount();
            ...
            auto extendTag = [&](const tbb::blocked_range<size_t> &r){
                ...
                auto sdf_axr{sdf[0]->getConstAccessor()};
            ...
            auto computeSDF = [&](const tbb::blocked_range<size_t> &r){
                ...
                auto sdf_axr{sdf[0]->getAccessor()};
                for (auto liter = r.begin(); liter != r.end(); ++liter) {
                    auto &leaf = *leaves[liter];
                    for (auto offset = 0; offset < leaf.SIZE; ++offset) {
                        auto coord = leaf.offsetToGlobalCoord(offset);
            ...
            for(int i=0;;++i)
            {
                ...
                int newAN = sdf[0]->activeVoxelCount();
        ...
        void cutOutGrid()
        {
            ...
            auto cutOuters = [&](const tbb::blocked_range<size_t> &r)
            {
                ...
                for (auto liter = r.begin(); liter != r.end(); ++liter) {
                    ...
                    for (auto offset = 0; offset < leaf.SIZE; ++offset) {
                        ...
                        if(!sdf_axr.isValueOn(coord))
                            continue;
                        if(sdf_axr.getValue(coord) > 0)
                            sdf_axr.setValueOff(coord);
                    }
                }
            }; 
            tbb::parallel_for(tbb::blocked_range<size_t>(0, leaves.size()), cutOuters);
        ...
        void recomputeSDF()
        {
            ...
            float dx = hLevels[0];
            sdfgrid->tree().getNodes(leaves);
            auto clearSDF= [&](const tbb::blocked_range<size_t> &r)
            {
                auto sdf_axr{sdfgrid->getAccessor()};
                for (auto liter = r.begin(); liter != r.end(); ++liter) {
                    auto &leaf = *leaves[liter];
                    for (auto offset = 0; offset < leaf.SIZE; ++offset) {
                        ...
                        if(!sdf_axr.isValueOn(coord))
                            continue;
                        float sdfV = sdf_axr.getValue(coord);
                        if(abs(sdfV) > dx)
                        {
                            sdf_axr.setValueOff(coord);
                        }
            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, leaves.size()), clearSDF);
            printf("heiha, clearSDFover\n");
            fillInner();
            printf("heiha, fill inner over\n");
        }
    };
