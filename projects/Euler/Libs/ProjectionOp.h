...
public:
    ...
    inline void operator()(const T& dt, const int& substep)
    {
        ...
        // bool negative_tot_energy = false;
        // bool negative_int_energy = false;
        auto flux_based_update = [&](const Vector<int, dim>& I) {
            if (field_helper.cell_type[field_helper.grid[I].idx] == CellType::GAS) {
                auto RK_coeffs = Bow::Math::TimeIntegration::TVDRK3<T, int>(substep);
                ...
                // // backup locally, for assert plot only
                // auto q_before_projection = field_helper.q[idx];
        ...
        field_helper.iterateGridParallel(flux_based_update);
        // BOW_ASSERT_INFO(!negative_density, "negative or zero density after
        // projection"); BOW_ASSERT_INFO(!negative_tot_energy, "negative or zero tot
        // energy after projection"); BOW_ASSERT_INFO(!negative_int_energy,
        // "negative or zero internal energy after projection");
    };
};
}
