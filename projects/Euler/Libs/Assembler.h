...
public:
    // if cut-cell is needed, compute coefficients on the fly will be needed as
    // well
    // TODO then we also need GS-LD integration
...
public:
    ...
    using FieldHelper = Bow::EulerGas::FieldHelperDense<T, dim, StorageIndex, XFastestSweep>;
    // Inputs:
        ...
        auto assemble_invM = [&](const IV& I) {
            if (get_idu(field_helper, I) >= 0) {
                ...
                field_helper.iterateKernel(
                    ...
                    I, -1, 2);
                BOW_ASSERT_INFO(M > 0, "negative or zero lumped mass");
                for (int d = 0; d < dim; d++) {
    ...
    void assemble_invS(FieldHelper& field_helper)
    {
        ...
        auto assemble_invS = [&](const IV& I) {
            if (get_idP(field_helper, I) >= 0) {
                ...
                // BOW_ASSERT_INFO(invS > 0, "negative or zero stiffness");
                invS_row[idP] = idP;
                invS_col[idP] = idP;
                invS_var[idP] = invS;
            }
        ...
        bool assemble_RHS_only = false)
    {
        ...
        // std::cout << "checking assembling results" + sys_name << std::endl;
