            ...
            if (proceed_to_save_mesh)
            {
                ...
                if (popuplate_vertex_maps)
                {
                    ...
                    for (vertex_array_iterator_t i = cc.vertices_begin(); i != cc.vertices_end(); ++i)
                    {
                        ...
                        else if (!m1_to_m0_cm_ovtx_colored.empty())
                        { // are we in the stitching stage..? (calling with "m1")
                        ...
                        if (m0_descr == mesh_t::null_vertex())
                        { // if still not found, then we are strictly "mX" polygons is "m0" polygons
                            m0_descr = mX_descr;
                        }
                ...
                if (popuplate_face_maps)
                {
                    ...
                    for (face_array_iterator_t f = cc.faces_begin(); f != cc.faces_end(); ++f)
                    {
                        ...
                        if (m1_to_m0_face_colored.size() > 0)
                        { // are we calling from during the patch stitching phase..?
        ...
        const int m0_num_cutpath_halfedges)
    {
        ...
        if (m0_h_is_ox)
        {
            ...
            { // was the opposite halfedge used to trace a polygon
                ...
                if (m0_sm_ihe_to_flag.at(prv_opp))
        ...
        { // then halfedge is either xx or xo (see the conditions with which function is called)
        ...
        if (!halfedges_on_same_side.empty())
        {                                                      // do we already have a halfedge on the [same side] which is tranformed...?
        ...
        { // do we already have a halfedge on the [other side] which is tranformed...?
  ...
  */
    ...
    void dispatch(output_t &output, const input_t &input)
    {
...
#else
        {
            ...
            do
            { // each iteration will find a set of edges that belong to a connected-component patch of intersectng faces (of sm or cm) in ps
                ...
                do
                { // each interation will add unregistered edges of current face, and add unvisited faces to queue
...
#if defined(MCUT_MULTI_THREADED)
        {
            ...
            std::tie(
                ...
                ps_tested_face_to_vertices) = partial_res;
            // merge results from other threads
...
#if defined(MCUT_MULTI_THREADED)
        {
                         ...
                         ++tested_faces_iter)
                    {
                        ...
                        if (have_plane_intersection)
                        {
                            ...
                            if (have_point_in_polygon)
                            {
                                ...
                                if (partial_cut_detected_LOCAL == false)
                                { // keep checking until (locally) true
            ...
            std::tie(
                ...
                intersection_points) = partial_res;
            ...
            { // one or more worker threads
                ...
                for (int i = 0; i < (int)futures.size(); ++i)
                {
                         ...
                         ++i)
                    {
                        ...
                        if (collision)
                        { // another thread also encounter the same edge
                         ...
                         ++i)
                    {
                        ...
                        if (collision)
                        { // another thread also encounter the same key
                            for (std::vector<vd_t>::const_iterator j = i->second.cbegin(); j != i->second.cend(); ++j)
                            {
                                const vd_t rel_descr = (*j);
                                const vd_t actual_descr = vd_t(intersection_point_descr_baseoffset + rel_descr);
                         ...
                         ++i)
                    {
                        ...
                        if (collision)
                        { // another thread also encounter the same face
                            for (std::vector<vd_t>::const_iterator j = i->second.cbegin(); j != i->second.cend(); ++j)
                            {
                                const vd_t rel_descr = (*j);
                                const vd_t actual_descr = vd_t(intersection_point_descr_baseoffset + rel_descr);
            ...
            char lp_intersection_result = geom::compute_line_plane_intersection(
                ...
                { // does the segment intersect the plane?
                    ...
                    { // the segment only touches the the plane (the line reprsented by segment still intersects the plane)
                        ...
                        if (segment_intersection_type == 'q' /*segment start*/ || segment_intersection_type == 'r' /*segment end*/)
                        { // only one segment end is touching plane
                            points_touching_plane.push_back((segment_intersection_type == 'q') ? &tested_edge_h0_source_vertex : &tested_edge_h0_target_vertex);
                        }
                        else
                        { // both point are in the plane, so we will need to confirm whether BOTH are outside our tested face.
                            points_touching_plane.push_back(&tested_edge_h0_source_vertex);
                            points_touching_plane.push_back(&tested_edge_h0_target_vertex);
                        }
                    ...
                    DEBUG_CODE_MASK(lg.unindent(););
                        ...
                        if (tested_edge_belongs_to_cm)
                        { // halfedge_pq belongs to cut mesh
    ...
    */
    ...
    if (!atleast_one_sm_edge_intersects_an_cs_face) {
        ...
        { // an iteration will build a cut-path sequence
            ...
            if (m0_ivtx_to_cutpath_edges_iter == m0_ivtx_to_cutpath_edges.cend())
            {          // still could not find any unmapped intersection point
        ...
        for (std::vector<int>::const_iterator it = explicit_cutpaths_making_holes.cbegin(); it != explicit_cutpaths_making_holes.cend(); ++it)
        {
            ...
            if (is_floating_polygon)
            {
                ...
                ed_t prev_edge = mesh_t::null_edge();
                do
                {
                    cur = next;
                    ...
                    if (prev_edge == mesh_t::null_edge() || fpi.polygon_vertices.size() < cutpath_sequence.size())
                    {
                        const std::vector<ed_t> &evec = ivtx_to_cp_edges.at(cur->first);
                        std::vector<ed_t>::const_iterator fiter = std::find_if(evec.cbegin(), evec.cend(), [&](const ed_t &e)
                                                                               { return e != prev_edge; });
        ...
        for (std::unordered_map<ed_t, std::vector<vd_t>>::const_iterator iter_ps_edge = ps_intersecting_edges.cbegin(); iter_ps_edge != ps_intersecting_edges.cend(); ++iter_ps_edge)
        {
            ...
            if (iter_ps_edge->second.size() > 1)
            { // intersection points on edge is more than 1 i.e. edge has more than three vertices
      ...
      if (x_coordinate_is_same) {
                                                         ...
                                                         InputStorageIteratorType block_end_) -> OutputStorageTypesTuple
            {
                ...
                for (edge_array_iterator_t iter_ps_edge = block_start_; iter_ps_edge != block_end_; ++iter_ps_edge)
                {
                    ...
                    else
                    {
                        ...
                        if (!m0_is_intersection_point(first, ps.number_of_vertices()))
                        { // o-->...
        ...
        for (edge_array_iterator_t iter_ps_edge = ps.edges_begin(); iter_ps_edge != ps.edges_end(); ++iter_ps_edge)
        {
            ...
            if (vertices_on_ps_edge.size() == 2) // simple case (edge did not intersect with any polygon)
            {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    const hd_t ps_edge_h = ps.halfedge(ps_edge, i);
                    if (ps_edge_h != mesh_t::null_halfedge())
                    { // note: ps_edge could be on the border!
                        const fd_t f = ps.face(ps_edge_h);
                        bool is_intersecting_ps_face = f != mesh_t::null_face() && ps_iface_to_m0_edge_list.find(f) != ps_iface_to_m0_edge_list.cend();
                        if (is_intersecting_ps_face)
                        {
            ...
            { // this is the more complex case where we add minimal set of non overlapping edges between 3 vertices
                    ...
                    else
                    {
                             ...
                             ++incident_edge_iter)
                        {
                            ...
                            for (int edge_he_iter = 0; edge_he_iter < 2; ++edge_he_iter)
                            {
                                ...
                                if (!m0_edge_he_src_is_ivertex && !m0_edge_he_tgt_is_ivertex)
                                { // o-->o (unmodified original edge)
                                ...
                                { // x-->x OR o-->x OR x-->o
                                    ...
                                    if (is_ox)
                                    {
                                        ...
                                        if (ps.face(tgt_ps_h) != ps_face)
                                        {
                                            ...
                                            MCUT_ASSERT(tgt_ps_h != mesh_t::null_halfedge());
                                        }

                                        const vd_t &m0_edge_he_src_as_ps_vertex = m0_to_ps_vtx.at(m0_edge_he_src);

                                        if (m0_edge_he_src_as_ps_vertex == ps.source(tgt_ps_h))
                                        { // is counter clock-wise halfedge
                                            first_boundary_halfedge = m0_edge_he;
                                            break;
                                        }
                                    }
                        ...
                        { // each iteration traces a child polygon
                            ...
                            { // each iteration walks a halfedge to incremetally trace a child polygon
                                ...
                                if (child_polygon.size() >= 3)
                                { // minimum halfedge count to constitute a valid polygon (triangle)
                                    ...
                                    if (m0.source(child_polygon.front()) == m0.target(child_polygon.back()))
                                    {
                                        ...
                                        { // ... if the current halfedge is the opposite of the first halfedge in list
        ...
        if (input.keep_cutmesh_seam)
        {
            ...
            if (sm_is_watertight || (all_cutpaths_are_circular || all_cutpaths_linear_and_make_holes))
            {
                ...
                if (separated_cut_mesh_fragments.size() == 1)
                {                                                                           // usual case
    ...
    for (std::map<hd_t, std::vector<int>>::const_iterator i = m0_h_to_ply.cbegin(); i != m0_h_to_ply.cend(); ++i) {
        ...
        { // atleast one cut-path makes a hole to be sealed later
                 ...
                 ++cs_ipoly_iter)
            {
                     ...
                     ++cs_poly_he_iter)
                {
                    ...
                    { // the current halfedge passed the sign test
                 ...
                 ++sm_poly_he_iter)
            {
                ...
                if (math::sign(scalar_prod) == math::NEGATIVE)
                {
                    ...
                    const std::vector<int>::const_iterator find_iter = std::find_if(
                        ...
                        [&](const int &e)
                        { return (e < traced_sm_polygon_count); });
        ...
        for (edge_array_iterator_t e = m0.edges_begin(); e != m0.edges_end(); ++e)
        {
            ...
            if (!(m0_is_intersection_point(m0_v0, ps_vtx_cnt) || m0_is_intersection_point(m0_v1, ps_vtx_cnt)))
            { // not coincident to an intersection vertex (i.e. is class-0 edge)
                ...
                if (m0_to_m1_vtx.at(m0_h0_src) == m1_halfedge_src)
                { // i.e. "is the m0_h0 equivalent to m1_halfedge?"
        ...
        do
        {
            ...
            do
            {
                ...
                { // process ih sequence starting from "seq_init_ihe"
                    ...
                    // o-->x OR x-->x
                    if (m0_cur_h_tgt_is_ivtx)
                    { // tgt vertex of current halfedge is an intersection point
                    ...
                    // x-->o OR x-->x
                    if (m0_cur_h_src_is_ivtx)
                    {                                 // src vertex of current halfedge is an intersection point
                    ...
                    if (!m0_cur_h_is_exterior || (m0_cur_h_is_exterior && m0_to_m1_ihe.find(m0_cur_h) == m0_to_m1_ihe.cend()))
                    {
                        ...
                        if (m0_cur_h_is_exterior)
                        { // is the current halfedge an exterior halfedge..?
                            ...
                            {
                                MCUT_ASSERT(std::find_if(fiter->second.cbegin(), fiter->second.cend(), [&](const std::pair<vd_t, hd_t> &p)
                                                         { return p.first == m1_cur_h_tgt; }) == fiter->second.cend());
                            ...
                            {
                                MCUT_ASSERT(std::find_if(fiter->second.cbegin(), fiter->second.cend(), [&](const std::pair<vd_t, hd_t> &p)
                                                         { return p.first == m1_cur_h_src; }) == fiter->second.cend());
            ...
            if (!class1_ihalfedge_found)
            { // The above search failed to find an untransformed class-1 halfedge.
                ...
                while (next_cs_border_he != mesh_t::null_halfedge())
                {
                    ...
                    do
                    {
                        ...
                        else
                        {
                            ...
                            if (opp_of_next_of_opp_of_cur_he_is_border)
                            { // found!
                 ...
                 ++patch_iter)
            {
                ...
                do
                {
                    ...
                    { // for each remaining halfedge of current polygon being stitched
                        ...
                        if (cur_is_last_to_be_transformed)
                        {
                            ...
                            { // class 3 : // x-->x
                                ...
                                { // check is interior ihalfedge (due ambiguity with exterior-interior halfedges x-->x)
                        ...
                        else if (!src_is_ivertex && tgt_is_ivertex)
                        { // class 1 : o-->x : this type of halfedge can only be "coming in" i.e. pointing torward source mesh
                            // o-->x
                        ...
                        */
                            ...
                            else
                            {
                                ...
                                if (opposite_patch_is_transformed) // if true, the current patch is the cw one
                                {
                                    ...
                                    if (m0_to_m1_he_instances_find_iter != m0_to_m1_he_instances.cend())
                                    { // must transformed at least once since opposite patch is transformed
                        ...
                        if (m0_to_m1_he_instances_find_iter == m0_to_m1_he_instances.end())
                        { // not yet transformed at all (i.e. m0_cur_patch_cur_poly_cur_he belongs to CCW polygon )
                         ...
                         ++m0_poly_he_iter)
                    {
                        ...
                        if (m0_to_m1_he_instances_find_iter != m0_to_m1_he_instances.cend())
                        { // check exists (i.e. m0_cur_patch_cur_poly_cur_he_opp has be transform but we dont know for which patch it has been transformed (CCW or CW)
                            ...
                            if (patch_idx_to_m1_he != patch_to_m1_he.cend())
                            { // check is stitched
                        ...
                        const std::vector<int>::const_iterator find_iter = std::find_if( // find the current polygon of current patch
                            ...
                            [&](const int poly_idx)
                            {
                                ...
                                if (is_ccw_patch)
                                { // is the current patch a "normal" patch?
