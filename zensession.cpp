...
namespace zen {
	...
	ZENAPI void INode::set_input_ref(std::string const & name, std::string const & srcname) {
		inputs[name] = srcname;
	}

	ZENAPI std::string INode::get_node_name() {
	...
	ZENAPI void INode::set_output_ref(std::string const & name, std::string const & srcname) {
		auto ref = get_output_ref(name);
	...
	ZENAPI std::vector<std::string> Session::getNodeRequirements(std::string const & name) {
		return safe_at(nodes, name, "node")->requirements();
	}
	ZENAPI void Session::setNodeParam(std::string const & name, std::string const & key, IValue const & value) {
		safe_at(nodes, name, "node")->set_param(key, value);
	}
	ZENAPI void Session::setNodeInput(std::string const & name, std::string const & key, std::string const & srcname) {
	...
	ZENAPI void Session::setReference(std::string const & name, std::string const & srcname) {
		auto refname = getReference(srcname).value_or(srcname);
		references[name] = refname;
	}
	ZENAPI std::optional<std::string> Session::getReference(std::string const & name) {
	...
	ZENAPI void Session::doDefNodeClass(std::unique_ptr<INodeClass> cls, std::string const & name, Descriptor const & desc) {
