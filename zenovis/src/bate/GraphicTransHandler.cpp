...
struct GraphicInteractTrans final : IGraphicHandler {
    Scene *scene;
    ...
    explicit GraphicInteractTrans(Scene *scene_, vec3f &center_)
...
std::shared_ptr<IGraphicHandler> makeGraphicInteractTrans(Scene *scene, vec3f center) {
    return std::make_shared<GraphicInteractTrans>(scene, center);
}
