...
struct GraphicsManager {
    ...
    void update_interactive_objects(const zeno::RenderObjsInfo& objs) {
        realtime_graphics.clear();
        for (auto& [key, obj] : objs.allObjects)
        {
            int interactive;
            if (obj->userData().has("interactive"))
                interactive = obj->userData().getLiterial<int>("interactive");
            ...
            if (interactive) {
                ...
                ig->nameid = key;
                ig->objholder = obj;
                realtime_graphics.m_curr.insert(std::make_pair(key, std::move(ig)));
                remove_object(key);
            }
            else {
                add_object(obj);
