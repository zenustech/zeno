...

    {
        ...
        if(reflect){
          ...
          if(metalPr>0.0f){
            ...
            tmpPdf *= (mat.roughness<=0.03f && reflectance==false)? 0.0f:1.0f;
        ...
        if(glassPr>0.0f)
        {
            ...
            if (reflect) {
              ...
              vec3 s = BRDFBasics::EvalMicrofacetReflection(ax, ay, wo, wi, wm,
                                                            mix(mix(Cspec0, mat.diffractColor, mat.diffraction), vec3(1.0f), F) * mat.specular,
                                            tmpPdf) * glassWt;
              tmpPdf *= (mat.roughness<=0.03f && reflectance==false)? 0.0f:F;
              s = s * (tmpPdf>0.0f? 1.0f:0.0f);
              sterm = sterm + s;
              f = f + s;
              fPdf += tmpPdf * glassPr;
              ...
              if(thin)
              {
                ...
                vec3 t = brdf * glassWt;
                tmpPdf *= (mat.roughness<=0.03f && reflectance==false)? 0.0f:(1.0f - F);
                t = t * (tmpPdf>0.0f? 1.0f:0.0f);
                tterm = tterm + t;
                f = f + t;
