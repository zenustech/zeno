...
namespace DisneyBSDF{
            ...
            )
    {
        ...
        float tax,tay;
        BRDFBasics::CalculateAnisotropicParams(roughness,anisotropic,tax,tay);

        float r0 = rnd(seed);
        float r1 = rnd(seed);
        vec3 wm = normalize(BRDFBasics::SampleGgxVndfAnisotropic(wo, tax, tay, r0, r1));
        ...
        float jacobian = LoH  / (sqrt(LoH + ior * VoH));
        pdf = (1.0f - F) / jacobian;
