...
struct NumericOperator : zeno::INode {
  ...
  template <class T, class...> using _left_t = T;

#define _PER_OP2(op, name)                                                     \
  template <class, class T1, class T2> struct _op_##name {                     \
  ...
  struct _op_##name<                                                           \
      std::void_t<decltype(std::declval<T1>() op std::declval<T2>())>, T1,     \
      T2> {                                                                    \
    static auto apply(T1 const &t1, T2 const &t2) { return t1 op t2; }         \
  };                                                                           \
                                                                               \
  template <class T1, class T2>                                                \
...
#define _PER_OP1(op, name)                                                     \
  template <class, class T1> struct _op_##name {                               \
    static int apply(T1 const &t1) {                                           \
      ...
      return 0;                                                                \
                                                                               ...
                                                                               \
  template <class T1>                                                          \
  struct _op_##name<std::void_t<decltype(op std::declval<T1>())>, T1> {        \
    static auto apply(T1 const &t1) { return op t1; }                          \
  };                                                                           \
                                                                               \
  template <class T1> static auto op_##name(T1 const &t1) {                    \
    return _op_##name<void, T1>::apply(t1);                                    \
  }

#define _PER_FN(name)                                                          \
  template <class, class... Ts> struct _op_##name {                            \
    static int apply(Ts const &... ts) {                                       \
      ...
      return 0;                                                                \
                                                                               ...
                                                                               \
  template <class... Ts>                                                       \
  struct _op_##name<std::void_t<decltype(zeno::name(std::declval<Ts>()...))>,  \
                    Ts...> {                                                   \
    static auto apply(Ts const &... ts) { return zeno::name(ts...); }          \
  };                                                                           \
                                                                               \
  template <class... Ts> static auto op_##name(Ts const &... ts) {             \
  ...
  virtual void apply() override {
    ...
    if (has_input("rhs")) {
      ...
      if (op == "set")
        lhs->value = rhs->value;
      std::visit(
          [op, &ret](auto const &lhs, auto const &rhs) {
            if (op == "copy")
              ret->value = lhs;
            else if (op == "copyr")
              ret->value = rhs;
...
struct PrintNumeric : zeno::INode {
  template <class T> struct do_print {
    do_print(T const &x) { std::cout << x; }
  };

  template <size_t N, class T> struct do_print<zeno::vec<N, T>> {
  ...
  virtual void apply() override {
    ...
    std::cout << hint << ": ";
    std::visit([](auto const &val) { do_print _(val); }, obj->value);
    std::cout << std::endl;
  }
};
...
struct NumericInterpolation : zeno::INode {
  template <class T1, class T2, class T3, class Void = void> struct uninterp {
  ...
  template <class T1, class T2, class T3, class Void = void> struct interp {
  ...
  template <class T1, class T2, class T3>
  struct interp<T1, T2, T3,
                std::void_t<decltype(std::declval<T1>() * (std::declval<T3>() -
                                                           std::declval<T2>()) +
                                     std::declval<T2>())>> {
    auto operator()(T1 const &fac, T2 const &dstMin, T3 const &dstMax) {
  ...
  virtual void apply() override {
    ...
    auto srcMin = has_input("srcMin")
                      ...
                      : 0;
    auto srcMax = has_input("srcMax")
                      ? get_input<zeno::NumericObject>("srcMax")->value
                      : 1;
    auto dstMin = has_input("dstMin")
                      ? get_input<zeno::NumericObject>("dstMin")->value
                      : 0;
    auto dstMax = has_input("dstMax")
                      ? get_input<zeno::NumericObject>("dstMax")->value
                      : 1;

    zeno::NumericValue fac;
    std::visit([&fac](auto src, auto srcMin,
                      auto srcMax) { fac = uninterp_f(src, srcMin, srcMax); },
               src, srcMin, srcMax);

    zeno::NumericValue dst;
    std::visit([&dst](auto fac, auto dstMin,
                      auto dstMax) { dst = interp_f(fac, dstMin, dstMax); },
               fac, dstMin, dstMax);
