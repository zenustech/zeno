...
#pragma omp parallel for
        ...
        // vec3 场数据可视化 v2，速度快很多
        struct TraceVec3FieldOneStep_v2 : INode {
            void apply() override
            {
                // 额外的标记
                int primOutPutType = 1;         // 0 = ref, 1 = copy
                ...
                // OneStep 之前点和线的数量
                int primVisVertsCount = primVis->size();
                int primVislinesCount = primVis->lines.size();

                // 为填入新的点和线准备空间
                int primDataVertsCount = primData->size();;
...
#pragma omp parallel for
        ...
        ZENDEFNODE(TraceVec3FieldOneStep_v2,
                   {  /* inputs: */ {
                           "primData",
                           "primEmpty",
                       }, /* outputs: */ {
...
//            set_output("prim", get_input("prim"));
        ...
        struct PrimSetAttrPerVertByName : zeno::INode {
            void apply() override {
                auto prim = get_input<PrimitiveObject>("prim");
...
//                       "prim",
        ...
        struct PrimGetAttrPerVertByName : zeno::INode {
            void apply() override {
                auto prim = get_input<PrimitiveObject>("prim");
                //auto name = std::get<std::string>(get_param("name"));
                auto name = get_input2<std::string>("name");
