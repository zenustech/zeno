...
struct tuple_hash {
    static constexpr std::size_t magics[] = {
        2718281828, 3141592653, 1618033989,
    };

    template <class T, std::size_t ...Is>
    std::size_t _call_helper(T const &t, std::index_sequence<Is...>) {
    ...
    std::size_t operator()(T const &t) const {
        static_assert(std::tuple_size_v<T> < std::size(magics), "maybe you want to add more magic numbers");
...
struct tuple_operator {
    ...
    template <class T, std::size_t ...Is>
    bool _call_helper(T const &t1, T const &t2, std::index_sequence<Is...>) {
