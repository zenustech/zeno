...
struct PrimitiveBend : zeno::INode {
    virtual void apply() override {
        ...
        if (std::abs(angle) > 0.005f && limitMax - limitMin > 0.001f && prim->size() != 0) {
            ...
            auto acc = parallel_reduce((size_t)0, prim->size(), vec4f(tanv0, tanv0, dirv0, dirv0), [&] (auto a, auto b) { return vec4f(std::min(a[0], b[0]), std::max(a[1], b[1]), std::min(a[2], b[2]), std::max(a[3], b[3])); }, [&] (size_t i) {
