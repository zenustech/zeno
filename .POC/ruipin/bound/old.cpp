...
public:
    constexpr Streamed(T &t) : m_t(t) {}

    constexpr Streamed &operator=(T const &t) {
        _mm_stream_si32(&reinterpret_cast<int &>(m_t), reinterpret_cast<int const &>(t));
        return *this;
    }
};

template <class T>
static Streamed<T> streamed(T &t) { return t; }

template <size_t Step, class It, class Body, class Rest>
static void tiled_range_for(It a, It b, Body &&body, Rest &&rest) {
    It i = a, end = b - (Step - 1);
    for (i = a; i < end; i += Step) {
        body(i);
    }
    for (; i < b; ++i) {
        rest(i);
    ...
    [&] (tbb::blocked_range<size_t> const &ro) {
        for (size_t t = ro.begin(); t < ro.end(); t++) {
            ...
            for (size_t z = std::max<size_t>(1, r.pages().begin()); z < std::min(n - 2, r.pages().end()); z++) {
                for (size_t y = std::max<size_t>(1, r.cols().begin()); y < std::min(n - 2, r.cols().end()); y++) {
                    for (size_t x = std::max<size_t>(1, r.rows().begin()); x < std::min(n - 2, r.rows().end()); x++) {
                        streamed(b[(z * n + y) * n + x]) = -6 * a[(z * n + y) * n + x]
                            + a[(z * n + y) * n + x + 1]
                            + a[(z * n + y) * n + x - 1]
                            + a[(z * n + y) * n + x + n]
                            + a[(z * n + y) * n + x - n]
