...
// The above preprocessor symbols are defined by compilers when exception handling is enabled.
 ...
 #pragma warning(disable:4190)	// Function has C-linkage specified, but returns C++ type
...
extern "C"
void handle_win_error( int error_code ) throw(std::runtime_error)
{
    throw std::runtime_error(decode_win_error(error_code));
}

#ifdef _MSC_VER
 #pragma warning(default:4290)	// C++ exception specification ignored
 #pragma warning(default:4190)	// Function has C-linkage specified, but returns C++ type
